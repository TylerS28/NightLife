RegisterNetEvent("srp-weapons:attackedByCash")
AddEventHandler("srp-weapons:attackedByCash", function(pAttacker)
    local victim = source
    TriggerClientEvent("srp-weapons:hitPlayerWithCash", pAttacker, victim)
end)

RegisterNetEvent("srp-weapons:processGiveCashAmount")
AddEventHandler("srp-weapons:processGiveCashAmount", function(pTarget, pAmount, pId)
    local attacker = source
    local victim = pTarget
    local victimUser = exports["srp-fw"]:getModule("Player"):GetUser(victim)

    victimUser:addMoney(pAmount)
            
    exports["srp-log"]:AddLog("Weapons", 
