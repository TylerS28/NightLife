...
function DPX.Core.ConsoleLog(self, msg, mod, ply)
	...
	if ply and tonumber(ply) then
		TriggerClientEvent("srp-fw:consoleLog", ply, msg, mod)
	end
end

AddEventHandler("onResourceStart", function(resource)
	TriggerClientEvent("srp-fw:waitForExports", -1)
...
RegisterNetEvent("srp-fw:playerSessionStarted")
...
AddEventHandler("srp-fw:characterLoaded", function(user, char)
	local src = source
...
-- 		print("^1[srp-fw] Paychecks Sent^0")
-- 	end
-- end)
	...
	exports.oxmysql:execute('SELECT `paycheck` FROM characters WHERE `id`= ?', {cid}, function(data)
		...
		if amount >= 1 then
			...
			local connect = {
				{
				  ["color"] = color,
				  ["title"] = "** qpixel [Paychecks] **",
				  ["description"] = "State ID: "..cid.." \n First Name: "..pChar.first_name.." Last Name: "..pChar.last_name.." \n Amount: $"..amount,
				}
			  }
			  PerformHttpRequest("https://discord.com/api/webhooks/1012082835915550830/uw2ZSnraggccZeZQLgFTKtXiWhWJdvOwz07AEGrHk8q3JDcot5w7ERpJU59qQX3ueO8O", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | Paycheck", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
