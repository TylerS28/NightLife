function DPX.Core.LoginPlayer(self, args, src, callback)
    TriggerEvent("srp-fw:playerAttemptLogin", src)
    ...
    local function fetchData(_err)
        ...
        DPX.DB:FetchPlayerData(src, function(data, err)
            ...
            if not err then TriggerEvent("srp-fw:playerLoggedIn", user) TriggerClientEvent("srp-fw:playerLoggedIn", src) end
        end)
    end
	...
	DPX.DB:PlayerExistsDB(src, function(exists, err)
		...
		if not exists then
			DPX.DB:CreateNewPlayer(src, function(created)
				...
				TriggerClientEvent('srp-admin:InsertPrioCL', src)
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.LoginPlayer, "srp-fw:loginPlayer")
...
function DPX.Core.FetchPlayerCharacters(self, args, src, callback)
	...
	DPX.DB:FetchCharacterData(user, function(data, err)
		if err then
			...
			TriggerEvent("srp-fw:charactersLoaded", user, data)
			TriggerClientEvent("srp-fw:charactersLoaded", src, data)
		end

		callback(data)
	end)
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.FetchPlayerCharacters, "srp-fw:fetchPlayerCharacters")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.CreateCharacter, "srp-fw:createCharacter")
...
DPX.Events:AddEvent(DPX.Core, DPX.Core.DeleteCharacter, "srp-fw:deleteCharacter")
...
function DPX.Core.SelectCharacter(self, id, src, callback)
	...
	TriggerClientEvent('srp-fw:setcontrols', src)
	TriggerClientEvent('updatepasses', src)

	TriggerEvent("srp-fw:characterLoaded", user, selectedCharacter)
	TriggerClientEvent("srp-fw:characterLoaded", src, selectedCharacter)

	callback({loggedin = true, chardata = selectedCharacter})
end
DPX.Events:AddEvent(DPX.Core, DPX.Core.SelectCharacter, "srp-fw:selectCharacter")