...
RegisterNetEvent('srp-jobs:passed')
AddEventHandler('srp-jobs:passed', function()
    pisInDodoJob = true
    ...
    TriggerEvent('srp-phone:goToLocation')
end)

RegisterNetEvent('srp-jobs:startDodoLogistics')
AddEventHandler('srp-jobs:startDodoLogistics', function()
    TriggerEvent('srp-phone:signInDodo')
    ...
    TriggerEvent('srp-phone:dodoJobOffer')
end)

RegisterNetEvent('srp-jobs:dodoLogisticsPayout')
AddEventHandler('srp-jobs:dodoLogisticsPayout', function()
    if pOnWayDodo == true then
        if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder"))  then
            if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                TriggerServerEvent('srp-jobs:dodoLogisticsPayout')
    ...
    while true do
        ...
        if isToDropLocation == true then
            ...
            if GetDistanceBetweenCoords(px,py,pz, GetEntityCoords(GetPlayerPed(-1),true)) < 2.5 then
                if IsVehicleModel(GetVehiclePedIsIn(GetPlayerPed(-1), true), GetHashKey("pounder")) then
                    exports['srp-interface']:showInteraction('[E] Drop Off Package')
                    if IsControlJustPressed(1,38) then
                        TriggerEvent('srp-phone:goBackDepo')
                        exports['srp-interface']:hideInteraction()
                            ...
                            local aDist = GetDistanceBetweenCoords(setCoords, coordA)
                                if aDist < 15.0 then
                                    ...
                                    if exports['srp-hud']:MoneyBuff() then
                                        TriggerServerEvent('srp-jobs:givePayout', math.random(878, 927), "bank", '[Dodo Logistics | With Buff]')
                                    else
                                        TriggerServerEvent('srp-jobs:givePayout', math.random(650, 778), "bank", '[Dodo Logistics | Without Buff]')
                                    end
                                end
                            end
                        end)
...
RegisterNetEvent("srp-jobs:spawn-truck")
AddEventHandler("srp-jobs:spawn-truck", function()
	Citizen.CreateThread(function()
...
exports["srp-polytarget"]:AddCircleZone("dodo_sign_in", vector3(-437.91, -2795.5, 7.39), 0.6, {
    useZ=true,
})

exports["srp-interact"]:AddPeekEntryByPolyTarget("dodo_sign_in", {{
    event = "srp-jobs:startDodoLogistics",
    id = "dodo_sign_in",
    icon = "circle",
    label = "Sign In",
    parameters = {},
