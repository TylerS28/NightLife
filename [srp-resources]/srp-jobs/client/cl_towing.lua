...
RegisterNetEvent('srp-jobs:tow_trucker_duty')
AddEventHandler('srp-jobs:tow_trucker_duty', function()
	if not clockedon then
		...
		clockedon = true
		TriggerEvent('srp-phone:signInInfoTowing')
		TriggerServerEvent('srp-duty:tow_trucker')
		TriggerEvent('srp-jobs:give_tow_job')
	else
		TriggerEvent('DoLongHudText', 'You\'re already clocked in.', 2)
	end
end)

RegisterNetEvent('srp-jobs_tow_trucker_off_duty')
AddEventHandler('srp-jobs_tow_trucker_off_duty', function()
	if clockedon then
...
RegisterNetEvent('srp-jobs:towing_accept')
AddEventHandler('srp-jobs:towing_accept', function()
	GiveTowJobWanker()
end)
...
function GiveTowJobWanker()
	...
	local vehicle = exports['srp-vehicles']:GetVehicleTable()
			...
			Citizen.CreateThread(function()
				while Aspect.Looking do
					...
					if #(vector3(Aspect.Location.x, Aspect.Location.y, Aspect.Location.z) - GetEntityCoords(PlayerPedId())) <= 320 then
						...
						setimpoundveh = plate
                        TriggerEvent('srp-fuel:setFuel', GetVehiclePedIsIn(PlayerPedId()), 100)
...
RegisterNetEvent('srp-jobs:give_tow_job')
AddEventHandler('srp-jobs:give_tow_job', function()
	...
	if clockedon then
		TriggerEvent('srp-phone:towing_offer')
	end
end)
...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("qpixel_impound_location", vector3(-236.51, -1172.86, 37.6), 20, 27.6, {
...
AddEventHandler('srp-polyzone:enter', function(name)
    if name == "qpixel_impound_location" then
        DesirePolyImp = true     
    end
end)
...
RegisterNetEvent('srp-jobs:impound_confirm_shit_idk_anymore')
AddEventHandler('srp-jobs:impound_confirm_shit_idk_anymore', function()
	if DesirePolyImp and CamImp then
		TriggerEvent('srp-jobs:towing_imp')
	end
end)

RegisterNetEvent('srp-jobs:impound_accept')
AddEventHandler('srp-jobs:impound_accept', function()
	...
	if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
		...
		if licensePlate == setimpoundveh then
			...
			if exports['srp-hud']:MoneyBuff() then
				TriggerServerEvent('srp-jobs:givePayout', math.random(1076, 1554), "cash", '[24/7 Delivery Job | With Buff]')
			else
				TriggerServerEvent('srp-jobs:givePayout', math.random(652, 886), "cash", '[24/7 Delivery Job | Without Buff]')
			end
			TriggerEvent('srp-jobs:give_tow_job')
...
RegisterNetEvent('srp-jobs:imp_decline')
AddEventHandler('srp-jobs:imp_decline', function()
...
RegisterNetEvent('srp-jobs:towing_imp')
AddEventHandler('srp-jobs:towing_imp', function()
	...
	if DoesEntityExist(veh) and NetworkHasControlOfEntity(veh) then
		...
		if licensePlate == setimpoundveh then
			if CanImp then
				TriggerEvent('srp-jobs_tow_menu')
			end
		else
...
RegisterNetEvent('srp-jobs_tow_menu')
AddEventHandler('srp-jobs_tow_menu', function()
	...
	local TowMenu = {
		...
		{
			title = "Accept",
			action = "srp-jobs:accept_impound_req",
		},
		{
			title = "Decline",
			action = "srp-jobs:decline_impound_req",
		},
	}
	exports["srp-interface"]:showContextMenu(TowMenu)
end)

RegisterInterfaceCallback('srp-jobs:accept_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('srp-jobs:impound_accept')
end)

RegisterInterfaceCallback('srp-jobs:decline_impound_req', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerEvent('srp-jobs:imp_decline')
end)
...
exports["srp-polyzone"]:AddBoxZone("towing_shit", vector3(-191.63, -1166.46, 23.67), 2, 2, {
	name="towing_shit",
	heading=0,
	--debugPoly=false,
	minZ=21.27,
...
AddEventHandler('srp-polyzone:enter', function(name)
	if name == "towing_shit" then
		...
		pSignOnOff()
		if not clockedon then
			exports['srp-interface']:showInteraction('[E] Sign In')
		else
			exports['srp-interface']:showInteraction('[E] Sign Out', 'error')
		end
	end
end)
	...
	Citizen.CreateThread(function()
        while towing_duty do
			...
			if IsControlJustReleased(0, 38) then
				if not clockedon then
					TriggerEvent('srp-jobs:tow_trucker_duty')
					exports['srp-interface']:hideInteraction()
				elseif clockedon then
					DeleteEntity(pVeh)
					TriggerEvent('srp-jobs_tow_trucker_off_duty')
					exports['srp-interface']:hideInteraction()
				end
			end
		end
...
AddEventHandler('pv:tow', function()
	...
	if isVehicleTow then
		...
		if not currentlyTowedVehicle then
		    ...
		    local finished = exports["srp-taskbar"]:taskBar(15000,"Hooking up vehicle.")
        	if finished == 100 and aDist < 5.0 then
				...
				if targetVehicle ~= 0 then
					...
					local part2 = exports["srp-taskbar"]:taskBar(7000,"Towing Vehicle")
		    ...
		    TriggerEvent("animation:tow")
		    TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck', 0.5)
		    local finished = exports["srp-taskbar"]:taskBar(7000,"Untowing Vehicle")
        	if finished == 100 and aDist < 2.0 then
                hasVehicleHookedUp = false

        		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10.0, 'towtruck2', 0.5)
        		local part2 = exports["srp-taskbar"]:taskBar(7000,"Unhooking Vehicle")
