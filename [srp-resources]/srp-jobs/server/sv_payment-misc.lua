...
AddEventHandler('server:GroupPayment', function( gangid, cashamount, sauce)
  ...
  RegisterServerEvent('srp-weedbox:payout')
AddEventHandler('srp-weedbox:payout', function(money)
    local source = source
        ...
        for i = 1, #Registers[regID] do
            if Registers[regID][i].regID == regID then
                ...
                if (tonumber(user:getCash()) >= tonumber(amount)) then
                    ...
                    local owner = exports["srp-fw"]:getModule("Player"):GetUser(Registers[regID][i].owner)
...
RegisterServerEvent("srp-jobs:cosmic_cannabis:update:pay")
AddEventHandler("srp-jobs:cosmic_cannabis:update:pay", function(cid)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
