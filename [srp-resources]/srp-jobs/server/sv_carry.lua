...
RegisterServerEvent('srp-jobs:galleryPayout')
AddEventHandler('srp-jobs:galleryPayout', function(amount)
    local src = source
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** qpixel [Gallery] | Payout Log **",
          ["description"] = "Gallery Payout Log | Amount: $"..amount.." | Character Name: "..charInfo.first_name.." "..charInfo.last_name.." | State ID: "..charInfo.id,
        }
      }
      PerformHttpRequest("https://discord.com/api/webhooks/1012082878152192011/LWs9LAtXdH1a9UxFp9VDubp7RVmgAJopKuG1AIovHhsYNEbQI53dormyk2mM9XRRj-3C", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)