RegisterServerEvent('srp-impound:select_plate')
AddEventHandler('srp-impound:select_plate', function(pPlate)
    local src = source
    ...
    {['@license_plate'] = pPlate}, function(pValues)
        if pValues[1].current_garage == "Full Impound" or pValues[1].current_garage == "Normal Impound" then
            if char.id == pValues[1].cid then
                TriggerClientEvent('srp-impound:show_car', src, pValues[1].name, pValues[1].license_plate, false)
            else
...
RegisterServerEvent('srp-impound:release_car')
AddEventHandler('srp-impound:release_car', function(pPlate)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent('srp-imp:FullImpound')
AddEventHandler('srp-imp:FullImpound', function(plate)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    {['@license_plate'] = plate}, function(pValues)
        if pValues[1].vin == "1" then
            ...
            local connect = {
                {
                  ["color"] = color,
                  ["title"] = "** qpixel [Impound] | Vin Scratch **",
              ...
              PerformHttpRequest("https://discord.com/api/webhooks/1012082431773388820/96MoH8laPVGfI_vJTWySg3hFDpXfIuw4BcOQECs7gpPOlOBf0AqYGbP0R0b0cGtEUemj", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
RegisterNetEvent('srp-imp:NormalImpound')
AddEventHandler('srp-imp:NormalImpound', function(plate)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    {['@license_plate'] = plate}, function(pValues)
        if pValues[1].vin == "1" then
            ...
            local connect = {
              ...
              PerformHttpRequest("https://discord.com/api/webhooks/1012082407526125600/cc8qwHIT92E4rdgm6OTfJj30P1wUdlQLlmcDu0sRhJ1rgk3pbcZAMmB3XMqzWS2rVq-k", function(err, text, headers) end, 'POST', json.encode({username = "qpixel | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
...
RegisterNetEvent('srp-imp:ScuffImpound')
AddEventHandler('srp-imp:ScuffImpound', function(plate)
