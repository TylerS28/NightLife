...
RegisterNetEvent('srp-dispatch:altercallsign;spawn')
AddEventHandler('srp-dispatch:altercallsign;spawn',function(getCallsign,pStatusType)
    exports['srp-dispatch']:callsign_command(getCallsign,pStatusType)
end)

RegisterNetEvent('srp-dispatch:beepEffect')
AddEventHandler('srp-dispatch:beepEffect', function()
    if exports['isPed']:isPed('myjob') == "police" then
        TriggerServerEvent('InteractSound_SV:PlayOnSource', '10-1314', 0.2)
    end
end)

RegisterNetEvent('srp-dispatch:stolenCar')
AddEventHandler('srp-dispatch:stolenCar', function(plate)
    local playerPed = PlayerPedId()	
    local padCoords = GetEntityCoords(playerPed)
    local alertCoord = vector3(padCoords.x, padCoords.y, padCoords.z)
...
RegisterCommand('911', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        ...
        local cancelled = exports['srp-taskbar']:taskBar(7000,'Dialing 911')
...
RegisterCommand('311', function(source, args, rawCommand)
    ...
    if string.len(msg) > 0 then
        ...
        local phone_number = user.phone_number
        TriggerEvent('animation:phonecall')
        local cancelled = exports['srp-taskbar']:taskBar(7000,'Dialing 311')
        if cancelled == 100 then
        ...
        if IsPedShooting(PlayerPedId()) then
            if math.random(100) <= 10 then
                if IsPedInAnyVehicle(PlayerPedId()) then
                    ...
                    exports["srp-dispatch"]:dispatchadd('10-71', "Gun shots Reported", "432")
                    canSendGunshot = false
                    Citizen.Wait(3000)
                    canSendGunshot = true
                end
