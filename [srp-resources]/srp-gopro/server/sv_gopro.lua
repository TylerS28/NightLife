RPC.register("srp-gopros:addSecCamera", function(pSource, pName, pCoords, pRotation)
...
RPC.register("srp-gopros:getSecurityCameraById", function(pSource, pCameraID)
    local result = MySQL.query.await([[
...
RPC.register("srp-gopros:getSecurityCameraByStateID", function(pSource, pCid)
...
RPC.register("srp-gopros:getSecurityCameraAccessByCameraID", function(pSource, pCamera)
...
RPC.register("srp-gopros:addUserToCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})

    print(pCid)
    local pCid = exports['srp-phone']:getCIDFromPhoneNumber(pCid)
    print(pCid)
...
RPC.register("srp-gopros:removeUserFromCamera", function(pSource, pCamera, pCid)
    local camera = MySQL.query.await([[
        SELECT * FROM _cameras WHERE id = ?
    ]], {pCamera})
