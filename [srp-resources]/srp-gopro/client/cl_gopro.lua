...
RegisterNetEvent("srp-gopros:updateRegisteredCams")
AddEventHandler("srp-gopros:updateRegisteredCams", function(pDashcams)
  activeDashcams = pDashcams
  exports["srp-interface"]:sendAppEvent("gopros", { dashcams = activeDashcams })
end)
AddEventHandler("playerSpawned", function()
  Wait(5550)
  RPC.execute("srp-gopros:playerSpawnedSendCams")
end)

RegisterCommand("srp-gopros:addUserToCamera", function()
  RPC.execute("srp-gopros:addUserToCamera",1,868)
  
  end)
  

--[[
RegisterUICallback("srp-interface:goproChangeSelectedPov", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = 'done' }})
  local activateGoPro = activateGoPro(tonumber(data.netId))
  Wait(600)
  exports["srp-interface"]:sendAppEvent("gopros", { switchingViews = false })
end)
]]

AddEventHandler("srp-interface:application-closed", function(name)
  if name ~= "gopros" then return end
  DoScreenFadeOut(400)
  Wait(400)
  TriggerEvent("srp-casino:inVRHeadset", false)
  deactivateGoPro()
  ...
  exports["srp-interface"]:sendAppEvent("hud", { display = true })
  DoScreenFadeIn(1000)
end)
          ...
          if IsControlJustReleased(0, 38) then
              ...
              if destinationSelected then
                ...
                exports['srp-interface']:openApplication('textbox', {
                  callbackUrl = 'srp-interface:goProRegisterSecCam',
                  key = 1,
                  items = {
                    {
                      icon = "pencil-alt",
                ...
                local camPos = exports["srp-selector"]:getCurrentSelection()
                ...
                Wait(100)
                exports["srp-interface"]:showInteraction(("[E] %s"):format("View Here"))
                destinationSelected = true
                listenForKeypress()
                exports["srp-selector"]:startSelecting(1, 1, function()
                  return false
                end)
              end
          end
...
AddEventHandler("srp-inventory:itemUsed", function(item)
  ...
  local result = exports["srp-objects"]:PlaceObject(
    `prop_spycam`,
    {
      collision = false,
      groundSnap = false,
  ...
  local prompt = exports["srp-interface"]:OpenInputMenu({
    {
      name = 'name',
      label = 'Security Camera Stream Name',
      icon = 'pencil-alt'
  ...
  local success = RPC.execute("srp-gopros:addSecCamera", prompt.name, result[2].coords, result[2].rotation)
  if success then
...
local function activateSecurityCameraById(pId)
  local cam = RPC.execute("srp-gopros:getSecurityCameraById", pId)
  ...
  exports["srp-interface"]:sendAppEvent("hud", { display = false })
  DoScreenFadeIn(1000)
end
...
--[[
RegisterUICallback("srp-interface:gopros:getSecurityCameras", function(data, cb)
  local cid = exports["isPed"]:isPed("cid")
  local cams = RPC.execute("srp-gopros:getSecurityCameraByCid", cid)
  cb({ data = cams, meta = { ok = true, message = "done" } })
end)

RegisterUICallback("srp-interface:gopros:viewCameraById", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  Wait(500)
  exports["srp-interface"]:closeApplication("phone")
...
Citizen.CreateThread(function()
  exports["srp-keybinds"]:registerKeyMapping("", "Camera", "Look Up", "+securityCamLookUp", "-securityCamLookUp", "PAGEUP")
  exports["srp-keybinds"]:registerKeyMapping("", "Camera", "Look Down", "+securityCamLookDown", "-securityCamLookDown", "PAGEDOWN")
  exports["srp-keybinds"]:registerKeyMapping("", "Camera", "Look Left", "+securityCamLookLeft", "-securityCamLookLeft", "HOME")
  exports["srp-keybinds"]:registerKeyMapping("", "Camera", "Look Right", "+securityCamLookRight", "-securityCamLookRight", "END")
  exports["srp-keybinds"]:registerKeyMapping("", "Camera", "Turn Off", "+securityCamOff", "-securityCamOff", "ESCAPE")
  ...
  exports["srp-interact"]:AddPeekEntryByModel({ `prop_spycam` }, {
    {
      id = "cam_blur_action",
      event = "srp-gopro:blurCamera",
      icon = "fingerprint",
      label = "Smudge",
    },
...
AddEventHandler('srp-gopro:blurCamera', function(pArgs, pEntity, pContext)
  ...
  local function loopSkill(count)
    ...
    while loopCount < count do
        ...
        local finished = exports["srp-ui"]:taskBarSkill(math.random(1400, 2000), math.random(7, 12))
  ...
  exports["srp-objects"]:UpdateObject(object.id, { blurred = true })
end)

AddEventHandler('srp-objects:objectCreated', function(pObject, pEntity)
  if pObject.data.model == `prop_spycam` then
    SetEntityCollision(pEntity, false, false)
  end
end)
--[[
RegisterUICallback("srp-interface:gopros:addUserToCamera", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  RPC.execute("srp-gopros:addUserToCamera", data.camera, data.cid)
end)
