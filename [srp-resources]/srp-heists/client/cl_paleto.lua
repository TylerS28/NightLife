...
RegisterNetEvent('srp-paleto:box')
AddEventHandler('srp-paleto:box', function()
    ...
    if distance < 1.5 then
        if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
            ...
            PaletoStep1()
            exports["srp-dispatch"]:dispatchadd('10-90', "Paleto Bank Robbery", "108")
...
RegisterNetEvent('qpixel:createpaletomoney')
AddEventHandler('qpixel:createpaletomoney', function(toggle)
    if toggle == true then
...
function CreateMoney()
    ...
    TriggerEvent('qpixel:CleanClientVaultD')
    TriggerEvent("qpixel:createpaletomoney" ,true)
    Citizen.Trace("Crates Spawned :  Sucess")
    Citizen.Wait(240000)
    Citizen.Wait(60000)
    TriggerEvent("qpixel:createpaletomoney" ,false)
end


RegisterNetEvent('srp-paleto:startpaleto')
AddEventHandler('srp-paleto:startpaleto', function()
    local player = GetEntityCoords(PlayerPedId())
...
function OnHackSuccess()
    ...
    TriggerEvent("inventory:removeItem", "heistlaptop2", 1)
    TriggerServerEvent('srp-paleto:startCoolDown')
    loadDicts()
    ...
    TriggerServerEvent('srp-doors:change-lock-state', 45, false) 
    CreateMoney()
end

RegisterNetEvent('srp-paletogold')
AddEventHandler('srp-paletogold', function()
    print('called 2?')
    TriggerServerEvent('srp-paleto:tut_tut')
end)

AddEventHandler("srp-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('srp-paleto:box')
    elseif pItem == "heistlaptop2" then
        TriggerServerEvent("srp-paleto:hacklaptop")
    end
end)