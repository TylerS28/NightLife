...
AddEventHandler("srp-inventory:itemUsed", function(pItem)
    if pItem == "thermitecharge" then
        TriggerEvent('srp-vault:firstdoor')
        TriggerEvent('srp-vault:seconddoor')
        TriggerEvent('srp-vault:lowerfirstdoor')
        TriggerEvent('srp-vault:lowerseconddoor')
    elseif pItem == "heistlaptop4" then
        TriggerEvent('srp-vault:startlowervault')
        TriggerEvent('srp-vault:startvault')
    end
end)

RegisterNetEvent('srp-vault:firstdoor')
AddEventHandler('srp-vault:firstdoor', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                TriggerServerEvent('srp-heists:vaultRobberyLog')
                exports["srp-dispatch"]:dispatchadd('10-90', "Pacific Bank Robbery", "429")
                TriggerEvent("client:newStress",true, 150)
                frontdoorvault()
                TriggerEvent('srp-dispatch:bankrobbery')
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('srp-doors:change-lock-state', 46, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('srp-vault:seconddoor')
AddEventHandler('srp-vault:seconddoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('srp-doors:change-lock-state', 48, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent('srp-vault:startvault')
AddEventHandler('srp-vault:startvault', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(253.25254, 228.45343, 101.68377) - player)
    local pd = RPC.execute("srp-police:getActiveUnits")
    if pd >= 4 then
        if distance < 1.5 then
            if exports["srp-inventory"]:hasEnoughOfItem("heistlaptop4", 1) then
                exports["srp-dispatch"]:dispatchadd('10-90', "Pacific Bank Robbery", "429")
                StartVaultKid()
                TriggerEvent('srp-dispatch:bankrobbery')
                TriggerEvent("client:newStress",true, 150)
                ...
                function()
                    ...
                    TriggerEvent('srp-heists:vault_door')
                    DeleteObject(bag)
                    DeleteObject(laptop)
...
RegisterNetEvent('srp-heists:vault_door')
AddEventHandler('srp-heists:vault_door', function()
    TriggerEvent('vault:success:panel', closestBank, true)
    CreatenBrakeCrates()
end)

RegisterNetEvent('srp-vault:lowerfirstdoor')
AddEventHandler('srp-vault:lowerfirstdoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('srp-doors:change-lock-state', 49, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent('srp-vault:lowerseconddoor')
AddEventHandler('srp-vault:lowerseconddoor', function()
    local player = GetEntityCoords(PlayerPedId())
            ...
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                        TriggerServerEvent('srp-doors:change-lock-state', 50, false) 
                        Citizen.Wait(3000)
                        FreezeEntityPosition(PlayerPedId(), false)
                    else
                        TriggerEvent('inventory:removeItem', 'thermitecharge', 1)
...
RegisterNetEvent("srp-vault:grab", function()
...
exports["srp-polytarget"]:AddCircleZone("qpixel_vault_lower_interact_1",  vector3(289.29, 227.55, 98.24), 0.25, {
    useZ = true
})

exports["srp-polytarget"]:AddCircleZone("qpixel_vault_lower_interact_2",  vector3(286.92, 227.39, 98.29), 0.2, {
    useZ = true
})

exports["srp-polytarget"]:AddCircleZone("qpixel_vault_lower_interact_3", vector3(284.69, 221.64, 98.24), 0.2, {
    useZ = true
})

exports["srp-polytarget"]:AddCircleZone("qpixel_vault_lower_interact_4", vector3(286.52, 220.06, 98.29), 0.25, {
    useZ = true
})


exports["srp-interact"]:AddPeekEntryByPolyTarget("qpixel_vault_lower_interact_1", {{
    event = "srp-vaultrob:lower:firstpindoor",
    id = "qpixel_vault_lower_interact_1",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["srp-interact"]:AddPeekEntryByPolyTarget("qpixel_vault_lower_interact_2", {{
    event = "srp-vaultrob:lower:secpindoor",
    id = "qpixel_vault_lower_interact_2",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["srp-interact"]:AddPeekEntryByPolyTarget("qpixel_vault_lower_interact_3", {{
    event = "srp-vaultrob:lower:thirdpindoor",
    id = "qpixel_vault_lower_interact_3",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
exports["srp-interact"]:AddPeekEntryByPolyTarget("qpixel_vault_lower_interact_4", {{
    event = "srp-vaultrob:lower:fourthpindoor",
    id = "qpixel_vault_lower_interact_4",
    icon = "circle",
    label = "Code?",
    parameters = {},
}}, {
...
RegisterNetEvent('srp-loot_gold_tray')
AddEventHandler('srp-loot_gold_tray', function()
    TriggerServerEvent('srp-heists:loot_tray')
end)

RegisterNetEvent('srp-loot_cash_tray')
AddEventHandler('srp-loot_cash_tray', function()
    TriggerServerEvent('srp-heists:cash_tray')
end)

RegisterNetEvent("srp-heists:grab_from_tray")
AddEventHandler("srp-heists:grab_from_tray", function()
    local chance = math.random(120)
    Loot()
    if chance < 25 then
