...
RegisterNetEvent('srp-heists:start_loop')
AddEventHandler('srp-heists:start_loop', function()
    pickup = true
    TriggerEvent("srp-heists:pick_cash")
    Wait(180000)
    Wait(additionalWait)
    pickup = false
end)

RegisterNetEvent('srp-heists:pick_cash')
AddEventHandler('srp-heists:pick_cash', function()
        ...
        if aDist < 2.0 and not LootedTruck then
            ...
            if IsDisabledControlJustReleased(0, 38) then
                if not LootedTruck then
                    ...
                    TriggerEvent('srp-hud:show_money')
                    local BankTruckBarLoot = exports['srp-taskbar']:taskBar(60000, 'Looting the truck')
                    if BankTruckBarLoot == 100 then
                        FreezeEntityPosition(PlayerPedId(), false)
                        TriggerEvent('srp-heists:banktruck_loot') 
                        TriggerEvent('srp-hud:hide_money') 
                        LootedTruck = true
                        Citizen.Wait(900000) 
                        LootedTruck = false     
                    else
...
RegisterNetEvent('srp-heists:banktruck_loot')
AddEventHandler('srp-heists:banktruck_loot', function()
...
RegisterNetEvent('srp-heists:spawn_peds')
AddEventHandler('srp-heists:spawn_peds', function(veh)
...
RegisterNetEvent('srp-heists:banktruck:cl')
AddEventHandler('srp-heists:banktruck:cl', function()
