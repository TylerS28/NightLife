...
RegisterNetEvent('srp-vault:startlowervault')
AddEventHandler('srp-vault:startlowervault', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["srp-inventory"]:hasEnoughOfItem("heistlaptop4", 1) then
                ...
                -- if exports['srp-blackout']:Blackout() then
                    StartLowerVaultKid()
                    TriggerEvent('srp-dispatch:bankrobbery')
                    exports['srp-hacking']:hacking(
                        function()
                            ClearPedTasksImmediately(PlayerPedId())
                            TriggerEvent('srp-heists:lower_vault_done')
                            DeleteObject(bag)
                            DeleteObject(laptop)
...
RegisterNetEvent('srp-heists:lower_vault_done')
AddEventHandler('srp-heists:lower_vault_done', function()
    TriggerServerEvent('srp-doors:change-lock-state', 278, false) 
    TriggerServerEvent("srp-doors:change-lock-state", 279, false)
end)
...
function CreatenzylozCrates()
  ...
  AddEventHandler('srp-vault:startlowervault', function()
    ...
    if distance < 1.5 then
        exports['srp-vault']:OpenVaultGame(function(success)
    if success then
        TriggerServerEvent("srp-doors:change-lock-state", 325, false)
        print("280")  
    else
...
exports["srp-polyzone"]:AddBoxZone("vault_lower_entrance", vector3(266.01, 231.64, 97.68), 5.2, 13.4, {
...
RegisterNetEvent('srp-vault:closevaultlower')
AddEventHandler('srp-vault:closevaultlower', function()
...
RegisterNetEvent('srp-vault:blockedvaultlower')
AddEventHandler('srp-vault:blockedvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('srp-vault:openvaultlower')
AddEventHandler('srp-vault:openvaultlower', function()
    RequestIpl("np_int_placement_ch_interior_6_dlc_casino_vaultmilo")
...
RegisterNetEvent('srp-vault:openvaultdoor_ITSLOWER')
AddEventHandler('srp-vault:openvaultdoor_ITSLOWER', function()
    TriggerEvent('srp-vault:openvaultlower')
end)


RegisterNetEvent('srp-vaultrob:lower:vaultdoor')
AddEventHandler('srp-vaultrob:lower:vaultdoor', function(s, args)
    local ped = PlayerPedId()
    ...
    if #(PLAYER_COORDS - vaultdoorlowerlocation) < 20.0 and not vaultdoor then
        ...
        TriggerEvent('srp-vault:openvaultdoor_ITSLOWER')
        CreatenzylozCrates()
        Wait(1800000)
        TriggerEvent("srp-vault:LOWERVAULT_REFRESH")
    end
end)

RegisterNetEvent("srp-vault:LOWERVAULT_REFRESH")
AddEventHandler("vault:LOWERVAULT_REFRESH", function()
    ...
    if vaultdoor then
        vaultdoor = false
        TriggerEvent('srp-vault:blockedvaultlower')
        Citizen.Wait(900000)
        TriggerEvent('srp-vault:closevaultlower')
    end
...
RegisterNetEvent("srp-vaultrob:lower:fourpincontrol")
AddEventHandler("srp-vaultrob:lower:fourpincontrol", function()
...
RegisterNetEvent('srp-vaultrob:lower:firstpindoor')
AddEventHandler('srp-vaultrob:lower:firstpindoor', function()
    if not firstpindoor  then
        exports['srp-vault']:OpenVaultGame(function(success)
            if success then
                firstpindoor = true
                TriggerEvent("srp-vaultrob:lower:fourpincontrol")
            else
                firstpindoor = false
            end
        end, 5, 5)
...
RegisterNetEvent('srp-vaultrob:lower:secpindoor')
AddEventHandler('srp-vaultrob:lower:secpindoor', function()
...
RegisterNetEvent('srp-vaultrob:lower:thirdpindoor')
AddEventHandler('srp-vaultrob:lower:thirdpindoor', function()
...
RegisterNetEvent('srp-vaultrob:lower:fourthpindoor')
AddEventHandler('srp-vaultrob:lower:fourthpindoor', function()
