...
RegisterInterfaceCallback('srp-fleeca:email_1', function(data, cb)
...
RegisterInterfaceCallback('srp-fleeca:email_2', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('srp-fleeca:email_3', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('srp-fleeca:email_4', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('srp-fleeca:email_5', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterInterfaceCallback('srp-fleeca:email_6', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
...
RegisterNetEvent('srp-robbery:usb')
AddEventHandler('srp-robbery:usb', function()
    local ped = GetPlayerPed(-1)
    local pos = GetEntityCoords(ped)
    local Police = RPC.execute("srp-police:getActiveUnits")
            ...
            if closestBank ~= nil then
                ...
                if dist < 1.5 then				
                    if not Config.SmallBanks[closestBank]["isOpened"] then 
                        if Police >= 4 then
                            TriggerServerEvent('srp-heists:fleecaBankLog')
                            TriggerServerEvent("srp-heists:availabilty")
                            exports["srp-dispatch"]:dispatchadd('10-90', "Fleeca Bank Robbery", "108")
...
RegisterNetEvent('srp-robbery:client:setBankState')
AddEventHandler('srp-robbery:client:setBankState', function(bankId, state)
...
RegisterNetEvent('srp-robbery:client:enableAllBankSecurity')
AddEventHandler('srp-robbery:client:enableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('srp-robbery:client:disableAllBankSecurity')
AddEventHandler('srp-robbery:client:disableAllBankSecurity', function()
    for k, v in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('srp-robbery:client:BankSecurity')
AddEventHandler('srp-robbery:client:BankSecurity', function(key, status)
...
RegisterNetEvent('srp-robbery:client:setLockerState')
AddEventHandler('srp-robbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "pacific" then	
        print('[QUEER]')
    else
...
RegisterNetEvent('srp-robbery:client:ResetFleecaLockers')
AddEventHandler('srp-robbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent("fleeca:success:panel", function()
    ...
    exports["srp-dispatch"]:dispatchadd('10-90', "Fleeca Bank Robbery", "108")
    TriggerServerEvent('srp-robbery:server:setBankState', closestBank, true)

     if closestBank == 1 then
        print('[srp-heists] CREATE FLEECA PINK CAGE TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:pink:cage:bank')
     elseif closestBank == 2 then
        print('[srp-heists] CREATE FLEECA LEGION SQUARE TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:legion:sqaure:bank')
     elseif closestBank == 3 then
        print('[srp-heists] CREATE FLEECA HAWICK AVE TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:hawick:ave:bank')
     elseif closestBank == 4 then
        print('[srp-heists] CREATE FLEECA DEL PERRO TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:del:perrro:blvd')
     elseif closestBank == 5 then
        print('[srp-heists] CREATE FLEECA GREAT OCEAN TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:great:ocean:highway')
     elseif closestBank == 6 then
        print('[srp-heists] CREATE FLEECA ROUTE 68 TROLLEYS')
        TriggerEvent('qpixel:create:props:trolleys:route:68:bank')
     end
     DeleteObject(laptop)
 end)
...
AddEventHandler("fleeca:loot", function()
    local card = exports["srp-taskbar"]:taskBar(40000,"Taking cash...")
    if card == 100 then
...
RegisterNetEvent('qpixel:create:props:trolleys:legion:sqaure:bank')
AddEventHandler('qpixel:create:props:trolleys:legion:sqaure:bank', function()
...
RegisterNetEvent('qpixel:create:props:trolleys:del:perrro:blvd')
AddEventHandler('qpixel:create:props:trolleys:del:perrro:blvd', function()
...
RegisterNetEvent('qpixel:create:props:trolleys:great:ocean:highway')
AddEventHandler('qpixel:create:props:trolleys:great:ocean:highway', function()
...
RegisterNetEvent('qpixel:create:props:trolleys:pink:cage:bank')
AddEventHandler('qpixel:create:props:trolleys:pink:cage:bank', function()
...
RegisterNetEvent('qpixel:create:props:trolleys:hawick:ave:bank')
AddEventHandler('qpixel:create:props:trolleys:hawick:ave:bank', function()
...
RegisterNetEvent('qpixel:create:props:trolleys:route:68:bank')
AddEventHandler('qpixel:create:props:trolleys:route:68:bank', function()
...
RegisterNetEvent("srp-fleeca:grab")
AddEventHandler("srp-fleeca:grab", function()
    local chance = math.random(120)
    Loot()
    if chance < 25 then
		TriggerEvent("player:receiveItem",'heistusb1', 1)
        TriggerEvent('srp-mining:get_gem')
	end
