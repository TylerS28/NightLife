...
RegisterNetEvent('srp-bobcat:firstdoor')
AddEventHandler('srp-bobcat:firstdoor', function()
    local player = GetEntityCoords(PlayerPedId())
    local distance = #(vector3(882.208, -2258.1, 30.541439) - player)
    local pd = RPC.execute("srp-police:getActiveUnits")
    if pd >= 5 then
        if distance < 1.5 then
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                frontdoor()
                TriggerServerEvent('srp-heists:bobcatLog')
                TriggerEvent("inventory:removeItem", "thermitecharge", 1)
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        TriggerServerEvent('srp-doors:change-lock-state', 272, false) 
                        TriggerServerEvent("srp-doors:change-lock-state", 273, false)
                        exports["srp-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
RegisterNetEvent('srp-bobcat:seconddoor')
AddEventHandler('srp-bobcat:seconddoor', function()
local player = GetEntityCoords(PlayerPedId())
        ...
        if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                ...
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        --Door IDS
                        TriggerServerEvent("srp-doors:change-lock-state", 274, false)
                        exports["srp-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
                        Citizen.Wait(3000)
                    else
                        Citizen.Wait(2000)
...
RegisterNetEvent('srp-heists:bobcat_armed_door')
AddEventHandler('srp-heists:bobcat_armed_door', function()
    ...
    if pd >= 5 then
        if Dist < 1.5 then
            ...
            TriggerEvent('srp-heists:bobcat_explosion_ped')
            FreezeEntityPosition(PlayerPedId(), true)
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent("srp-doors:change-lock-state", 276, false)
            TriggerServerEvent("srp-doors:change-lock-state", 277, false)
        end
    else
        TriggerEvent('DoLongHudText', 'Not enough cops on duty', 2)
    end
...
RegisterNetEvent("bobcat:success:panel", function()
    TriggerEvent('DoLongHudText', "Success!", 1)
    TriggerServerEvent("srp-doors:change-lock-state", 275, false)
    TriggerServerEvent("srp-doors:change-lock-state", 276, false)
    exports["srp-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
end)
...
RegisterNetEvent('srp-bobcat:bubbles')
...
RegisterNetEvent('blow_door', function()
    TriggerServerEvent('srp-heists:bobcatLog')
    TriggerServerEvent('srp-bobcat:blow_door')
end)

RegisterNetEvent('srp-heists:bobcat_explosion_ped')
AddEventHandler('srp-heists:bobcat_explosion_ped', function()
...
RegisterNetEvent('srp-heists:explosion_ped_walk') -- PED WALK
AddEventHandler('srp-heists:explosion_ped_walk', function()
    exports["srp-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
...
AddEventHandler('lumo:client:requestblast', function()
	TriggerServerEvent("srp-bobcat:request_door")
end)
...
AddEventHandler('lumo:pedbomb', function()
    ...
    TriggerServerEvent("srp-bobcat:bubbles")
    -- TriggerEvent("lumo:propcreate")
    Wait(1500)
end)


RegisterNetEvent('srp-bobcat:search_crate_1unchecked') -- PED WALK
AddEventHandler('srp-bobcat:search_crate_1unchecked', function()
    TriggerServerEvent("srp-bobcat:search_crate_1")
end)

RegisterNetEvent('srp-bobcat:search_crate_2unchecked') -- PED WALK
AddEventHandler('srp-bobcat:search_crate_2unchecked', function()
    TriggerServerEvent("srp-bobcat:search_crate_2")
end)

RegisterNetEvent('srp-bobcat:search_crate_3unchecked') -- PED WALK
AddEventHandler('srp-bobcat:search_crate_3unchecked', function()
    TriggerServerEvent("srp-bobcat:search_crate_3")
end)

RegisterNetEvent('srp-bobcat:search_crate_1') 
AddEventHandler('srp-bobcat:search_crate_1', function()
    FreezeEntityPosition(GetPlayerPed(-1),true)
    TriggerEvent("doAnim","search")
    exports["srp-dispatch"]:dispatchadd('10-90', "Bobcat Security Robbery", "500")
    local finished = exports["srp-taskbar"]:taskBar(30000,"Looting")
    if finished == 100 then
        TriggerEvent('srp-heists:bobcat_loot')
...
RegisterNetEvent('srp-bobcat:search_crate_2') 
...
RegisterNetEvent('srp-bobcat:search_crate_3')
...
RegisterNetEvent('srp-heists:bobcat_loot')
AddEventHandler('srp-heists:bobcat_loot', function()
...
AddEventHandler("srp-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerEvent('srp-bobcat:firstdoor')
        TriggerEvent('srp-bobcat:seconddoor') 
    end
end)

AddEventHandler("srp-inventory:itemUsed", function(item)
    if item == "bobcatsecuritycard" then
        TriggerEvent('srp-heists:bobcat_armed_door')
    end
end)
...
exports["srp-polytarget"]:AddCircleZone("qpixel_bobcat_loot_1",  vector3(886.61, -2287.32, 29.92), 1.0, {
    useZ = true,
})

exports["srp-polytarget"]:AddCircleZone("qpixel_bobcat_loot_3",  vector3(881.4, -2282.62, 30.42), 1.0, {
    useZ = true,
})

exports["srp-polytarget"]:AddCircleZone("qpixel_bobcat_loot_2",  vector3(882.49, -2286.17, 30.37), 1.0, {
    useZ = true,
})

exports["srp-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_1', {{
    id = "qpixel_bobcat_loot_1",
    event = "srp-bobcat:search_crate_1unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["srp-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_2', {{
    id = "qpixel_bobcat_loot_2",
    event = "srp-bobcat:search_crate_2unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })

exports["srp-interact"]:AddPeekEntryByPolyTarget('qpixel_bobcat_loot_3', {{
    id = "qpixel_bobcat_loot_3",
    event = "srp-bobcat:search_crate_3unchecked",
    icon = "circle",
    label = "Loot!",
    paramers = {},
}}, { distance = { radius = 3.5 }  })
