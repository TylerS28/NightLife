RegisterNetEvent('srp-heists:bay_city:first_door')
AddEventHandler('srp-heists:bay_city:first_door', function()
    ...
    if pd >= 4 then
        if distance < 1.5 then
            if exports["srp-inventory"]:hasEnoughOfItem("thermitecharge", 1) then
                firstDoorBC()
                TriggerServerEvent('srp-heists:bayCityLog')
                exports["srp-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
                ...
                exports['srp-thermite']:OpenThermiteGame(function(success)
                    if success then
                        ...
                        TriggerServerEvent('srp-doors:change-lock-state', 563, false) 
                        Citizen.Wait(3000)
...
RegisterNetEvent('srp-heists:bay_city:second_door')
AddEventHandler('srp-heists:bay_city:second_door', function()
    local pd = RPC.execute("srp-police:getActiveUnits")
    if pd >= 4 then
        exports["srp-dispatch"]:dispatchadd('10-90', "Bay City Bank", "500")
        FreezeEntityPosition(PlayerPedId(), true)
        local finished = exports['srp-taskbar']:taskBar(15000, 'Opening Door.')
        if finished == 100 then
            TriggerEvent('aspect:createTrolleys')
            TriggerServerEvent('pStopSecondDoor')
            FreezeEntityPosition(PlayerPedId(), false)
            TriggerServerEvent('srp-doors:change-lock-state', 564, false) 
            -- Create Trolleys.
        end
    end
end)

RegisterNetEvent('srp-heists:bay_city:second_door:sv')
AddEventHandler('srp-heists:bay_city:second_door:sv', function()
    TriggerServerEvent('srp-heists:second_door:cl')
end)

exports["srp-polytarget"]:AddCircleZone("qpixel_baycity_unlock", vector3(-1309.13, -815.79, 17.7), 0.45, {
    useZ = true
})

exports["srp-interact"]:AddPeekEntryByPolyTarget("qpixel_baycity_unlock", {{
    event = "srp-heists:bay_city:second_door:sv",
    id = "qpixel_baycity_unlock",
    icon = "warehouse",
    label = "Open Door.",
    parameters = {},
}}, {
...
AddEventHandler("srp-inventory:itemUsed", function(item)
    if item == "thermitecharge" then
        TriggerServerEvent('srp-heists:first_door:cl')
    end
end)