...
RegisterServerEvent("srp-garages:resetpdgarage")
AddEventHandler("srp-garages:resetpdgarage",function ()
    ResetPDGarages()
end)



RegisterServerEvent("srp-garages:resetcartoimpound")
AddEventHandler("srp-garages:resetcartoimpound",function (plate)
...
RPC.register("srp-garages:select",function(pSource,garname)
	local src = source
	if garname.param == "Impound Lot" then
		exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE vehicle_state = @state', {['@state'] = "Out"}, function(vehicles)
			for k, v in ipairs(vehicles) do
				TriggerClientEvent("srp-garages:resetcartoimpound", src, v.license_plate)
			end
		end)
	end
...
RPC.register("srp-garages:selectShared",function(pSource,garname)
    local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
...
RPC.register("srp-garages:attempt:sv",function(pSource,pdata)
	local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
		...
		for ind, value in pairs(result) do
			TriggerClientEvent('srp-context:sendMenu', src, {
				...
				},
				{
					...
					params = {
						event = "srp-garages:attempt:spawn",
						args = result
					}
				},
				{
...
RPC.register("srp-garages:attempt:sv2",function(pSource,pdata)
	local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
		...
		for ind, value in pairs(result) do
			TriggerClientEvent('srp-context:sendMenu', src, {
				...
				},
				{
					...
					txt = 'Spawn Selected Vehicle',
					params = {
						event = "srp-garages:attempt:spawn2",
						args = result
					}
				},
				{
...
RPC.register("srp-garages:spawned:get",function(pSource,valid)
	local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE cid = @cid AND id = @id', { ['@cid'] = char.id, ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("srp-garages:attempt:spawn", src, result, true)
		else
...
RPC.register("srp-garages:spawned:getShared",function(pSource,valid)
	local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
	...
	exports.ghmattimysql:execute('SELECT * FROM characters_cars WHERE id = @id', { ['@id'] = valid.param }, function(result)
		if result[1].vehicle_state == "In" then
			TriggerClientEvent("srp-garages:attempt:spawn2", src, result, true)
		else
			TriggerClientEvent('DoLongHudText', src, 'Vehicle is not in the garage. Check in your phone maybe.', 2)
		end
    end)
...
RPC.register("srp-garages:getonlyimpound",function(pSource)
	local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local char = user:getCurrentCharacter()
