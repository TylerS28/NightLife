...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("ems_buy", vector3(301.07, -574.73, 43.26), 1.4, 1, {
        name="ems_buy",
        heading=18,
        --debugPoly=false,
        minZ=40.46,
        ...
        while nearBuy2 do
            ...
            if job == 'ems' then
                if IsControlJustReleased(0, 38) then
                    TriggerEvent('srp-garages:openBuyMenu2')
                end
            end
        end
    end)
...
RegisterNetEvent('srp-garages:openBuyMenu2')
AddEventHandler('srp-garages:openBuyMenu2', function()
    TriggerEvent('srp-context:sendMenu', {
		{
			id = 1,
			header = "EMS Ambulance",
			txt = "Purchase for $1000",
			params = {
				event = "srp-garages:PurchasedAmb"
			}
		},
        {
			id = 2,
			...
			params = {
				event = "srp-garages:PurchasedSpeedo"
			}
		},
        {
			id = 3,
			header = "EMS Explorer",
			txt = "Purchase for $20000",
			params = {
				event = "srp-garages:PurchasedExp"
			}
		},
...
RegisterNetEvent('srp-garages:PurchasedSpeedo')
AddEventHandler('srp-garages:PurchasedSpeedo', function()
    if exports["isPed"]:isPed("mycash") >= 15000 then
        --TriggerServerEvent('bankingremoveMoney')
        TriggerServerEvent('yorktax:speedo')
        TriggerEvent('srp-garages:PurchasedVeh2', 'Speedo', 'emsnspeedo', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('srp-garages:PurchasedAmb')
AddEventHandler('srp-garages:PurchasedAmb', function()
    if exports["isPed"]:isPed("mycash") >= 1000 then 
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bamb')
        TriggerEvent('srp-garages:PurchasedVeh2', 'Ambulance', 'emsa', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)

RegisterNetEvent('srp-garages:PurchasedBexp')
AddEventHandler('srp-garages:PurchasedBexp', function()
    if exports["isPed"]:isPed("mycash") >= 15000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:bexp')
        TriggerEvent('srp-garages:PurchasedVeh2', 'Explorer', 'npolexp2', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)   

RegisterNetEvent('srp-garages:PurchasedExp')
AddEventHandler('srp-garages:PurchasedExp', function()
    if exports["isPed"]:isPed("mycash") >= 35000 then
        --TriggerServerEvent('bankingremoveMoney', 5000)
        TriggerServerEvent('yorktax:emspro')
        TriggerEvent('srp-garages:PurchasedVeh2', 'Sierra', 'npolexp2', '0')
    else
        TriggerEvent('DoLongHudText', "You do not have enough money!", 2)
    end
end)  

RegisterNetEvent('srp-garages:PurchasedVeh2')
AddEventHandler('srp-garages:PurchasedVeh2', function(name, veh, price)
    ...
    TriggerServerEvent('srp-garages:FinalizedPur2', plate, name, vehicle, price)
    Citizen.Wait(100)
    exports['srp-interface']:hideInteraction()
end)
