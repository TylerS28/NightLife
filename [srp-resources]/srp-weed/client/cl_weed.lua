...
function addPlant(pPlant)
    exports["srp-polyzone"]:AddCircleZone("srp-weed:plant", pPlant.coords, 50, {
        --debugPoly = true,
        data = pPlant,
    })
...
function showPlantMenu(pPlantId)
    ...
    if growth < PlantConfig.HarvestPercent then
        context[#context+1] = {
            ...
            key = { id = pPlantId },
            action = 'srp-weed:addWater',
            disabled = not exports["srp-inventory"]:hasEnoughOfItem("water", 1, false),
        }

        context[#context+1] = {
            title  = 'Add Fertilizer',
            disabled = not exports["srp-inventory"]:hasEnoughOfItem("fertilizer", 1, false),
            description = "",
            children = {
                {
                    title = 'Add Fertilizer' .. ' (N)',
                    key = { id = pPlantId, type = "n" }, 
                    action = 'srp-weed:addFertilizer',
                },
                {
    ...
    if getStageFromPercent(growth) < 3 and plant.gender == 0 then
        context[#context+1] = {
            title = 'Add Male Seed',
            key = { id = pPlantId },
            action = 'srp-weed:addMaleSeed',
            description = 'Make the plant preggies',
            disabled = not exports["srp-inventory"]:hasEnoughOfItem("maleseed", 1, false)
        }
    end
    ...
    if myjob == "police" or myjob == "state" or myjob == "judge" then
        context[#context+1] = {
            ...
            description = "", 
            key = { id = pPlantId },
            action = 'srp-weed:removePlant',
        }
    end

    exports["srp-interface"]:showContextMenu(context);
end
...
AddEventHandler("srp-polyzone:enter", function(zone, data)
    if zone == "srp-weed:area" then
        inZone = true
    elseif zone == "srp-weed:plant" then
...
AddEventHandler("srp-polyzone:exit", function(zone, data)
    if zone == "srp-weed:area" then
        inZone = false
    elseif zone == "srp-weed:plant" then
        if activeZones[data.id] ~= nil then
            ...
            if hit then
                ...
                for matHash,matType in pairs(MaterialHashes) do
                    if materialHash == matHash then
                        ...
                        local finished = exports["srp-taskbar"]:taskBar(15000, 'Planting Seed', false, true, false, false, nil, 5.0, PlayerPedId())
                        ClearPedTasks(PlayerPedId())
                        if finished == 100 then
                            FreezeEntityPosition(PlayerPedId(), false)
                            ClearPedTasks(PlayerPedId())
                            RPC.execute("srp-weed:plantSeed", endCoords, typeMod)
...
RegisterNetEvent("srp-weed:trigger_zone", function (type, pData)
...
AddEventHandler("srp-weed:checkPlant", function(pContext, pEntity)
    local plantId = false
...
RegisterInterfaceCallback("srp-weed:addWater", function (pParams)
    if not exports["srp-inventory"]:hasEnoughOfItem("water", 1, true, true) then return end

    playPourAnimation()
    local finished = exports["srp-taskbar"]:taskBar(2000, "Adding Water", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local success = RPC.execute("srp-weed:addWater", pParams.key.id)
        if not success then
...
RegisterInterfaceCallback("srp-weed:addFertilizer", function (pParams)
    if not exports["srp-inventory"]:hasEnoughOfItem("fertilizer", 1, true, true) then return end

    playPourAnimation()
    local finished = exports["srp-taskbar"]:taskBar(2000, "Adding Fertilizer", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
            local success = RPC.execute("srp-weed:addFertilizer", pParams.key.id, pParams.key.type)
        if not success then
...
RegisterInterfaceCallback("srp-weed:addMaleSeed", function (pParams)
    if not exports["srp-inventory"]:hasEnoughOfItem("maleseed", 1, true, true) then return end

    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["srp-taskbar"]:taskBar(3000, "Adding Male Seed", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("srp-weed:addMaleSeed", pParams.key.id)
        TriggerEvent("inventory:removeItem", "maleseed", 1)
    end
    showPlantMenu(pParams.key.id)
end)

RegisterInterfaceCallback("srp-weed:removePlant", function (pParams)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["srp-taskbar"]:taskBar(3000, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        local getFertilizer = getPlantGrowthPercent(weedPlants[pParams.key.id]) > 20.0
        local success = RPC.execute("srp-weed:removePlant", pParams.key.id, getFertilizer)
        if not success then
...
AddEventHandler("srp-weed:pickPlant", function(pContext, pEntity)
    local plantId = false
    ...
    local plyWeight = exports["srp-inventory"]:getCurrentWeight()
    ...
    TriggerEvent("animation:PlayAnimation", "layspike")
    local finished = exports["srp-taskbar"]:taskBar(10000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
        RPC.execute("srp-weed:harvestPlant", plantId)
    end
end)
...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddPolyZone("srp-weed:area", {
    ...
    local _plants = RPC.execute("srp-weed:getPlants")
