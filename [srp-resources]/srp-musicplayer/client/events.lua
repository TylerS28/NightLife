RegisterNUICallback("data_status", function(data)
    if soundInfo[data.id] ~= nil then
        if data.type == "finished" then
            ...
            TriggerEvent("srp-musicplayer:songStopPlaying", data.id)
        end
...
RegisterNetEvent("srp-musicplayer:stateSound")
AddEventHandler("srp-musicplayer:stateSound", function(state, data)
