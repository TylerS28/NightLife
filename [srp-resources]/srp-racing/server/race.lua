...
function joinRace(src, id, alias, cid, dontpay)
    ...
    TriggerClientEvent("srp-racing:joinedRace", src, Races[id])
    TriggerClientEvent("srp-racing:addedPendingRace", -1, PendingRaces[id])

    for k, v in pairs(PendingRaces[id]["players"]) do
        TriggerClientEvent("srp-racing:playerJoinedYourRace", v.serverId, cid, alias)
    end
end
...
RegisterNetEvent("srp-racing:updatePositionInfo")
AddEventHandler("srp-racing:updatePositionInfo", function(curRaceid, totalDist)
    local src = source
    local srcString = tostring(src)
    ...
    TriggerClientEvent("srp-racing:updatePosition", src, position)
end)
...
RPC.register("srp-racing:createPendingRace", function(src, ids, optionss)
    ...
    TriggerClientEvent("srp-racing:addedPendingRace", -1, options)
    TriggerClientEvent('srp-racing:pendingRacenotif', -1)
    print("GETTING TO JOIN AFTER CREATING RACE",options)
    joinRace(src, id, alias, cid, true)
end)

RPC.register("srp-racing:joinRace", function(src, ids, aliass, cids)
    local id = ids
    local alias = aliass
    local cid = cids
...
RPC.register("srp-racing:leaveRace", function(src)
    local srcString = tostring(src)
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    ...
    if cid == PendingRaces[raceId]["owner"] then
        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("srp-racing:endRace", v.serverId)
        end

        PendingRaces[raceId] = nil
        TriggerClientEvent("srp-racing:removedPendingRace", -1, raceId)
    else
        PendingRaces[raceId]["players"][srcString] = nil
        TriggerClientEvent("srp-racing:leftRace", src)
        TriggerClientEvent("srp-racing:addedPendingRace", -1, PendingRaces[raceId])

        for k, v in pairs(PendingRaces[raceId]["players"]) do
            TriggerClientEvent("srp-racing:playerLeftYourRace", v.serverId, cid, alias)
        end
    end
end)

RPC.register("srp-racing:dnfRace", function(src, ids)
    local id = ids
    local srcString = tostring(src)
    ...
    TriggerClientEvent("srp-racing:leftRace", src)
    TriggerClientEvent("srp-racing:updatedActiveRace", -1, ActiveRaces[id])
    ...
    if ActiveRaces[id]["positions"] > countPlayers then
        ...
        TriggerClientEvent("srp-racing:raceHistory", -1, ActiveRaces[id])
        ...
        ActiveRaces[id] = nil
        TriggerClientEvent("srp-racing:removedActiveRace", -1, id)
    end
end)

RPC.register("srp-racing:startRace", function(src, ids, countdowns, ownerID)
    ...
    TriggerClientEvent("srp-racing:addedActiveRace", -1, ActiveRaces[id])

    PendingRaces[id] = nil
    TriggerClientEvent("srp-racing:removedPendingRace", -1, id)
    ...
    for k, v in pairs(ActiveRaces[id]["players"]) do
        ...
        if isStartEnabled then
            TriggerClientEvent("srp-racing:startRace", v.serverId, race, countdown * 1000)
            TriggerClientEvent("srp-racing:api:currentRace", v.serverId, race)
        else
            local user = exports['srp-fw']:getModule("Player"):GetUser(v.serverId)
            local char = user:getCurrentCharacter()
...
RPC.register("srp-racing:endRace", function(src)
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    ...
    TriggerClientEvent("srp-racing:removedActiveRace", -1, raceId)
end)

RPC.register("srp-racing:finishedRace", function(src, curRaceids, bestLapTimes, totalTimes)
    ...
    TriggerClientEvent("srp-racing:finishedRace", src, ActiveRaces[curRaceid]["players"][srcString]["position"], totalTime, ActiveRaces[curRaceid]["buyIn"])
    TriggerClientEvent("srp-racing:updatedActiveRace", -1, ActiveRaces[curRaceid])
    ...
    if ActiveRaces[curRaceid]["players"][srcString]["position"] == countPlayers then
        ...
        TriggerClientEvent("srp-racing:raceHistory", -1, ActiveRaces[curRaceid])
        ...
        TriggerClientEvent("srp-racing:removedActiveRace", -1, curRaceid)
    elseif ActiveRaces[curRaceid]["dnfPosition"] > 0 and ActiveRaces[curRaceid]["dnfPosition"] == ActiveRaces[curRaceid]["players"][srcString]["position"] then
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            TriggerClientEvent("srp-racing:startDNFCountdown", v.serverId, countdown)
        end
        ...
        for k, v in pairs(ActiveRaces[curRaceid]["players"]) do
            if v.position == 0 then
                TriggerClientEvent("srp-racing:dnfRace", v.serverId)
            end
        end
