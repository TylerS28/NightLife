...
function createPendingRace(id, options)
    ...
    RPC.execute("srp-racing:createPendingRace", id, options)
end
...
function startRace(countdown)
    ...
    for k, v in pairs(pendingRaces) do
        ...
        if v.owner == characterId then
            ...
            RPC.execute("srp-racing:startRace", v.id, countdown or v.countdown, v.owner)
            return
        end
    end
end
...
function endRace()
    ...
    if curRace then
        RPC.execute("srp-racing:endRace")
        print("ENDING RACE 1")
    else
        RPC.execute("srp-racing:leaveRace")
    end
end

function joinRace(id, alias, characterId)
    RPC.execute("srp-racing:joinRace", id, alias, characterId)
end
...
function leaveRace()
    ...
    if curRace then
        RPC.execute("srp-racing:dnfRace", curRace.id)
        cleanupRace()
    else
        RPC.execute("srp-racing:leaveRace")
    end
...
function getAllRaces()
    ...
    local res = RPC.execute("srp-racing:getAllRaces")
    races = res.races
...
function getHasRaceUsbAndAlias()
    ...
    local pdRacingUsbItem = exports["srp-inventory"]:GetInfoForFirstItemOfName("racingusb3")
