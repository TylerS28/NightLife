...
RegisterInterfaceCallback("srp-signin:handler", function(data, cb)
  ...
  if eventData.sign_in then
    if eventData.job == "police" then
      ...
      TriggerServerEvent("srp-signin:duty", eventData.job)
    end
...
RegisterInterfaceCallback("srp-signin:spawnVehicle", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  local eventData = data.key
...
AddEventHandler("srp-signin:peekAction", function(pArgs, pEntity, pContext)
  ...
  print(MenuData[name])
  exports["srp-interface"]:showContextMenu(MenuData[name])
end)

AddEventHandler('srp-island:hideBlips', function(pState)
