...
RegisterServerEvent("srp-signin:duty")
AddEventHandler("srp-signin:duty", function(pJobType, pSrc)
    ...
    jobs:SetJob(user, pJobType, false, function()
        if pJobType == "police" then
            exports['srp-dispatch']:callsign_command()
...
RegisterServerEvent("srp-signoff:duty")
AddEventHandler("srp-signoff:duty", function(job)
	local src = source
    if job == 'police' then
