...
RegisterNetEvent("srp-scenes:getScenes")
AddEventHandler("srp-scenes:getScenes", function()
    local src = source
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))

    if not data then
        TriggerClientEvent("srp-scenes:allScenes", src, {})
        return
    end
    ...
    TriggerClientEvent("srp-scenes:allScenes", src, scenes)
end)

RegisterNetEvent("srp-scenes:addScene")
AddEventHandler("srp-scenes:addScene", function(pData)
    local src = source
    ...
    TriggerClientEvent("srp-scenes:refreshScenes", -1, data[1].id, data[1])
end)

RegisterNetEvent("srp-scenes:deleteScene")
AddEventHandler("srp-scenes:deleteScene", function(pCoords)
    local data = Await(SQL.execute("SELECT * FROM __scenes", {}))
    for k,v in pairs(data) do
        ...
        if #(acCoord - pCoords) < 2.0 then
            ...
            if delete then
                TriggerClientEvent("srp-scenes:deleteScene", -1, v.id)
                return
            end
        end
    end
