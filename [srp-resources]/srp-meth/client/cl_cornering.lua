...
AddEventHandler("srp-meth:cornerSellProduct", function(pArgs, pEntity)
    local gameTimer = GetGameTimer()

    if lastCornerSale and (gameTimer - lastCornerSale) <= 4000 then
        return TriggerEvent('srp-meth:cleanAttempt', lastCornerSale - gameTimer)
    end

    lastCornerSale = gameTimer

    if not exports["srp-inventory"]:hasEnoughOfItem("methlabproduct", 1, false, true) then
    ...
    local success = RPC.execute("srp-meth:attemptCornerSale", NetworkGetNetworkIdFromEntity(pEntity), inCornerId)
    if success then
        TriggerEvent("money:clean")
        TriggerServerEvent("srp-heists:generateSafeCrackingTool")
    end
...
AddEventHandler("srp-meth:cornerStartSelling", function()
    local result = RPC.execute("srp-meth:startCornering", inCornerId)
    if not result then return end
    TriggerEvent("srp-meth:showSellDrugsMenuItem", "cancorner", false)
    playerCornering = true
end)

AddEventHandler("srp-polyzone:enter", function(name, data)
    if name ~= "meth_corner" then return end
    if not exports["srp-inventory"]:hasEnoughOfItem("methlabproduct", 1, false) then return end
    inCorner = true
    inCornerId = 1
    -- inCornerId = data.id
    TriggerEvent("DoLongHudText", "Looks like a good spot to sell...")
    TriggerEvent("srp-meth:showSellDrugsMenuItem", "cancorner", true)
end)
AddEventHandler("srp-polyzone:exit", function(name, data)
    if name ~= "meth_corner" then return end
    if not inCorner then return end
    if playerCornering then
        RPC.execute("srp-meth:stopCornering")
...
RegisterNetEvent("srp-meth:findCandidateVehicle")
AddEventHandler("srp-meth:findCandidateVehicle", function(pCornerId, pSrc)
  PopulateNow()
  Wait(0)
  ...
  TriggerServerEvent("srp-meth:corneringFoundVehicle", pCornerId, pSrc, { vehicle = cornerVehicle, driver = cornerPed })
end)

RegisterNetEvent('srp-meth:findCandidate', function()
  PopulateNow()
  Wait(0)
  local vehicles = GetGamePool('CVehicle')
