...
AddEventHandler("srp-inventory:itemUsed", function(item, info)
    ...
    if not exports["srp-inventory"]:hasEnoughOfItem("qualityscales", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to weigh with...")
        return
    end
    if not exports["srp-inventory"]:hasEnoughOfItem("emptybaggies", 1, false) then
        TriggerEvent("DoLongHudText", "Need something to pack with...")
        return
    end
    local finished = exports["srp-taskbar"]:taskBar(1000, "Packing")
    if finished ~= 100 then return end
    RPC.execute("srp-meth:packCuredMeth", json.decode(info))
...
Citizen.CreateThread(function()
    ...
    local rv = exports["srp-npcs"]:RegisterNPC(data, 'srp-meth')
end)
