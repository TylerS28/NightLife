...
local function processAction(action, pEntity, fnCb)
    ...
    if animSettings then
        ...
        result:next(function (data)
            if data == 100 then
                RPC.execute("srp-meth:doAction", insideLab, action)
                if fnCb then
                  fnCb()
                end
            else
...
--[[ RegisterUICallback("srp-ui:submitRangeValues", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ClearPedTasksImmediately(PlayerPedId())
    Wait(100)
    exports["srp-ui"]:closeApplication("range-picker")
    
    local result = RPC.execute("srp-meth:doAction", insideLab, "START_COOKING", data)
...
AddEventHandler("srp-meth:startCooking", function()
    if not skipKeyChecks[insideLab] and not exports["srp-inventory"]:hasEnoughOfItem("methlabkey", 1, false, true) then
    ...
    print("3")
    exports["srp-ui"]:openApplication("range-picker")
end)

AddEventHandler("srp-meth:pickupIngredient", function(pArgs, pEntity)
...
AddEventHandler("srp-meth:adjustFridgeTemp", function(pArgs, pEntity)
    processAction("FRIDGE_TEMPERATURE", pEntity)
end)

AddEventHandler("srp-meth:adjustSteamLevel", function(pArgs, pEntity)
    processAction("DISTIL_STEAM", pEntity)
end)

AddEventHandler("srp-meth:adjustDistilSettings", function(pArgs, pEntity)
    processAction("DISTIL_SETTINGS", pEntity)
end)

AddEventHandler("srp-meth:adjustMixerTemp", function(pArgs, pEntity)
    processAction("MIXER_TEMPERATURE", pEntity)
end)

AddEventHandler("srp-meth:addIngredient", function(pArgs, pEntity)
...
AddEventHandler("srp-meth:adjustMixerSettings", function(pArgs, pEntity)
...
--     RPC.execute("srp-meth:useDoorKey", NetworkGetNetworkIdFromEntity(PlayerPedId()), info)
-- end)

AddEventHandler("srp-meth:purchaseMethLabKey", function()
    local wallet, message = RPC.execute("phone:adjustCryptoBalance", 1, getConfig())
    if not wallet then
        TriggerEvent("DoLongHudText", message, 2)
        return
    end
    local result = RPC.execute("srp-meth:purchaseLabKey")
end)
...
--     if foundObject then
--         foundObject = nil
--         TriggerEvent("srp-menu:updateMethObject", foundObject)
--     end
--     return
--   end
...
RegisterNetEvent("srp-meth:keyPurchasedEmail")
AddEventHandler("srp-meth:keyPurchasedEmail", function()
  TriggerEvent(
    "phone:emailReceived",
    "Dark Market",
    "Lab Information",
