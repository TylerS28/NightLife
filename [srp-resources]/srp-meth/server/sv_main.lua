...
local function get_info_from_db ()
  local src = source
  local user = exports['srp-fw']GetModule('Player')(src)
  local cid = user.id
...
RPC.register('srp-meth:getConfig',function(pData, pInfo)
  print(pData, pInfo)
  local src = source
  table = {
    ACTIVE_CORNERS = {
...
RegisterServerEvent('srp-meth:corneringFoundVehicle',function (pCorner , pSrc , pTable)
  print('Hmmm',json.encode(pTable))
  
end)

RPC.register('srp-meth:startCornering', function(pID)
  local src = source
  TriggerClientEvent('srp-meth:findCandidateVehicle', 1 , src)
  return true
end)

RPC.register('srp-meth:corneringFoundVehicle',function(pData, pInfo , pTable)
  local src = source
  return false , 'No vehicle found bro'
end)

RPC.register('srp-meth:attemptCornerSale', function (pCar , pInfo)
...
RPC.register('srp-meth:stopCornering', function()
  local src = source 
  TriggerClientEvent('srp-meth:showSellDrugsMenuItem',src,"cancorner" ,false)
end)

RPC.register('srp-meth:purchaseLabKey', function()
  local src = source
