...
function Login.CreatePlayerCharacterPeds(characterModelData,isReset)
    ...
    TriggerEvent("srp-weathersync:client:DisableSyncSpawn")

    local events = exports["srp-fw"]:getModule("Events")
    events:Trigger("srp-fw:fetchPlayerCharacters", nil, function(data)
        if data.err then
            return
        end
...
--[[
    ...
    Description: dealing with srp-fw functionality in order to set/get the correct information for chars
]]
...
function Login.getCharacters(isReset)
    ...
    if not isReset then
        ...
        events:Trigger("srp-fw:loginPlayer", nil, function(data)
...
function Login.SelectedChar(data)
	...
	local events = exports["srp-fw"]:getModule("Events")
	events:Trigger("srp-fw:selectCharacter", data.actionData, function(returnData)
        ...
        if Login.CurrentClothing[data.actionData] == nil then
	       ...
	       -- SetPlayerInvincible(PlayerPedId(), true)
	        TriggerEvent("srp-fw:firstSpawn")
	    end
    end)
end
...
function Login.setClothingForChar()
    ...
    TriggerServerEvent('srp-spawn:SetNewRouting', math.random(1, 999))
    --TriggerEvent('srp_clothing:setclothes', {}, nil, true)
...
RegisterNetEvent('srp-spawn:justCreated')
AddEventHandler('srp-spawn:justCreated', function()
    TriggerServerEvent("srp-spawn:SetOldRouting")
    spawnChar()
    TriggerServerEvent("srp-spawn:licenses")
    Citizen.Wait(5000)
    TriggerEvent('srp-spawn:spawnCharacter')
end)
...
function spawnChar()
    Login.actionsBlocked = false
    deleteTrain()
    --TriggerEvent("srp-weathersync:spawned")
    TriggerEvent("srp-weathersync:client:EnableSync")
    -- SetPlayerInvincible(PlayerPedId(), true)
    TriggerEvent("srp-fw:firstSpawn")
...
exports["srp-polyzone"]:AddBoxZone("scuff_shit", vector3(-3.17, 1.97, 1.47), 15, 15, {
    heading=0,
    debugPoly=false,
    minZ=0.07,
    maxZ=4.07
...
AddEventHandler('srp-polyzone:enter', function(name)
    if name == "scuff_shit" then
        pInZone = true
        exports["srp-interface"]:showInteraction("[E] Information")
        TPFUNCTION()
    end
end)
