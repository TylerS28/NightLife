...
AddEventHandler('spawn:clientSpawnData', function(spawnData, heheData)
		...
		if spawnData.overwrites ~= nil then
			...
			elseif spawnData.overwrites == "new" then
				...
				TriggerEvent("srp-spawn:firstspawnyo")
			end
			return
		end
		...
		if Spawn.housingCoords == nil then
			Spawn.housingCoords = exports["srp-housing"]:retriveHousingTable()
		end
...
RegisterNetEvent("srp-spawn:firstspawnyo")
AddEventHandler("srp-spawn:firstspawnyo", function()
	if triggered then
		triggered = false
		if not exports["srp-inventory"]:hasEnoughOfItem("newaccountbox",1,false) then
			TriggerEvent("player:receiveItem","newaccountbox",1,true)
		end
	end
end)

RegisterNetEvent('srp-spawn:finishedClothingstuff')
AddEventHandler('srp-spawn:finishedClothingstuff', function()
	
end)

RegisterNetEvent('srp-spawn:finishedClothingstuffbish')
AddEventHandler('srp-spawn:finishedClothingstuffbish', function()
	...
	if not exports["srp-inventory"]:hasEnoughOfItem("lockpick",1,false)then
		TriggerEvent("player:receiveItem","lockpick",3)
	end
	if not exports["srp-inventory"]:hasEnoughOfItem("sandwich",3,false)then
		TriggerEvent("player:receiveItem","sandwich",1)
	end
	if not exports["srp-inventory"]:hasEnoughOfItem("water",1,false)then
		TriggerEvent("player:receiveItem","water",3)
	end
	if not exports["srp-inventory"]:hasEnoughOfItem("repairkit",1,false)then
...
function Spawn.getBusinessSpawn(pBusinessCode, pBusinessName)
	...
	local hasPermission = RPC.execute("srp-business:hasPermission", pBusinessCode, "key_access")
