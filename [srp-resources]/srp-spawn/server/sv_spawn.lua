...
RegisterServerEvent("srp-login:disconnectPlayer")
AddEventHandler("srp-login:disconnectPlayer", function()
    local src = source
    DropPlayer(src, "You DisConnected")
end)

RegisterServerEvent("srp-fw:playerSessionStarted")
...
RegisterServerEvent("srp-spawn:initBoosting")
AddEventHandler("srp-spawn:initBoosting", function()
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RegisterServerEvent('srp-spawn:SetNewRouting')
AddEventHandler('srp-spawn:SetNewRouting', function(pRoutingNum)
    local src = source
    SetPlayerRoutingBucket(src, pRoutingNum)
    print('Spawn routing bucket: '..pRoutingNum)
end)

RegisterServerEvent('srp-spawn:GetRouting')
AddEventHandler('srp-spawn:GetRouting', function()
    local src = source
    GetPlayerRoutingBucket(src)
end)

RegisterServerEvent('srp-spawn:SetOldRouting')
AddEventHandler('srp-spawn:SetOldRouting', function()
    local src = source
...
RegisterServerEvent('srp-spawn:createCharacter:log')
AddEventHandler('srp-spawn:createCharacter:log', function(pFirstName, pSecondName, pDOB, pGender)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    ...
    local connect = {
        {
          ["color"] = color,
          ["title"] = "** srp [srp-spawn] | Create Character Log **",
          ["description"] = "First Name: "..pFirstName.."\n Second Name: "..pSecondName.."\n DOB: "..pDOB.."\n Gender: "..pGender.." \n Steam ID: "..sid,
        }
      }
      PerformHttpRequest("https://discord.com/api/webhooks/1012082909533982720/spQhZlo5XmZS--PtwxMUPr_ix1jWvmLktUMkTPEJl-IjGorxS2IvlxHQ1dLI1BrFzKD9", function(err, text, headers) end, 'POST', json.encode({username = "srp | Job Payout Log", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)