...
RegisterServerEvent('srp-boosting:removeGNE')
AddEventHandler('srp-boosting:removeGNE', function(amount, cid)
    exports.oxmysql:execute('SELECT gne FROM boosting_users WHERE identifier = ?', {cid}, function(result)
...
RegisterServerEvent('srp-boosting:pay_gne')
AddEventHandler('srp-boosting:pay_gne', function()
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
    local gnes = GetGNE(cid)
...
RegisterServerEvent('srp-boosting:giveGNE')
AddEventHandler('srp-boosting:giveGNE', function(amount)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
	local cid = user:getCurrentCharacter().id
