...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('srp-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("srp-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vin = exports['srp-vehicles']:GetVehicleIdentifier(pEntity)
  local vehicleOwner = RPC.execute("srp-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("srp-fishing:currentlyRentedBoat", function(handle)
