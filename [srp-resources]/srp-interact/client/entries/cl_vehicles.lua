    ...
    data = {
        {
            ...
            icon = "cut",
            event = "srp-miscscripts:carBombs:removeBomb",
            parameters = {}
        },
    },
    options = {
        distance = { radius = 1.8 },
        isEnabled = function(pEntity, pContext)
            return exports['srp-carbombs']:isThisVehicleABomb(NetworkGetNetworkIdFromEntity(pEntity))
        end
    }
}
...
Entries[#Entries + 1] = {
    ...
    options = {
        distance = { radius = 1.8 },
        isEnabled = function(pEntity, pContext)
            return exports['srp-activities']:canHandOffPackages() and exports['srp-inventory']:hasEnoughOfItem('darkmarketpackage', 1)
        end
    }
}
    ...
    options = {
        ...
        isEnabled = function(pEntity, pContext)
            return (not (isCloseToHood(pEntity, PlayerPedId()) or isCloseToBoot(pEntity, PlayerPedId(), 2.0, pContext.model))
              or pContext.model == GetHashKey("emsnspeedo"))
              and not IsPedInAnyVehicle(PlayerPedId(), false)
              and NetworkGetEntityIsNetworked(pEntity) and exports['srp-boosting']:pCanFinishScratch()
        end
    }
}
