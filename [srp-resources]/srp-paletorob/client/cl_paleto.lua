...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('srp-paleto:getGetDoorStateSV')
...
RegisterNetEvent("srp-paleto:getDoorCheckCL")
AddEventHandler("srp-paleto:getDoorCheckCL", function(result)
...
RegisterNetEvent("srp-paleto:resetDoorState")
AddEventHandler("srp-paleto:resetDoorState", function(name)
    Freeze[name] = 0
end)

RegisterNetEvent("srp-paleto:lootup_c")
AddEventHandler("srp-paleto:lootup_c", function(var, var2)
    LootCheck[var][var2] = true
end)

RegisterNetEvent("srp-paleto:outcome")
AddEventHandler("srp-paleto:outcome", function(oc, arg)
    for i = 1, #Check, 1 do
        Check[i] = false
    end
    ...
    if oc then
        Check[arg] = true
        TriggerEvent("srp-paleto:startheist", Paleto.Banks[arg], arg)
    elseif not oc then
        TriggerEvent("DoLongHudText", arg, 2)
    end
end)

RegisterNetEvent("srp-paleto:startLoot_c")
AddEventHandler("srp-paleto:startLoot_c", function(data, name)
    currentname = name
            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if LootCheck[name].Stop or (LootCheck[name].Loot) then
                        ...
                        if initiator then
                            TriggerEvent("srp-paleto:reset", name, data)
                            return
                        end
                        return
                    end
...
RegisterNetEvent("srp-paleto:stopHeist_c")
AddEventHandler("srp-paleto:stopHeist_c", function(name)
    LootCheck[name].Stop = true
end)

-- MAIN DOOR UPDATE --

AddEventHandler("srp-paleto:freezeDoors", function()
    Citizen.CreateThread(function()
...
RegisterNetEvent("srp-paleto:ClosePaletoDoor")
AddEventHandler("srp-paleto:ClosePaletoDoor", function()
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418,6472.1274414062,31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
        ...
        Citizen.Wait(10000)
    until count == 900
    TriggerServerEvent('srp-paleto:getGetDoorStateSVSV', false)
end)


-- RegisterNetEvent("srp-paleto:toggleVault")
...
--             TriggerServerEvent("srp-paleto:updateVaultState", key, Doors[key][1].state)
--         elseif state then
--             local obj = GetClosestObjectOfType(Paleto.Banks[key].doors.startloc.x, Paleto.Banks[key].doors.startloc.y, Paleto.Banks[key].doors.startloc.z, 2.0, GetHashKey(Paleto.vaultdoor), false, false, false)
--             local count = 0
...
RegisterNetEvent("srp-paleto:OpenPaletoDoor")
AddEventHandler("srp-paleto:OpenPaletoDoor", function()
    TriggerServerEvent('srp-paleto:getGetDoorStateSVSV', true)
    local count = 0
    local VaultDoor = GetClosestObjectOfType(-104.8747253418, 6472.1274414062, 31.621948242188, 25.0, `v_ilev_cbankvauldoor01`, 0, 0, 0)
    repeat
...
AddEventHandler("srp-paleto:reset", function(name, data)
    ...
    TriggerServerEvent("srp-paleto:closeDoor")
    -- TriggerServerEvent("srp-paleto:toggleVault", name, true)
    TriggerEvent("srp-paleto:cleanUp", data, name)
end)


AddEventHandler("srp-paleto:startheist", function(data, name)
    disableinput = true
    currentname = name
  --  currentcoords = vector3(data.doors.startloc.x, data.doors.startloc.y, data.doors.startloc.z)
    initiator = true
    ...
    TriggerServerEvent('srp-paleto:openDoor')
    TriggerServerEvent("srp-paleto:setCooldown", name)
    --TriggerServerEvent("srp-paleto:toggleVault", name, false)
    startdstcheck = true
    currentname = name
    SpawnTrolleys(data, name)
end)

-- AddEventHandler("srp-paleto:cleanUp", function(data, name)
--     Citizen.Wait(10000)
...
function SpawnTrolleys(data, name)
    ...
    TriggerServerEvent("srp-paleto:startLoot", data, name, missionplayers)
    done = false
end
        ...
        if startdstcheck then
            if initiator then
                ...
                if (GetDistanceBetweenCoords(playercoord, currentcoords, true)) > 20 then
                    ...
                    startdstcheck = false
                    TriggerServerEvent("srp-paleto:stopHeist", currentname)
                end
            end
            Citizen.Wait(1)
        else
...
RegisterNetEvent('srp-paleto:darkmail')
AddEventHandler('srp-paleto:darkmail', function()
    Citizen.Wait(1000)
...
RegisterNetEvent('srp-paleto:UseBlueLapTop')
AddEventHandler('srp-paleto:UseBlueLapTop', function(item)
    local ply = PlayerPedId()
    local plyCoords = GetEntityCoords(ply)
    local p = promise:new()

    Paleto.Banks, doors = RPC.execute("srp-paleto:getBanks")
	TriggerEvent("srp-paleto:freezeDoors")
    ...
    for k, v in pairs(Paleto.Banks) do
        if not v.onaction then
            ...
            if dist < 1.0 then
                ...
                if exports["srp-inventory"]:hasEnoughOfItem("heistlaptop2", 1) then
                    -- if HasItem then
                    ...
                    exports['hacking']:OpenHackingGame(function(Success)
                        if Success then
                            ...
                            Citizen.SetTimeout(timeout, function()
                                TriggerServerEvent("srp-paleto:startcheck", k)
                            end)
...
AddEventHandler('paletogate:doorburn', function()
	TriggerServerEvent('srp-doors:change-lock-state', 498, false)
end)

RegisterNetEvent('paletogate2:doorburn') -- Doorlock
AddEventHandler('paletogate2:doorburn', function()
	TriggerServerEvent('srp-doors:change-lock-state', 517, false)
end)
