;(function () {
  ...
  !(function () {
    ...
    setImmediate(async () => {
      ...
      Number(objectTable.length) === 0 && emitNet('srp-objects:requestObjects')
    })
    onNet('srp-objects:loadObjects', async (pObjectData) => {
      console.log('[srp-objects] Load Objects')
    ...
    on('srp-polyzone:enter', async (name, data) => {
      if (name !== 'object_zone') {
        return
      }
      ...
      console.log('[srp-objects] Entered zone, load objects in distance.')
    ...
    on('srp-polyzone:exit', (name, data) => {
      if (name !== 'object_zone') {
        return
      }
      ...
      console.log('[srp-objects] Left zone, unload objects in distance.')
    ...
    onNet('srp-objects:clearObjects', (objectData) => {
    ...
    onNet('srp-objects:updateObjects', (objectData) => {
      Object.entries(objectData).forEach(([k, v]) => {
        let findData = newObjTable.findIndex(
          (fountData) => fountData.id.toString() === v.toString()
        )
    ...
    async function prepareObject(objectData) {
      console.log('[srp-objects] Adding object: ', objectData)
      let coords = {
    ...
    function DelObject(obj) {
      let deleted = RPC.execute('srp-objects:DeleteObject', obj)
      return deleted
    }
    exports('DelObject', DelObject)
    function UpdateObject(object, updatedObject) {
      RPC.execute('srp-objects:UpdateObject', object, updatedObject)
    }
    ...
    RegisterCommand(
      ...
      (src, args, user) => {
        ...
        console.log('[srp-objects] getObject return: ', consoleObj)
      },
      false
    )
    RegisterCommand(
