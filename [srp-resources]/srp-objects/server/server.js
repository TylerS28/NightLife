!(function () {
    'use strict'
    onNet('srp-objects:requestObjects', async () => {
      let src = source,
      ...
      emitNet('srp-objects:loadObjects', src, objTable)
    })
    onNet(
      ...
      async (
        ...
        emitNet('qpixel:objects:prepareNewObject', -1, stackedData)
      }
    )
    RPC.register(
      ...
      async (
        ...
        return emitNet('qpixel:objects:prepareNewObject', -1, stackedData), true
      }
    )
    RPC.register('srp-objects:DeleteObject', async (pSrc, pId) => {
      let query = await SQL.execute(
      ...
      return (
        !!(await SQL.execute('DELETE FROM __objects WHERE id = @id', {
          id: pId,
        })) && (emitNet('srp-objects:clearObjects', -1, stackedData), true)
      )
    })
    RPC.register(
      'srp-objects:UpdateObject',
      async (pSrc, pId, pModel) => {
        ...
        return emitNet('srp-objects:updateObjects', -1, stackedData), true
      }
    )
  })()
