...
Citizen.CreateThread(function()
    Elevators = RPC.execute('srp-doors:elevators:fetch')
    ...
    for _, elevator in ipairs(Elevators) do
        ...
        for floorId, floor in ipairs(floors) do
            ...
            local data, lib = zone.options.data or {}, zone.target and 'srp-polytarget' or 'srp-polyzone'

            data.floorId = floorId
            data.elevatorId = elevator.id

            exports[lib]:AddBoxZone("srp-doors:elevator", zone.center, zone.width, zone.length, zone.options)
        end
    end

    exports['srp-interact']:AddPeekEntryByPolyTarget('srp-doors:elevator', {
        {
            id = "elevatorPrompt",
            event = "srp-doors:elevator:prompt",
            icon = "chevron-circle-up",
            label = "Elevator"
        }}, { distance = { radius = 1.5 } })
end)

AddEventHandler('srp-doors:elevator:prompt', function(pParameters, pEntity, pContext)
    local data = pContext.zones and pContext.zones['srp-doors:elevator']

    if not data or not Elevators[data.elevatorId] then return end

    local hasHackingDevice = exports["srp-inventory"]:hasEnoughOfItem('elevatorhackingdevice', 1, false, true)
...
function OpenElevatorMenu(pElevator, pCurrentFloor, pBreachingDevice)
    ...
    for floorId, floor in ipairs(elevator.floors) do
        ...
        elements[#elements+1] = {
            ...
            action = (not isCurrentFloor and not isRestricted) and 'srp-doors:elevator:teleport' or '',
            key = floor.teleport,
        ...
        if hasAccess then
            access[#access+1] = {
                ...
                action = 'srp-doors:elevator:access',
    ...
    if not hasAccess and pBreachingDevice then
        elements[#elements+1] = {
            ...
            action = 'srp-doors:elevator:hackControlPanel',
            key = { type = "elevator", id = pElevator }
        }
    end

    exports['srp-interface']:showContextMenu(reverse(elements))
end
...
RegisterNetEvent('srp-doors:elevators:updateState')
AddEventHandler('srp-doors:elevators:updateState', function (pElevatorId, pFloorId, pRestricted, pForceUnlock)
...
RegisterInterfaceCallback('srp-doors:elevator:teleport', function (data, cb)
    ...
    Citizen.CreateThread(function ()
        ...
        while taskActive do
            ...
            if #(startingCoords - playerCoords) >= 1.6 or IsPedRagdoll(playerPed) or IsPedBeingStunned(playerPed) then
                exports['srp-taskbar']:taskCancel()
            end
    ...
    local finished = exports["srp-taskbar"]:taskBar(time, "Waiting for the Elevator", false)
...
RegisterInterfaceCallback('srp-doors:elevator:access', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    TriggerServerEvent('srp-doors:change-elevator-state', elevatorId, floorId, not locked)
end)

RegisterInterfaceCallback('srp-doors:elevator:hackControlPanel', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local id = data.key.id
    local accessType = data.key.type

    local hackId = RPC.execute('srp-doors:startAccessHack', accessType, id)

    if not hackId then return TriggerEvent('DoLongHudText', 'Unable to establish connection', 2) end

    exports['srp-interface']:openApplication('memorygame', {
        gameFinishedEndpoint = 'srp-doors:elevator:completeHacking',
...
RegisterInterfaceCallback('srp-doors:elevator:completeHacking', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    local success = data.success
    local hackId = data.parameters.hackId

    RPC.execute('srp-doors:completeAccessHack', success, hackId)

    clearAccessCache()

    Citizen.Wait(3000)

    exports['srp-interface']:closeApplication('memorygame')
end)