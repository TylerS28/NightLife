...
RegisterNetEvent('srp-doors:request-lock-state')
AddEventHandler('srp-doors:request-lock-state', function()
    local src = source
    TriggerClientEvent('srp-doors:initial-lock-state', src, doors)
end)

RegisterNetEvent('srp-doors:change-lock-state')
AddEventHandler('srp-doors:change-lock-state', function(pDoorId, pDoorLockState)
    if doors[pDoorId] then
        doors[pDoorId].lock = pDoorLockState
        TriggerClientEvent('srp-doors:change-lock-state', -1, pDoorId, pDoorLockState)
    end
end)
...
-- RegisterNetEvent("srp-doors:save-config")
-- AddEventHandler("srp-doors:save-config", function(pDoorData)
