...
AddEventHandler("qpixel:admin:searchPlayerInventory", function(pTarget)
    local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(pTarget))
    local cid = user:getCurrentCharacter().id
    TriggerClientEvent("server-inventory-open", source, "1", 'ply-'..cid)
end)

RegisterServerEvent('srp-admin:insertPrio')
AddEventHandler('srp-admin:insertPrio', function()
    local src = source
