...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    if context.type == 1 then
        context.flags = exports["srp-flags"]:GetPedFlags(pEntity)
    ...
    elseif context.type == 2 then
        context.flags = exports["srp-flags"]:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.flags = exports["srp-flags"]:GetObjectFlags(pEntity)
    end
...
function GetPedContext(pEntity, pContext)
    ...
    if pContext.flags["isJobEmployer"] then
        pContext.job = exports["srp-jobs"]:GetNPCJobData(npcId)
    end
end

--[[function isDisabled()
    return exports["srp-fw"]:getVar("dead") or
        exports["srp-fw"]:getVar("handcuffed")
end]]

function hasPhone()
    return exports["srp-inventory"]:hasEnoughOfItem("mobilephone", 1, false) or
...
function hasKeys(pEntity)
    ...
    local hasKeys = exports["srp-vehicles"]:HasVehicleKey(pEntity)
...
function hasRepairItems(type)
    ...
    for i, v in pairs(items) do
        if exports["srp-inventory"]:hasEnoughOfItem(v, 1, false) then
            return true
        end
    end
...
AddEventHandler("srp-polyzone:enter", function(zone, data)
    if zone == "srp-jobs:impound:dropOff" then IsImpoundDropOff = true end
...
RegisterNetEvent("srp-jobs:jobChanged")
AddEventHandler("srp-jobs:jobChanged", function(pJobId)
    CurrentJob = pJobId
