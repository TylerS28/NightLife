...
Citizen.CreateThread(function()
    ...
    exports["srp-polyzone"]:AddPolyZone("mrpd_classroom",
   ...
   --[[  exports["srp-polyzone"]:AddPolyZone("ktown",
    {
    ...
    exports["srp-polyzone"]:AddBoxZone("mrpd_meetingroom", vector3(474.07, -995.08, 30.69), 10.2, 5.2, {
        heading = 0,
        minZ = 29.64,
        maxZ = 32.84
    })

    exports["srp-polytarget"]:AddBoxZone("mrdp_change_picture", vector3(439.44, -985.89, 34.97), 1.0, 0.4, {
        heading = 0,
        minZ = 35.37,
        maxZ = 36.17
    })

    exports["srp-polytarget"]:AddBoxZone("mrpd_meetingroom_screen", vector3(474.02, -1001.79, 30.69), 3.6, 2.8, {
        heading = 1,
        minZ = 30.54,
        maxZ = 32.54
    })
   ...
   --[[  exports["srp-polytarget"]:AddBoxZone("screen01", vector3(-587.1, -1060.58, 24.20), 0.6, 0.25, {
        heading = 1,
        minZ = 23.54,
        maxZ = 24.47
    })

    exports["srp-interact"]:AddPeekEntryByPolyTarget("screen01", {{
        event = "srp-polce:changewhiteboardurl",
    ...
    exports["srp-interact"]:AddPeekEntryByPolyTarget("mrdp_change_picture", {{
        event = "srp-polce:changewhiteboardurl",
    ...
    exports["srp-interact"]:AddPeekEntryByPolyTarget("mrpd_meetingroom_screen", {{
        event = "srp-polce:changewhiteboardurl",
...
AddEventHandler("srp-polyzone:enter", function(pZoneName, pZoneData)
    if pZoneName == "srp-police:zone" then
        if exports["srp-fw"]:getChar("job") == pZoneData.job then
            currentPrompt = pZoneData.zone
            exports["srp-interaction"]:showInteraction(zoneData[pZoneData.zone].promptText)
            listenForKeypress(pZoneData.zone, pZoneData.action)
        end
    elseif pZoneName == "mrpd_classroom" then
        if not dui then
            dui = exports["srp-lib"]:getDui(currentClassRoomBoardUrl)
            AddReplaceTexture("prop_planning_b1", "prop_base_white_01b", dui.dictionary, dui.texture)
        else
            exports["srp-lib"]:changeDuiUrl(dui.id, currentClassRoomBoardUrl)
        end
        inClassRoom = true
    elseif zone == "mrpd_meetingroom" then
        if not dui then
          dui = exports["srp-lib"]:getDui(currentMeetingRoomBoardUrl)
          AddReplaceTexture("prop_planning_b1", "prop_base_white_01b", dui.dictionary, dui.texture)
        else
          exports["srp-lib"]:changeDuiUrl(dui.id, currentMeetingRoomBoardUrl)
        end
        inMeetingRoom = true
    end
end)

AddEventHandler("srp-polyzone:exit", function(pZoneName, pZoneData)
    if pZoneName == "srp-police:zone" then
    ...
    elseif pZoneName == "mrpd_classroom" then
        ...
        if dui ~= nil then
            exports["srp-lib"]:releaseDui(dui.id)
            dui = nil
        end
        inClassRoom = false
    elseif zone == "mrpd_meetingroom" then
...
AddEventHandler("srp-polce:changewhiteboardurl", function(pParams)
     exports['srp-interface']:openApplication('textbox', {
        callbackUrl = 'srp-ui:WhiteboardURL',
        key = 1,
        items = {
          {
            icon = "link",
...
RegisterInterfaceCallback('srp-ui:WhiteboardURL', function(data, cb)
...
AddEventHandler("police:changewhiteboardcli", function(pUrl, pRoom)
    ...
    --[[ elseif pRoom == "uwuscreenone" and inUwuCafe and dui then
        ...
        if inUwuCafe and dui then
            exports["srp-lib"]:changeDuiUrl(dui.id, uwuScreenOneBoardUrl)
        end ]]
