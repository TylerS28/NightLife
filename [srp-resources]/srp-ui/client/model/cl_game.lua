...
AddEventHandler("srp-spawn:characterSpawned", function()
    charSpawned = true
    Citizen.CreateThread(function()
        DisplayRadar(0)
...
function sendCharacterData()
    Citizen.CreateThread(function()
        ...
        SendUIMessage({ source = "srp-nui", app = "character", data = character });
        
        local endpointData = exports["srp-fiber"]:GetGameDetails()
        SendUIMessage({ source = "srp-nui", app = "game", data = endpointData });

        Citizen.Wait(5000)

        TriggerEvent('srp-ui:phoneReady')
        --TriggerServerEvent('srp-ui:phoneReady')
    end)
end
...
function isGrapeBarrel(p2)
  ...
  AddEventHandler("srp-inventory:itemUsed", function(pItem, pData)
  ...
  AddEventHandler("srp-vineyard:pickupBarrel", function(p1, p2, p3)
    ...
    exports['srp-objects']:DeleteObject(p3.meta.id)
    TriggerEvent("player:receiveItem", "vineyardbarrel", 1, false, data, data)
  end)
  
  
  RegisterUICallback("srp-vineyard:changeBarrelName", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["srp-ui"]:closeApplication("textbox")
    RPC.execute("srp-vineyard:changeBarrelName", data.key, data.values.name)
  end)
  
  RegisterUICallback("srp-vineyard:addGrapesToBarrel", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = "done" } })
    exports["srp-ui"]:closeApplication("textbox")
    local newData = data.key.newData
    local objectId = data.key.objectId
    local items = exports["srp-inventory"]:getItemsOfType("vineyardgrapes", false, true)
    ...
    RPC.execute("srp-vineyard:changeBarrelData", objectId, newData)
  end)
  
  local function getTimestamp(data)
    local remaining = math.floor((exports["srp-misc-code"]:getCurrentTime() - data.created_at) / 1000)
  ...
  AddEventHandler("srp-vineyard:barrelAction", function(p1, p2, p3)
    if not isGrapeBarrel(p3) then return end
    local data = p3.meta.data.metadata
    ...
    if p1.action == "change_name" then
      exports["srp-ui"]:openApplication("textbox", {
        callbackUrl = "srp-vineyard:changeBarrelName",
        key = objectId,
        items = {
    ...
    if p1.action == "seal" then
      ...
      local newData = { sealed = true, created_at = exports["srp-misc-code"]:getCurrentTime() }
      RPC.execute("srp-vineyard:changeBarrelData", objectId, newData)
      return
    end
    ...
    if p1.action == "add_grapes" then
      ...
      if #items == 0 then return end
      exports["srp-ui"]:openApplication("textbox", {
        callbackUrl = "srp-vineyard:addGrapesToBarrel",
    ...
    if p1.action == "pour_bottles" then
      local hasEnough = exports["srp-inventory"]:hasEnoughOfItem("vineyardwinebottleempty", 1, false, true)
      if not hasEnough then
        TriggerEvent("DoLongHudText", "You need empty wine bottles!", 2)
        return
      end
      TriggerEvent("doAnim","cokecut")
      local finished = exports["srp-taskbar"]:taskBar(10000, "Bottling Wine")
      ClearPedTasks(PlayerPedId())
      if finished ~= 100 then return end
      exports['srp-objects']:DeleteObject(objectId)
  ...
  AddEventHandler("srp-inventory:attachmentsToggle", function(pEnabled, pId)
  ...
  Citizen.CreateThread(function()
    exports["srp-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      {
        id = "peek_prop_vineyard_barrel",
        event = "srp-vineyard:pickupBarrel",
        icon = "shopping-basket",
        label = "Pick Up",
      },
      {
        id = "peek_prop_vineyard_barrel_details",
        event = "srp-vineyard:barrelAction",
        icon = "info-circle",
        label = "View Label",
    ...
    exports["srp-interact"]:AddPeekEntryByModel({ `prop_wooden_barrel` }, {
      ...
      isEnabled = function(p1, p2, p3)
        ...
        local hasEnough = exports["srp-inventory"]:hasEnoughOfItem("vineyardbarrelheadopener", 1, false, true)
        return hasEnough
      end,
    })
  end)
