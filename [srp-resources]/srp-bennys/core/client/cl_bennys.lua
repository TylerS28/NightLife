...
function RepairVehicle()
    ...
    ExitBennys()
    local finished = exports["srp-taskbar"]:taskBar(10000 + (missingBodyHealth / 50), "Repairing vehicle...", false, true)
...
function ExitBennys()
    ...
    TriggerServerEvent("srp-bennys:removeFromInUse", currentBennys)
    if currentBennys:find('^devbennys') then
        --
    else
        exports["srp-interface"]:showInteraction("Bennys")
    end
    currentBennys = nil
end
...
AddEventHandler("bennys:enter", function()
    ...
    --[[local isBennysInUse = RPC.execute("srp-bennys:checkIfUsed", currentBennys)
    ...
    ::hasAccess::
    TriggerServerEvent("srp-bennys:addToInUse", currentBennys)
    print(currentBennys)
    ...
    elseif currentBennys == "bennyshayes" then
        if exports['srp-business']:IsEmployedAt('hayes_autos') then
            enterLocation(bennys, repair)
        end
    ...
    elseif currentBennys == "bennysharmony" then
        if exports['srp-business']:IsEmployedAt('harmony_repairs') then
            enterLocation(bennys, repair)
        end
    elseif currentBennys == "ottosbay1" then
        if exports['srp-business']:IsEmployedAt('ottos_autos') then
            enterLocation(bennys, repair)
        end
...
RegisterNetEvent("srp-bennys:repairVeh")
AddEventHandler('srp-bennys:repairVeh', function()
    ...
    if car ~= 0 then
        ...
        local finished = exports["srp-taskbar"]:taskBar(4000,"Repairing Engine",false,false,playerVeh)
        Citizen.Wait(1000)
        local finished = exports["srp-taskbar"]:taskBar(3000,"Repairing Body",false,false,playerVeh)
        if (finished == 100) then
...
RegisterNetEvent('srp-public-bennys')
AddEventHandler('srp-public-bennys', function()
    TriggerServerEvent('srp-bennys:payPublicBennys')
end)

Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("pubbennys", vector3(-33.2, -1053.37, 28.4), 8.0, 9.4, {
        heading=340,
        minZ=25.72,
        maxZ=29.52
    })
