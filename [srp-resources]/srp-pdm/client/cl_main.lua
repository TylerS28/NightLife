...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("srp-pdm:CarTablePDM",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("srp-pdm:testdrive")
AddEventHandler("srp-pdm:testdrive", function()
	if exports["srp-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("srp-pdm:EnableBuy")
AddEventHandler("srp-pdm:EnableBuy", function()
	if #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["srp-business"]:IsEmployedAt("pdm") then
		return
	end
	...
	TriggerServerEvent("srp-pdm:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("srp-pdm:BuyEnabledCL")
AddEventHandler("srp-pdm:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("srp-pdm:PDMCommission")
AddEventHandler("srp-pdm:PDMCommission", function(newAmount)
	if exports["srp-business"]:IsEmployedAt("pdm") or #(vector3(-30.31887, -1089.345, 26.04948) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("srp-pdm:ReturnPDMTTable")
AddEventHandler("srp-pdm:ReturnPDMTTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('srp-pdm:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #carspawns do
		if #(vector3(carspawns[i]["x"],carspawns[i]["y"],carspawns[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["srp-business"]:IsEmployedAt("pdm") then
				if buyPlate[addplate] ~= nil then
					exports['srp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change | [G] to buy.")
				else
					exports['srp-interface']:showInteraction("$" .. math.ceil(price) .. " | Com: %" ..commission.. " | [E] to change")
				end
			else
				if buyPlate[addplate] ~= nil then
					exports['srp-interface']:showInteraction("$" .. math.ceil(price) .. " [G] to buy.")
				else
					exports['srp-interface']:showInteraction("Buy Price: $" .. math.ceil(price) .. " ")
				end
			end
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("srp-pdm:RequestPDMTTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			local VehicleProps = exports['srp-bennys']:FetchVehProps(personalvehicle)
			...
			TriggerServerEvent('srp-pdm:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('srp-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'PDM')
			TriggerServerEvent('srp-financials:business_money', price / 10, 'pdm', 'add')
			exports['srp-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("srp-pdm:FailedPurchase")
AddEventHandler("srp-pdm:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "jobvehicles" or this == "a-class" or this == "b-class" or this == "s-class" or this == "pdm-tuners" or this == "c-class" or this == "casino" or this == "d-class" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" then
		if ownerMenu then
			...
			TriggerServerEvent('srp-pdm:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["srp-business"]:IsEmployedAt("pdm") and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commissionpdm', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt("pdm") then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('srp-pdm:PDMCommission', amount)
		else
...
RegisterCommand('testdrivepdm', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt("pdm") then
		TriggerEvent('srp-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuypdm', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt("pdm") then
		TriggerEvent('srp-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)

RegisterNetEvent('srp-fuel:pdmmenushit', function()
...
RegisterInterfaceCallback('srp-pdm:pdmcom', function(data, cb, pParams)
	cb({ data = {}, meta = { ok = true, message = '' } })
	exports['srp-interface']:openApplication('textbox', {
		callbackUrl = 'srp-pdm:send_pdm',
		key = 1,
		items = {
		  {
			icon = "address-card", 
...
RegisterInterfaceCallback('srp-pdm:send_pdm', function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
	TriggerServerEvent("srp-pdm:wtf", data.values.pPDMPayCom)
end) 



RegisterNetEvent("srp-pdm:commissionpdm") 
AddEventHandler("srp-pdm:commissionpdm", function(args)
	print("YO")
	if exports["srp-business"]:IsEmployedAt("pdm") then
		print("IM PDM WORKER")
		local amount = args
...
RegisterNetEvent("srp-pdm:testdrivepdm")
AddEventHandler("srp-pdm:testdrivepdm", function()
	if exports["srp-business"]:IsEmployedAt("pdm") then
		TriggerEvent('srp-pdm:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
...
RegisterNetEvent("srp-pdm:enableBuypdm")
AddEventHandler("srp-pdm:enableBuypdm", function()
	if exports["srp-business"]:IsEmployedAt("pdm") then
		TriggerEvent('srp-pdm:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
