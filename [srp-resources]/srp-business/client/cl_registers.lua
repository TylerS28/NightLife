...
for id,register in ipairs(activeRegisters) do
        ...
        exports["srp-polytarget"]:AddBoxZone(ptId, register.polytarget[1], register.polytarget[2], register.polytarget[3], register.polytarget[4])
        exports['srp-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "srp-business:registerPurchasePrompt",
            id = ptId .. "_purchase",
            icon = "cash-register",
            label = "make payment",
        ...
        exports['srp-interact']:AddPeekEntryByPolyTarget(ptId, {{
            event = "srp-business:registerChargePrompt",
            id = ptId .. "_charge",
            icon = "credit-card",
...
AddEventHandler('srp-business:registerPurchasePrompt', function(pParameters, pEntity, pContext)
    ...
    local acceptContext = {
        ...
        },
        {
            ...
            action = "srp-business:finishPurchasePrompt",
            icon = 'credit-card',
    ...
    exports['srp-interface']:showContextMenu(acceptContext)
end)

AddEventHandler('srp-business:registerChargePrompt', function(pParameters, pEntity, pContext)
    exports['srp-interface']:openApplication('textbox', {
        callbackUrl = 'srp-interface:business:charge',
...
RegisterNetEvent('srp-business:activePurchase')
AddEventHandler("srp-business:activePurchase", function(data)
    activePurchases[data.registerId] = data
end)

--Remove at registerId pos
RegisterNetEvent('srp-business:closePurchase')
AddEventHandler("srp-business:closePurchase", function(data)
    activePurchases[data.registerId] = nil
end)

RegisterInterfaceCallback('srp-business:finishPurchasePrompt', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    local success, message = RPC.execute("srp-business:completePurchase", data.key)  
    if not success then
        TriggerEvent("DoLongHudText", message, 2)
    end 
end)

RegisterInterfaceCallback("srp-interface:business:charge", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    ...
    RPC.execute("srp-business:startPurchase", {cost = cost, comment = comment, registerId = data.key.registerId, business = data.key.business})
end)