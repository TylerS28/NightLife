...
RegisterCommand("creationmenu", function()
	...
	local hasPerm = RPC.execute("srp-business:hasPermission", "ace_industries", "craft_access")
	
	if employed and hasPerm then
		TriggerEvent("srp-business:stash:creationMenu")
	end
end)
...
RegisterInterfaceCallback("srp-business:stash:enteredAuthCode", function(data, cb)
...
function getPDAuthCode()
  ...
  exports["srp-interface"]:openApplication("textbox", {
    callbackUrl = "srp-business:stash:enteredAuthCode",
    key = "",
    items = {
...
function openInventory(type, data)
  ...
  if type == "stash" then
    local success = RPC.execute("srp-business:hasPermission", biz, "stash_access")
  ...
  elseif type == "craft" then
    local success = RPC.execute("srp-business:hasPermission", biz, "craft_access")
    if not success and isPD() then
      success = getPDAuthCode()
    end
    if success then
...
function enterPoly(name, data)
  ...
  exports["srp-interface"]:showInteraction("[E] " .. name)
end
...
AddEventHandler("srp-polyzone:exit", function(name, data)
  if name == "business_stash" or name == "business_craft" then
...
local creationMenu = {
  key = 1,
  show = true,
  callbackUrl = "srp-business:client:createStash",
  items = { }
}

Citizen.CreateThread(function()
  exports["srp-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      id = "business_stash",
      event = "srp-business:client:openStash",
      icon = "archive",
      label = "Open Storage",
    },
  }, 
  { 
    ...
    isEnabled = function(pEntity)
      local objInfo = exports["srp-objects"]:GetObjectByEntity(pEntity)
      return objInfo ~= nil
    end
  })
  exports["srp-interact"]:AddPeekEntryByModel(businessStashProps, {
    {
      ...
      event = "srp-business:client:removeStash",
      icon = "trash",
      label = "Remove Storage",
    },
    {
      id = "business_stash_update",
      event = "srp-business:client:changeStashModel",
      icon = "archive",
...
RegisterNetEvent("srp-business:stash:creationMenu")
AddEventHandler("srp-business:stash:creationMenu", function ()
  ...
  exports["srp-interface"]:openApplication("textbox", creationMenu)
end)

AddEventHandler("srp-business:client:removeStash", function (pData, pEntity)
  local objInfo = exports["srp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end

	TriggerEvent("animation:PlayAnimation", "hammering")

  local finished = exports["srp-taskbar"]:taskBar(15000, "Destroying Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local deleted = exports["srp-objects"]:DelObject(objInfo.id)
  if not deleted then
...
AddEventHandler("srp-business:client:openStash", function (pData, pEntity)
  local objInfo = exports["srp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local success = RPC.execute("srp-business:hasPermission", businessCode, "stash_access")
  if not success and isPD() then
    success = getPDAuthCode()
  end
...
AddEventHandler("srp-business:client:changeStashModel", function (pData, pEntity)
  local objInfo = exports["srp-objects"]:GetObjectByEntity(pEntity)
  if objInfo == nil then return end
  ...
  local menuData = {
    key = { id = objInfo.id },
    show = true,
    callbackUrl = "srp-business:client:updateStashModel",
    items = {
      { icon = "object-ungroup", _type = "select", label = "New Unit Model", name = "stashProp", options = unitOptions },
    }
  }
  exports["srp-interface"]:openApplication("textbox", menuData)
end)

RegisterInterfaceCallback("srp-business:client:updateStashModel", function(pData, pCb) 
  ...
  local finished = exports["srp-taskbar"]:taskBar(15000, "Changing Storage")
  ClearPedTasks(PlayerPedId())
  if finished ~= 100 then return end

  local updated = exports["srp-objects"]:UpdateObject(pData.key.id, stashProp)
end)

RegisterInterfaceCallback("srp-business:client:createStash", function(pData, pCb) 
  pCb({ data = {}, meta = { ok = true, message = "done" } })
  exports["srp-interface"]:closeApplication("textbox")
  ...
  local result = exports["srp-objects"]:PlaceAndSaveObject(
    stashProp, 
    metaData, 
    { groundSnap = true }
  )
