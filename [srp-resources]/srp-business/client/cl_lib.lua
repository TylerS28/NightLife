...
function HasPermission(pBusiness, pPermission)
    ...
    if not PermissionCache[pBusiness][pPermission] or (GetGameTimer() - PermissionCache[pBusiness][pPermission].UpdatedAt) > 15 * 60000 then
        ...
        local success = RPC.execute("srp-business:hasPermission", pBusiness, pPermission, cid)
...
RegisterNetEvent('srp-spawn:phonerefresh')
AddEventHandler('srp-spawn:phonerefresh', function()
    RefreshEmploymentList()
end)
...
RegisterNetEvent('srp-business:employmentStatus')
AddEventHandler('srp-business:employmentStatus', function(pBusiness, pEmployed)
