...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("chop_zone", vector3(1359.1, -2097.81, 52.0), 15, 15, {
    heading=40,
    --debugPoly=false,
    minZ=51.0,
    maxZ=55.0
...
AddEventHandler("srp-polyzone:enter", function(zone)
    if zone == "chop_zone" and GetVehiclePedIsIn(PlayerPedId(), false) == aspectChopCar then
        ...
        aspectStartChopping()
        exports['srp-interface']:showInteraction('[E] Start Chopping')
    end
end)
...
function ChopVehicleTyre(vehicle, boneID, tyreIndex)
    ...
    local finished = exports["srp-taskbar"]:taskBar(10000,"Slicing Vehicle Tyre")
...
function ChopVehicleDoor(vehicle, boneID, doorIndex)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["srp-taskbar"]:taskBar(14000,"Messing With Door")
...
function ChopVehicleRemains(vehicle, boneID)
    ...
    FreezeEntityPosition(GetPlayerPed(-1),true)
    local finished = exports["srp-taskbar"]:taskBar(25000,"Seeing Whats Left ..")
...
AddEventHandler('aspect-chop:cooldown', function()
    ...
    if ClockedIntoChop then
        TriggerEvent('srp-phone:chop_shop_offer')
        CreateChopCar()
    end
end)
...
function ChopVehiclePart(vehicle)
    ...
    if success then
        TriggerServerEvent("srp-chopshop:vehicle_loot", bone.type)
...
RegisterNetEvent('srp-chopshop:clock-in')
AddEventHandler('srp-chopshop:clock-in', function()
    if CoolDownActive then
        TriggerEvent('DoLongHudText', 'You have a active cooldown!', 2)
    elseif ClockedIntoChop then
        TriggerEvent('srp-chopshop:clock-out')
    else
        ClockedIntoChop = true
        TriggerEvent('srp-phone:signInInfoChopshop')
        Citizen.Wait(math.random(60000, 120000))
        TriggerEvent('srp-phone:chop_shop_offer')
    end
end)

RegisterNetEvent('srp-chopshop:clock-out')
AddEventHandler('srp-chopshop:clock-out', function()
    if ClockedIntoChop then
