...
RegisterServerEvent('srp-jacksons_autos:CarTable')
AddEventHandler('srp-jacksons_autos:CarTable', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('srp-jacksons_autos:ReturnCarTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('srp-jacksons_autos:FinaceEnabledSV')
AddEventHandler('srp-jacksons_autos:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('srp-jacksons_autos:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('srp-jacksons_autos:BuyEnabledSV')
AddEventHandler('srp-jacksons_autos:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('srp-jacksons_autos:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('srp-jacksons_autos:RequestCarTable')
AddEventHandler('srp-jacksons_autos:RequestCarTable', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('srp-jacksons_autos:ReturnCarTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('srp-jacksons_autos:ChechMoney')
AddEventHandler('srp-jacksons_autos:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('srp-jacksons_autos:FailedPurchase', src)
    end
end)

RegisterServerEvent('srp-jacksons_autos:BuyVehicle')
AddEventHandler('srp-jacksons_autos:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
