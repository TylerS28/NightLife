...
function updateCarTable(model,price,name)
	for i=1, #carTable do
		...
		TriggerServerEvent("srp-jacksons_autos:CarTable",carTable)
	end
end

local myspawnedvehs = {}

RegisterNetEvent("srp-jacksons_autos:testdrive")
AddEventHandler("srp-jacksons_autos:testdrive", function()
	if exports["srp-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.9081, -1032.289, 13.719951) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("srp-jacksons_autos:EnableBuy")
AddEventHandler("srp-jacksons_autos:EnableBuy", function()
	if #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 and exports["srp-business"]:IsEmployedAt('jacksons_autos') then
		return
	end
	...
	TriggerServerEvent("srp-jacksons_autos:BuyEnabledSV",vehplate)
end)

RegisterNetEvent("srp-jacksons_autos:BuyEnabledCL")
AddEventHandler("srp-jacksons_autos:BuyEnabledCL", function(addplate)
	buyPlate[addplate] = true
	Citizen.Wait(60000)
	buyPlate[addplate] = nil
end)

RegisterNetEvent("srp-jacksons:CommissionPrice")
AddEventHandler("srp-jacksons:CommissionPrice", function(newAmount)
	if eexports["srp-business"]:IsEmployedAt('jacksons_autos') or #(vector3(-775.4795, -1032.456, 14.129936) - GetEntityCoords(PlayerPedId())) > 50.0 then
...
RegisterNetEvent("srp-jacksons_autos:ReturnCarTable")
AddEventHandler("srp-jacksons_autos:ReturnCarTable", function(newTable)
	carTable = newTable
	DespawnSaleVehicles()
	SpawnSaleVehicles()
end)
...
function AttemptBuy(tableid)
	...
	TriggerServerEvent('srp-jacksons_autos:ChechMoney',name, model, price)
...
function DrawPrices()
	for i = 1, #pSpawnLocations do
		if #(vector3(pSpawnLocations[i]["x"],pSpawnLocations[i]["y"],pSpawnLocations[i]["z"]) - GetEntityCoords(PlayerPedId())) < 1.5 then
			...
			if exports["srp-business"]:IsEmployedAt('jacksons_autos') then
...
function SpawnSaleVehicles()
	if not hasspawned then
		TriggerServerEvent("srp-jacksons_autos:RequestCarTable")
		Citizen.Wait(1500)
	end
...
function CloseCreator(name, veh, price, plate)
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			TriggerServerEvent('srp-jacksons_autos:BuyVehicle', plate, vehname, vehicle, price, VehicleProps)
			TriggerServerEvent('srp-cardealer:log', exports['isPed']:isPed('cid'), vehname, price, plate, 'Jacksons Autos')
			TriggerServerEvent('srp-financials:business_money', price / 10, 'jacksons_autos', 'add')
			exports['srp-interface']:hideInteraction()
			DespawnSaleVehicles()
			SpawnSaleVehicles()
		end
...
RegisterNetEvent("srp-jacksons_autos:FailedPurchase")
AddEventHandler("srp-jacksons_autos:FailedPurchase", function()
...
function ButtonSelected(button)
	...
	elseif this == "b-class" or this == "s-class" then
		if ownerMenu then
			...
			TriggerServerEvent('srp-jacksons_autos:ChechMoney',button.name, button.model, button.costs)
		end
	end
end
	...
	while true do
		...
		if vehshop.opened then
			...
			for i,button in pairs(menu.buttons) do
				...
				if exports["srp-business"]:IsEmployedAt('jacksons_autos') and i >= vehshop.menu.from and i <= vehshop.menu.to then
...
RegisterCommand('commission', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt('jacksons_autos') then
		...
		if tonumber(amount) > 0 and tonumber(amount) <= 60 then
			TriggerEvent('srp-jacksons_autos:CommissionPrice', amount)
		else
...
RegisterCommand('testdrive', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('srp-jacksons_autos:testdrive')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)
...
RegisterCommand('enableBuy', function(source, args, raw)
	if exports["srp-business"]:IsEmployedAt('jacksons_autos') then
		TriggerEvent('srp-jacksons_autos:EnableBuy')
	else
		TriggerEvent('DoLongHudText', 'You dont have permissions for this!', 2)
	end
end)