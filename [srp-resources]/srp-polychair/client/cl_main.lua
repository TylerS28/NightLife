Citizen.CreateThread(function()
    ...
    for idx,polyTarget in ipairs(ChairZones) do
        exports['srp-interact']:AddPeekEntryByPolyTarget('srp-polychair:chair_' .. tostring(idx) .. "-".. tostring(polyTarget[4].name), {{
            event = "srp-polychair:chairSit",
...
function chairSit(chairPosition, cancel)
    ...
    if isSitting then
        ...
        isSitting = false
        TriggerEvent('srp-polychair:stopSitting')
    else
        ...
        TriggerEvent('srp-polychair:startSitting')
    end
    exports["srp-flags"]:SetPedFlag(ped, 'isSittingOnChair', isSitting)
end

AddEventHandler('srp-polychair:chairSit', function(pParameters, pEntity, pContext)
    chairSit(pParameters.chairPosition, true)
end)

RegisterNetEvent("srp-emotes:sitOnChair")
AddEventHandler("srp-emotes:sitOnChair", function(pArgs, pEntity, pContext)
    chairSit(nil, true)
end)
