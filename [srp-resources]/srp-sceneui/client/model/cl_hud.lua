...
--
local cmdsSet = {}
RegisterNUICallback("srp-ui:hudSetPreferences", function(data, cb)
    ...
    if data["hud.presets"] then
      ...
      for _, p in pairs(data["hud.presets"]) do
        ...
        if not cmdsSet[idx] then
          ...
          exports["srp-keybinds"]:registerKeyMapping("", "HUD", "Enable " .. s, "+hud_enable_" .. s, "-hud_enable_" .. s)
          RegisterCommand('+hud_enable_' .. s, function()
            exports["srp-ui"]:sendAppEvent("preferences", eventOptions)
          end, false)
    ...
    TriggerEvent("srp-preferences:setPreferences", data)
end)
...
AddEventHandler("srp-ui:watch", function()
...
RegisterNetEvent('srp-inventory:itemCheck')
AddEventHandler('srp-inventory:itemCheck', function (item, hasItem, quantity)
...
AddEventHandler("qpixel:admin:currentDevmode", function(devmode)
    setGameValue("modeDev", devmode)
end)

RegisterNetEvent("qpixel:admin:currentDebug")
AddEventHandler("qpixel:admin:currentDebug", function(debugToggle)
...
AddEventHandler('srp-vehicles:pursuitMode', function (pEnabled, pMode, pModes)
...
AddEventHandler('srp-jail:attachedCollar', function (pAttached)
...
RegisterNetEvent('srp-boosting:client:setBoostCompletions')
AddEventHandler('srp-boosting:client:setBoostCompletions', function (pAmount)
...
RegisterNetEvent('srp-weapons:client:setWeaponFireRate')
AddEventHandler('srp-weapons:client:setWeaponFireRate', function (pAmount)
    weaponFireRate = pAmount
    setHudValue("weaponFireRate", weaponFireRate)
end)

RegisterNetEvent('srp-weapons:client:showWeaponFireRate')
AddEventHandler('srp-weapons:client:showWeaponFireRate', function (pToggle)
    showWeaponFireRate = pToggle
    setHudValue("showWeaponFireRate", pToggle)
end)

RegisterNetEvent('srp-ui:hud:forceShowMinimap', function(pForce)
...
function roundedRadar()
    ...
    Citizen.CreateThread(function()
        ...
        -- for k,v in pairs(exports["srp-fw"]:getModule("Blips")) do
...
function getFuel()
    fuel = exports["srp-vehicles"]:CurrentFuel() or 0
end
    ...
    Citizen.CreateThread(function()
        while engineOn do
            ...
            if GetPedInVehicleSeat(veh, -1) == GetPed() then
                harnessDurability = exports["srp-vehicles"]:GetVehicleMetadata(veh, 'harness')
            end

            local degradation = exports["srp-vehicles"]:GetVehicleDegradation(veh)
...
AddEventHandler('srp-ui:setGaze', function (pEntity, pDelay)
    GazeEntity = pEntity
    GazeDelay = pDelay
end)
...
Citizen.CreateThread(function()
  while true do
    ...
    if toggleParachuteOff and pState == -1 then
      ...
      exports["srp-ui"]:sendAppEvent("hud", { hasParachute = false })
...
AddEventHandler("hud:equipParachute", function()
  hasParachute = true
  exports["srp-ui"]:sendAppEvent("hud", { hasParachute = true })
end)

AddEventHandler("srp-island:onIsland", function(pState)
    onIsland = pState
end)
