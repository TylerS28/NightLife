...
function dwStatusHud(pValues)
  if dwRefreshChipCount then
   -- dwRefreshChipCount = false
    dwCurrentChipCount = RPC.execute("srp-casino:getCurrentChipCount")
  end
...
--   exports["srp-ui"]:sendAppEvent("status-hud", {
--     show = true,
--     title = title,
--     values = values,
--   })
    ...
    while true do 
        if not sittingAtBlackjackTable then
            if closestChair ~= nil and closestChairDist < 1.6 then
                if not timeoutHowToBlackjack then
                    if not uiIntShown then
                        ...
                        if exports['srp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) and not isHighRoller then
                            background = "success"
                        ...
                        -- exports["srp-ui"]:showInteraction(
                        --   "[E] Play! " .. dwGetLimitTitleString(),
                        --   background
                        -- )
                        exports["srp-ui"]:showInteraction("[E] Play! " .. dwGetLimitTitleString() , background)
                        uiIntShown = true
                    end
                end
            elseif uiIntShown then
                uiIntShown = false
                local background = "info"
                if exports['srp-inventory']:hasEnoughOfItem("casinomember", 1, false, true) or exports['srp-inventory']:hasEnoughOfItem("casinovip", 1, false, true) then
                    background = "success"
                else
                    background = "error"
                end
                exports['srp-ui']:hideInteraction()
            end
        elseif uiIntShown then
            uiIntShown = false
            local background = "info"
...
RegisterUICallback("srp-ui:blackjackHitStand", function(data, cb)
  ...
  elseif action == "double" then
    local dwUseChipsResult = RPC.execute("srp-casino:useChips", currentBetAmount)
  ...
  elseif action == "surrender" then
    ...
    -- RPC.execute("srp-casino:winChips", currentBetAmount / 2)
...
AddEventHandler("srp-ui:application-closed", function(name)
...
Citizen.CreateThread(function()
    while true do
        ...
        --         data[#data + 1] = {
        --           action = "srp-ui:blackjackHitStand",
...
RegisterUICallback("srp-ui:blackJackBet", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
...
AddEventHandler("srp-ui:application-closed", function(name, data)
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    -- RPC.execute("srp-casino:winChips", currentBetAmount)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
...
AddEventHandler("Blackjack:blackjackWin",function(tableId, dwHitBlackjack)
    ...
    -- RPC.execute("srp-casino:winChips", (currentBetAmount * 2) + extra)
    dwRefreshChipCount = true
    if closeToCasino then
        blackjackGameInProgress = false
        dealerPed = getDealerFromTableId(tableId)
