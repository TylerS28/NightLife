...
RegisterServerEvent('srp-carrent:hasmoney')
AddEventHandler('srp-carrent:hasmoney', function(price, vehicle)
    local vehicleName = vehicle
    local user = exports["srp-fw"]:getModule("Player"):GetUser(source)
    local money = tonumber(user:getCash())
    if money >= price then
            user:removeMoney(price)
            TriggerClientEvent("srp-carrent:spawncar", user.source, vehicleName)
    else
