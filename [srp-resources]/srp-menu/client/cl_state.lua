...
AddEventHandler('srp-polyzone:entered:garages', polyZoneEnter)
AddEventHandler('srp-polyzone:exited:garages', polyZoneExit)
...
RegisterNetEvent("menu:send:srp-housing:garages", function(pTable, pHid, pHeading)
        exports["srp-polyzone"]:AddBoxZone("housingGarage-"..pHid, pTable, 10, 10, {
            heading = pHeading,
            minZ = -25,
            maxZ = 100,
          })
...
AddEventHandler('srp-polyzone:enter', function(name)
    ...
    for k, v in pairs (sharedGarages) do
        if name == v then
            local garageData = exports["srp-garages"]:getGarageData(name)
