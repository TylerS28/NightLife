...
function toggleNUI(){
    ...
    SendNuiMessage(JSON.stringify({
        action: 'setKeyBinds',
        selectionMenu: global.exports["srp-keybinds"].getKeyMapping("+enableSelectionMenu"),
        selectionKey: global.exports["srp-keybinds"].getKeyMapping("+adminSelect")
    }))
}
function exitNUI()
{
...
on('__cfx_nui:closeMenu', (data, cb) => {
    ...
    SetNuiFocus(false,false)
    SetNuiFocusKeepInput(false)
    emit('qpixel:admin:updateUI')


    if(data.toSelect){
        global.exports["srp-admin"].enterSelection();
    }
...
on("onResourceStart", (resourceName) => {
    if(GetCurrentResourceName() != 'srp-admin') {
      return;
    }
...
on('__cfx_nui:insideCommand', (data, cb) => {
    ...
    if(data.inside){
        ...
        emit('srp-binds:should-execute',false)
        ...
        emit('srp-binds:should-execute',true)
        if(blockingActions){
...
global.exports["srp-keybinds"].registerKeyMapping("", "zzAdmin", "Selection Menu", "+enableSelectionMenu", "-enableSelectionMenu", "")
