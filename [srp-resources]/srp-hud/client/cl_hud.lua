...
AddEventHandler('baseevents:leftVehicle', function ()
    exports['srp-vehicles']:nitroLevelLeave()
    pursuitmode = 0
end)

AddEventHandler('baseevents:enteredVehicle', function (currentVehicle, currentSeat)
    exports['srp-vehicles']:nitroLevelEnter()
    
    pursuitmode = exports['srp-hud']:pursuitmode() 
  end)
...
RegisterNetEvent("srp-hud:EnableHud", function(status)
    isLoggedIn = status
    if status == true then
        exports['srp-interface']:showInteraction('[F] To pay respects')
        HudActive = false
    end
end)

RegisterNetEvent("srp-hud:changeRange", function(pRange)
    voice = pRange or 2
end)

RegisterNetEvent('qpixel:admin:currentDevmode', function(status)
    developer = status
end)

RegisterNetEvent('qpixel:admin:currentDebug', function(status)
    debug = status
end)

RegisterNetEvent('srp-weapons:client:setWeaponFireRate', function (pAmount)
--    weaponFireRate = pAmount
end)

RegisterNetEvent("srp-weapons:client:showWeaponFireRate", function (pBool)
    weaponFireRate = pBool
end) 

RegisterNetEvent("srp-hud:harness_values", function(cinturon)
	harness = cinturon
end)

RegisterNetEvent("srp-hud:pursuit_values", function(pursuit)
	pursuitmode = pursuit
    print(pursuit)
end)
...
Citizen.CreateThread(function ()
	...
	while true do
        ...
        local pRadioActive = exports['srp-voice']:pRadioActive()
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isLoggedIn then
            ...
            local pRadio = exports['srp-voice']:pRadioActive()
            ...
            SendNUIMessage({
                ...
                gps = gps,
                gpu = gpu,
                nos = exports['srp-vehicles']:nitroLevel(),
                dev = developer,
                debug = debug,
...
RegisterNetEvent('srp-hud:ChangeThirst', function(amount)
...
RegisterNetEvent('srp-hud:ChangeHunger', function(amount)
	if isLoggedIn == true then
...
RegisterNetEvent("client:newStress", function(status, amount)
    ...
    if status == true then
        ...
        Wait(1000)
        TriggerServerEvent('srp-hud:UpdateStress_SV', pStress)
...
RegisterNetEvent('qpixel:admin:maxstats', function()
    currentValues["thirst"] = 100
	currentValues["hunger"] = 100 
    TriggerEvent("heal", PlayerPedId())
    TriggerEvent("Hospital:HealInjuries", PlayerPedId(),true) 
    TriggerServerEvent("srp-death:reviveSV", source)
...
RegisterNUICallback("time", function(data)
    exports['srp-carhud']:SetTimeEnabled(data.time)
end)

RegisterNUICallback("SaveSettings", function(data)
    local pInfo = exports["srp-fw"]:getModule("LocalPlayer")
    ...
    exports['srp-carhud']:SetCompassFPS(tonumber(data.compassFPSValue))
    exports['srp-carhud']:SetSpeedFPS(tonumber(data.speedFPSValue))
    TriggerEvent('setModel', model)
    TriggerEvent('srp-phone:grabBackground', bg)
    TriggerEvent('changeBG', bg)
    TriggerEvent("srp-phone:setSomeShit",bg,model,twatNotify,callNotify)
    local circleTaskbar = GetResourceKvpInt(cid..'-circleTaskbar')
    if circleTaskbar == 1 then
        TriggerEvent("srp-preferences:setPreferences", true)
    elseif circleTaskbar == 0 then
        TriggerEvent("srp-preferences:setPreferences", false)
    end
...
AddEventHandler('srp-preferences:setPreferences', function(bool)
    local data = {
        ["hud.misc.circle.taskbar.enabled"] = bool
    }
    exports["srp-interface"]:sendAppEvent('preferences', data)
end)

RegisterNetEvent("srp-hud:SetValues", function()
    local pInfo = exports["srp-fw"]:getModule("LocalPlayer")
    local cid = exports['isPed']:isPed('cid')
    ...
    local model = exports['srp-phone']:getModelForHud()
    local background = exports['srp-phone']:getBackgroundForHud()
    local twat = exports['srp-phone']:getTwatForHud()
    local call = exports['srp-phone']:getCallForHud()
    ...
    exports['srp-carhud']:SetCompassFPS(SetCompassFPSValue)
    exports['srp-carhud']:SetSpeedFPS(SetSpeedFPSValue)
    ...
    if SetTimeToggled == 1 then
        exports['srp-carhud']:SetTimeEnabled(true)
    elseif SetTimeToggled == 0 then
        exports['srp-carhud']:SetTimeEnabled(false)
    end
...
RegisterCommand('ui-r', function()
    ...
    TriggerEvent('srp-hud:EnableHud', false)
    TriggerEvent('closeInventoryGui')
    TriggerEvent('srp-banking:ResetUI')
    TriggerEvent('srp-radio:ResetUI')
    TriggerEvent('srp-menu:ResetUI')
    TriggerEvent('srp-interact:ResetUI')
    TriggerEvent('srp-mdt:ResetUI')
    TriggerEvent('phone:reset')
    TriggerEvent('clothing:close')
    exports['srp-interface']:showInteraction("Restarting UI .")
    Wait(1000)
    exports['srp-interface']:showInteraction("Restarting UI ..")
    Wait(1000)
    exports['srp-interface']:showInteraction("Restarting UI ...")
    Wait(1000)
    exports['srp-interface']:showInteraction("UI Successfully Restarted")
    Wait(1000)
    exports['srp-interface']:hideInteraction()
...
RegisterNetEvent('srp-hud:boosting_value')
AddEventHandler('srp-hud:boosting_value', function(pValue)
