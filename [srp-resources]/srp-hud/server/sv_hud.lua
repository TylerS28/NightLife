...
AddEventHandler('carfill:checkmoney', function(costs)
	...
	if exports['srp-fw']:getModule('GetCash')(src) >= costs then
		TriggerClientEvent("RefuelCarServerReturn", src)
		exports['srp-fw']:getModule('RemoveCash')(src, costs)
	else
		moneyleft = costs - exports['srp-fw']:getModule('GetCash')(src)
		TriggerClientEvent('DoLongHudText', src, "Requires $" .. costs)
	end
end)

RegisterServerEvent('srp-hud:UpdateStress_SV')
AddEventHandler('srp-hud:UpdateStress_SV',function(amount)
	local src = source
