...
RegisterNetEvent('srp-shops:exchangeGold')
AddEventHandler('srp-shops:exchangeGold', function()
	if exports["srp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
		local finished = exports["srp-taskbar"]:taskBar(120000,"Trading Bars")
		if finished == 100 then
			if exports["srp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
				TriggerServerEvent('srp-banking:addMoney', 35000)
...
AddEventHandler('DragonJewelCraft', function()
	local isEmployed = exports["srp-business"]:IsEmployedAt("JewelDragon")
    if isEmployed then
...
AddEventHandler('slushy:general', function()
	...
	local finished = exports ["srp-ui"]:taskBarSkill(18000,math.random(10,20))
	if (finished == 100) then
		ExecuteCommand("e handshake")
		local finished = exports ["srp-ui"]:taskBarSkill(18000,math.random(10,20))
		if (finished == 100) then
...
AddEventHandler("illegal:crafting", function()
	...
	local finished = exports['srp-taskbar']:taskBar(12000, 'Inspecting Table')
	if (finished == 100) then
...
-- 	local finished = exports['srp-taskbar']:taskBar(20000, 'Smelting')
-- 	if (finished == 100) then
...
RegisterNetEvent("srp-shops:redCounter")
AddEventHandler("srp-shops:redCounter", function()
    TriggerEvent("server-inventory-open", "1", "redcircle_counter");
end)

RegisterNetEvent("srp-shops:redCounter2")
AddEventHandler("srp-shops:redCounter2", function()
...
AddEventHandler('weapon:general', function()
	TriggerServerEvent('srp-shops:WeaponShopStatus')
end)
...
AddEventHandler("hunting:level1", function()
	if exports["srp-inventory"]:getQuantity("coyotepelt") >= 1 then
		playerAnim()
		local finished = exports["srp-taskbar"]:taskBar(4000,"Selling Coyotepelt",true,false,playerVeh)
		if finished == 100 then
			if exports["srp-inventory"]:getQuantity("coyotepelt") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'coyotepelt', 1)
				TriggerServerEvent('srp-banking:addMoney', math.random(80, 150))
			else
...
AddEventHandler("hunting:level2", function()
	if exports["srp-inventory"]:getQuantity("buck") >= 1 then
		playerAnim()
		local finished = exports["srp-taskbar"]:taskBar(4000,"Selling Buck",true,false,playerVeh)
		if finished == 100 then
			if exports["srp-inventory"]:getQuantity("buck") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'buck', 1)
				TriggerServerEvent('srp-banking:addMoney', math.random(160, 200))
			else
...
AddEventHandler("hunting:level3", function()
	if exports["srp-inventory"]:getQuantity("boar") >= 1 then
		playerAnim()
		local finished = exports["srp-taskbar"]:taskBar(4000,"Selling Boar",true,false,playerVeh)
		if finished == 100 then
			if exports["srp-inventory"]:getQuantity("boar") >= 1 then
				ClearPedTasksImmediately(PlayerPedId())
				TriggerEvent('inventory:removeItem', 'boar', 1)
				TriggerServerEvent('srp-banking:addMoney', math.random(350, 500))
			else
...
--     exports['srp-menuinteract']:AddButton("Sell Coyotepelt", "Wow you suck", "hunting:level1", 'example button 1', "jobmenu")
--     exports['srp-menuinteract']:AddButton("Sell Buck", "Wow you kinda okay", "hunting:level2", 'example button 2', "jobmenu")
--     exports['srp-menuinteract']:AddButton("Sell Boar", "Wow you good", "hunting:level3", 'example button 3', "jobmenu")
--     exports['srp-menuinteract']:SubMenu("Hunting Menu" , "Click to select sell!" , "jobmenu")
-- end
...
Citizen.CreateThread(function()
	while true do
		...
		SetDiscordRichPresenceAssetText('Spectrum Rp')
		SetDiscordRichPresenceAction(0, 'Join Today!', 'https://discord.gg/fhwm3yu9Dr')
	end
end)