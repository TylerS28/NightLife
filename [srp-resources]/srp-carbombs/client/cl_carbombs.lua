...
local function explodeVehicle(pEntity)
  exports['srp-sync']:SyncedExecution('NetworkExplodeVehicle', pEntity, 1, 0, 0)
  TriggerServerEvent('srp-miscscripts:carbombs:removeBomb', NetworkGetNetworkIdFromEntity(pEntity))
end
...
local function checkForCarBomb(pEntity) 
  ...
  local progress = exports['srp-taskbar']:taskBar(2500, 'Searching for car bomb...', true)
  ClearPedTasks(PlayerPedId())

  if progress ~= 100 then return end

  local metaData = RPC.execute("srp-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
...
--[[   local remoteBomb = RPC.execute('srp-usableprops:hasPhoneBomb', NetworkGetNetworkIdFromEntity(pEntity))

  if remoteBomb and remoteBomb.planted then
    local number = remoteBomb.bomb.number
    TriggerServerEvent('srp-usableprops:defusePhoneBomb', number)
    TriggerEvent('srp-police:client:showTextPopup', { show = true, text = 'Cell Phone Found, Number: ' .. number })
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    TriggerServerEvent('srp-miscscripts:carbombs:foundBomb', NetworkGetNetworkIdFromEntity(pEntity), carBombMeta)
...
RegisterNetEvent("srp-miscscripts:carbombs:foundBombAll", function(pNetTable)
...
AddEventHandler("srp-inventory:itemUsed", function(name, info)
  ...
  local prompt = exports['srp-interface']:OpenInputMenu(elements)
  ...
  local progress = exports['srp-taskbar']:taskBar(2500, 'Planting car bomb...', true)
  ...
  local added = RPC.execute('srp-miscscripts:carbombs:addCarBomb', netId, minSpeed, ticksBeforeExplode, ticksForRemoval, gridSize, 0, timeToComplete)
  TriggerEvent('inventory:removeItem', 'car_bomb', 1)
  
  return TriggerEvent('DoLongHudText', 'Successfully added car bomb to vehicle', 1)
end)

AddEventHandler('srp-miscscripts:carBombs:removeBomb', function (pData, pEntity)
    local metaData = RPC.execute("srp-miscscripts:carbombs:getCarBombDataFromNetID", NetworkGetNetworkIdFromEntity(pEntity))
    local carBombMeta = metaData.carBombData or false
  ...
  if carBombMeta and carBombMeta.hasCarBomb then
    ...
    TriggerEvent('animation:PlayAnimation', 'kneel')
    TriggerServerEvent('srp-miscscripts:carbombs:removeBomb', myNetIdentifier)
...
--[[     exports['srp-interface']:openApplication('memorygame', {
      gameFinishedEndpoint = 'srp-miscscripts:carBombs:completeHacking',
