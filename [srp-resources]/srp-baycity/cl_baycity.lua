...
-- -105.12893676758, 6471.46484375, 31.626705169678 Heading: 40.885322570801
            if exports["srp-inventory"]:hasEnoughOfItem("thermite",1,false) then
                ...
                local FirstDoorBigBank = exports["srp-taskbar"]:taskBar(9000,"Preparing For Hack")
                if FirstDoorBigBank == 100 then
                    ...
                    function() -- Success
                        ...
                        TriggerEvent('srp-vaultDoor3', -1, method)
                        --TriggerEvent('srp-ffs2:startheist')
                    ...
                    --     function() -- success
                    --         StopAnimTask(PlayerPedId(), hackAnimDict, "hack_loop", 1.0)
                    --         TriggerEvent('srp-vaultDoor2', -1, method)
                    --         TriggerEvent('srp-ffs2:startheist')
    ...
    if distance < 1.5 then
        if exports["srp-inventory"]:hasEnoughOfItem("thermite",1,false) then
            ...
            if FirstDoorBigBank == 100 then
                ...
                exports["memorygame"]:thermiteminigame(1, 3, 4, 10,
                function() -- Success
                    TriggerEvent("srp-BayCityTriggerSecondDoor")
                    TriggerEvent("inventory:removeItem","thermite",1)
                    TriggerEvent('DoLongHudText', 'Well done dumbass please cry more!', 2)
                end,
                function() -- failure
...
RegisterNetEvent('srp-BayCityTriggerSecondDoor')
AddEventHandler('srp-BayCityTriggerSecondDoor', function()
	anim66()
	Citizen.Wait(3500)
end)

RegisterNetEvent('srp-Seconddoorsbaycity') 
AddEventHandler('srp-Seconddoorsbaycity', function()
    TriggerServerEvent("srp-doors:change-lock-state", 524, false)
end)
    ...
    if distance < 1.5 then
        if exports["srp-inventory"]:hasEnoughOfItem("thermite",1,false) then
            ...
            if FirstDoorBigBank == 100 then
                ...
                exports["memorygame"]:thermiteminigame(1, 3, 4, 10,
                function() -- Success
                    TriggerEvent("srp-BayCityTriggerFirstDoor")
                    TriggerEvent("inventory:removeItem","thermite",1)
                    TriggerEvent('DoLongHudText', 'Well done dumbass please cry more!', 2)
...
RegisterNetEvent('srp-BayCityTriggerFirstDoor')
AddEventHandler('srp-BayCityTriggerFirstDoor', function()
	anim69()
	Citizen.Wait(3500)
end)

RegisterNetEvent('srp-firstdoorsbaycity') 
AddEventHandler('srp-firstdoorsbaycity', function()
    TriggerServerEvent("srp-doors:change-lock-state", 525, false)
end)
...
RegisterNetEvent('srp-vaultDoor3')
AddEventHandler('srp-vaultDoor3', function(method)
...
RegisterNetEvent("srp-baycity:getDoorCheckCL")
AddEventHandler("srp-baycity:getDoorCheckCL", function(result)
...
Citizen.CreateThread(function()
    ...
    while true do 
        ...
        if isNear then
            TriggerServerEvent('srp-baycity:getGetDoorStateSV')
...
RegisterNetEvent("srp-ptfxparticlebaycity")
AddEventHandler("srp-ptfxparticlebaycity", function(method)
    local ptfx
...
-- 252.99769592285, 220.96379089355, 101.68360137939 Heading: 166.93214416504
    ...
    TriggerServerEvent("srp-particleserverbaycity", method)
    SetPtfxAssetNextCall("scr_ornate_heist")
    local effect = StartParticleFxLoopedAtCoord("scr_heist_ornate_thermal_burn", ptfx, 0.0, 0.0, 0.0, 1.0, false, false, false, false)
    ...
    DeleteObject(bomba)
    StopParticleFxLooped(effect, 0)
	TriggerEvent("srp-Seconddoorsbaycity")
end

RegisterNetEvent("srp-ptfxparticlebaycity1")
AddEventHandler("srp-ptfxparticlebaycity1", function(method)
    local ptfx
...
-- 252.99769592285, 220.96379089355, 101.68360137939 Heading: 166.93214416504
    ...
    DetachEntity(bomba, 1, 1)
    FreezeEntityPosition(bomba, true)
    TriggerServerEvent("srp-particleserverbaycity1", method)
    SetPtfxAssetNextCall("scr_ornate_heist")
    local effect = StartParticleFxLoopedAtCoord("scr_heist_ornate_thermal_burn", ptfx, 0.0, 0.0, 0.0, 1.0, false, false, false, false)
    ...
    DeleteObject(bomba)
    StopParticleFxLooped(effect, 0)
	TriggerEvent("srp-firstdoorsbaycity")
end