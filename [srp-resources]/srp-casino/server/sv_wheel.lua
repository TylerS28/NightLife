...
RPC.register('srp-casino:purchaseMembershipCard',function(pCid)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    if user:getCash() >= 250 then
          user:removeMoney(250)
          TriggerClientEvent('srp-casino:purchaseMembership',src)
    end
end)

RegisterServerEvent('srp-luckywheel:getLucky')
AddEventHandler('srp-luckywheel:getLucky', function(money)
    local src = source
            ...
            if user:getCash() >= amount  then
              --  user:removeMoney(amount)
                ...
                SetTimeout(7000, function()
                    ...
                    TriggerClientEvent("srp-luckywheel:rollFinished", -1)
                end)
                TriggerClientEvent("srp-luckywheel:doRoll", -1, _priceIndex)
            else
                TriggerClientEvent("srp-luckywheel:rollFinished", -1)  
...
RegisterServerEvent("attempt:spin", function()
    local pSrc = source
    if CasinoSpin then
        TriggerClientEvent("srp-luckywheel:spinit", pSrc, true)
    else
