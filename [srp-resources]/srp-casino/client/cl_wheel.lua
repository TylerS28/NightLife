...
AddEventHandler("srp-casino:wheel:spinWheelTurbo", function()
  ...
  RPC.execute("srp-casino:wheel:spinWheel", true, json.decode(info.information).state_id)
end)
...
RegisterNetEvent("srp-luckywheel:doRoll")
AddEventHandler("srp-luckywheel:doRoll", function(_priceIndex) 
    _isRolling = true
	SetEntityHeading(_wheel, 358.35986328125)
    Citizen.CreateThread(function()
...
RegisterNetEvent("srp-luckywheel:rollFinished")
AddEventHandler("srp-luckywheel:rollFinished", function() 
    _isRolling = false
end)
...
function doRoll()
    if not _isRolling then
        ...
        local lib, anim = _lib, 'enter_right_to_baseidle'
            ...
            TriggerServerEvent("srp-luckywheel:getLucky")
            TaskPlayAnim(playerPed, lib, 'armraisedidle_to_spinningidle_high', 8.0, -8.0, -1, 0, 0, false, false, false)
        
    end
end

RegisterNetEvent("srp-luckywheel:spinit")
AddEventHandler("srp-luckywheel:spinit", function() 
    if not _isRolling  then
        doRoll()
    end		
end)
