...
function drawCarForWins()
  ...
  --   exports['srp-vehicles']:SetVehicleAppearance(spinningCar, carOnShowAppearence)
  -- end
  -- if carOnShowMods then
  --   exports['srp-vehicles']:SetVehicleMods(spinningCar, carOnShowMods)
  -- end
...
AddEventHandler("srp-casino:elevatorEnterCasino", function()
  enterCasino(true)
end)
AddEventHandler("srp-casino:elevatorExitCasino", function()
  enterCasino(false)
end)

AddEventHandler("srp-polyzone:enter", function(zone)
...
--   -- RPC.execute("srp-bugs:playerLogAction", {
...
function enterCasino(pIsInCasino)
  ...
  if not inCasino then
    ...
    TriggerEvent("srp-casino:casinoExitedEvent")
    inCasino = false
    postGateTriggered = false
    return
  end
  -- local pedCoords = RPC.execute("srp-casino:getSpawnedPedCoords", true)
  -- handlePedCoordsBaby(pedCoords)
  doInitStuff()
  TriggerEvent("srp-casino:casinoEnteredEvent")
end

Citizen.CreateThread(function()
  -- enterCasino(true)
  Wait(10000)
  RPC.execute("srp-casino:getCurrentInteriorSetName")
end)
    ...
    while inCasino do
      ...
      if inCasino and postGateTriggered then
        if hasMembership() then
          --RPC.execute("srp-casino:generateRaffleTicket")
        end
      end
    end
...
function doSomeTextureReplacesBruv(apply)
  if not apply then
    ...
    exports["srp-lib"]:releaseDui(wheelDui.id)
    wheelDui = nil
    return
  end
  if wheelDui == nil then
    wheelDui = exports["srp-lib"]:getDui(wheelUrl, 1024, 1024)
    AddReplaceTexture('vw_prop_vw_luckywheel_01a', 'script_rt_casinowheel', wheelDui.dictionary, wheelDui.texture)
  else
    exports["srp-lib"]:changeDuiUrl(wheelDui.id, wheelUrl)
  end
end
...
function showThe6StrImage(apply)
  if not apply then
    ...
    exports["srp-lib"]:releaseDui(tvDui.id)
    tvDui = nil
    return
  end
  if tvDui == nil then
    tvDui = exports["srp-lib"]:getDui(tvUri, 512, 256)
    AddReplaceTexture('vw_prop_vw_cinema_tv_01', 'script_rt_tvscreen', tvDui.dictionary, tvDui.texture)
  else
    exports["srp-lib"]:changeDuiUrl(tvDui.id, tvUri)
  end
end
...
-- chips
RegisterInterfaceCallback("srp-ui:casinoPurchaseChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount 
  RPC.execute("srp-casino:purchaseChips", amount)
  exports['srp-ui']:closeApplication('textbox')
end)

RegisterInterfaceCallback("srp-ui:transferChipsAmount", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  local amount = data.values.amount
  local stateId = data.values.state_id
  RPC.execute("srp-casino:transferChips", stateId, amount)
  exports['srp-ui']:closeApplication('textbox')
end)

AddEventHandler("srp-casino:purchaseChipsAction", function(pArgs)
  ...
  --     callbackUrl = 'srp-ui:casinoPurchaseChipsAmount',
  --     key = 1,
  --     header = "",
  --     items = {
  --       {
  ...
  if action == "purchase:dirty" then
    ...
    if exports["srp-inventory"]:hasEnoughOfItem("markedbills", 20, false, true) then
      TriggerEvent("inventory:removeItem", "markedbills", 20)
      payment = payment + (250 * 20) -- $5k / $250 per
    end
    if exports["srp-inventory"]:hasEnoughOfItem("rollcash", 5, false, true) then
      TriggerEvent("inventory:removeItem", "rollcash", 5)
      payment = payment + (30 * 5) -- $150 / $30 per
    end
    if exports["srp-inventory"]:hasEnoughOfItem("band", 5, false, true) then
      TriggerEvent("inventory:removeItem", "band", 5)
      payment = payment + (300 * 5) -- $1500, / $300 per
    end
    RPC.execute("srp-casino:purchaseChipsDirty", payment)

  elseif action == "withdraw:cash" then
    RPC.execute("srp-casino:withdrawChips", "cash")
  elseif action == "withdraw:bank" then
    RPC.execute("srp-casino:withdrawChips", "bank")
  -- elseif action == "transfer" then
  --   Wait(100)
  --   exports['srp-ui']:openApplication('textbox', {
  --     callbackUrl = 'srp-ui:transferChipsAmount',
  --     key = 1,
  --     items = {
  --       {
  --         icon = "user",
...
AddEventHandler("srp-casino:purchaseMembership", function()
  ...
  exports['srp-ui']:openApplication('textbox', {
    callbackUrl = 'srp-ui:casinoGetMembership',
    key = 1,
    header = '',
    items = {
      {
...
RegisterInterfaceCallback("srp-ui:casinoGetMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['srp-ui']:closeApplication('textbox')
  local state_id = {
...
AddEventHandler("srp-casino:purchaseMembershipCard", function()
  RPC.execute("srp-casino:purchaseMembershipCard")
end)

AddEventHandler("srp-casino:getLoyaltyCard", function()
  local characterId = exports["isPed"]:isPed("cid")
...
AddEventHandler("srp-casino:purchaseVipMembership", function()
  local characterId = exports["isPed"]:isPed("cid")
  local jobAccess = RPC.execute("IsEmployedAtBusiness", { character = { id = characterId }, business = { id = "casino"} })
  if not jobAccess then
    TriggerEvent("DoLongHudText", "Please talk to a member of floor staff", 2)
  ...
  exports['srp-ui']:openApplication('textbox', {
    callbackUrl = 'srp-ui:casinoGetVipMembership',
    key = 1,
    header = '',
    items = {
      {
...
RegisterInterfaceCallback("srp-ui:casinoGetVipMembership", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  exports['srp-ui']:closeApplication('textbox')
  TriggerEvent("player:receiveItem", "casinovip", 1, false, { state_id = data.values.state_id }, { state_id = data.values.state_id })
end)

AddEventHandler("srp-casino:purchaseDrinks", function(data)
  TriggerEvent("server-inventory-open", "46", "Shop")
end)

function hasMembership(hrOnly)
  local characterId = exports["isPed"]:isPed("cid")
  local info = exports["srp-inventory"]:GetInfoForFirstItemOfName("casinomember")
  if not info then
    return false
  end
...
-- end, false)
-- Citizen.CreateThread(function()
--   exports["srp-keybinds"]:registerKeyMapping("", "Casino", "Add Coords", "+addCasinoCoords", "-addCasinoCoords")
-- end)

-- Citizen.CreateThread(function()
--   for _, v in pairs(exports["srp-casino"]:getPedCoordsC()) do
