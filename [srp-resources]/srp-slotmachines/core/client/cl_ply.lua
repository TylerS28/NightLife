...
local function drawStatusHUD(state, pValues)
  ...
  local balance = RPC.execute("srp-casino:getCurrentChipCount")
  local values = {}
  isSlotMachinesEnabled, currentJackPot = RPC.execute("srp-slotmachines-sv:checkIfSlotMachinesState")
  ...
  -- exports["srp-textui"]:sendAppEvent("status-hud", {
  --   show = state,
  --   title = title,
  --   values = values
  -- })
...
local function getUpFromSlotMachine()
  ...
  TriggerServerEvent("srp-slotmachines-sv:getUpFromSlotMachine")
  drawStatusHUD(false, {})
    ...
    if isSeatedAtSlotMachine then
      if not isBetContextShowing then
        if currentSlotMachineDetails ~= nil then
          ...
          table.insert(data, {
            action = "srp-textui-slotmachines:placeBet",
            title = "Place bet",
            description = "$100.00",
            key = {
              coinValue = 100,
          ...
          table.insert(data, {
            action = "srp-textui-slotmachines:leaveMachine",
            title = "Leave Machine",
            description = "",
            key = {},
          })

          exports["srp-ui"]:showContextMenu(data) 
        end
      end
    end
...
local function initializeSlotMachines()
  ...
  TriggerServerEvent("srp-slotmachines-sv:getMachineData")
        ...
        while true do
          ...
          if not isNear(plyPos, v.chairPos, 0.75) then
            if isInteractionShowing then
              ...
              exports["srp-ui"]:hideInteraction(interactionBackground)
            end
          ...
          if not isInteractionShowing then
            ...
            if exports["srp-inventory"]:hasEnoughOfItem("casinomember", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            elseif exports["srp-inventory"]:hasEnoughOfItem("casinovip", 1, false, true) then
              interactionBackground = "success"
              canPlayAtSlotMachine = true
            else
            ...
            exports["srp-ui"]:showInteraction("[E] Play " .. v.label, interactionBackground)
          end

        
          if IsControlJustReleased(1, 38) and canPlayAtSlotMachine then --Key: E
            TriggerServerEvent("srp-slotmachines-sv:sitDownAtSlotMachine", k)
            break
          end
...
--#[Event Handlers]#--
RegisterNetEvent("srp-slotmachines-cl:getMachineData")
AddEventHandler("srp-slotmachines-cl:getMachineData", function(data)
...
RegisterNetEvent("srp-slotmachines-cl:updateMachineData")
AddEventHandler("srp-slotmachines-cl:updateMachineData", function(machine, ply)
  if currentSlotMachineObjs[machine] ~= nil then
    currentSlotMachineObjs[machine].ply = ply
  end
end)

RegisterNetEvent("srp-slotmachines-cl:sitDownAtSlotMachine")
AddEventHandler("srp-slotmachines-cl:sitDownAtSlotMachine", function(machine, coinValue)
  local plyPed = PlayerPedId()
  local plyPos = GetEntityCoords(plyPed)
...
RegisterNetEvent("srp-slotmachines-cl:playSlotMachineSound")
AddEventHandler("srp-slotmachines-cl:playSlotMachineSound", function(machinePos, machineModel, sound)
  if isSMSetup then
    local soundRef = ""
...
RegisterNetEvent("srp-slotmachines-cl:stopReels")
AddEventHandler("srp-slotmachines-cl:stopReels", function(reel,rot)
...
RegisterNetEvent("srp-slotmachines-cl:spinFinished")
AddEventHandler("srp-slotmachines-cl:spinFinished", function(amountWon)
  if amountWon > 0 then
    -- RPC.execute("srp-casino:winChips", amountWon)
...
RegisterNetEvent("srp-slotmachines-cl:toggleSlotMachines")
AddEventHandler("srp-slotmachines-cl:toggleSlotMachines", function(state)
...
AddEventHandler("srp-casino:casinoEnteredEvent", function()
  isInsideCasino = true
  isSlotMachinesEnabled, currentAccJackpot = RPC.execute("srp-slotmachines-sv:checkIfSlotMachinesState")
...
AddEventHandler("srp-casino:casinoExitedEvent", function()
  isInsideCasino = false
  isSlotMachinesEnabled = RPC.execute("srp-slotmachines-sv:checkIfSlotMachinesState")
  if not isSlotMachinesEnabled then
    return
  end
...
--#[NUI Callbacks]#--
RegisterInterfaceCallback("srp-textui-slotmachines:placeBet", function(data, cb)
  ...
  if currentSlotMachine and not isSlotMachineSpinning then
    ...
    local useChipsResult = RPC.execute("srp-casino:useChips", currentBet)
    ...
    local user = exports["srp-fw"]:getModule("LocalPlayer")
    local cid = exports["isPed"]:isPed("cid")

    --local info = (exports["srp-inventory"]:GetInfoForFirstItemOfName("casinoloyalty") or { information = "{}" })
    TriggerServerEvent(
      "srp-slotmachines-sv:spinMachine",
...
RegisterInterfaceCallback("srp-textui-slotmachines:leaveMachine", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = "done" } })
  getUpFromSlotMachine()
  SendNUIMessage({
    app = "status-hud",
