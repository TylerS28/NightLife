...
RegisterNUICallback('srp-interface:applicationClosed', function(data, cb)
    if (data.name ~= 'textbox' or data.callbackUrl ~= 'srp-interface:inputResponse') then return end
...
RegisterNUICallback('srp-interface:inputResponse', function(data, cb)
...
function OpenInputMenu(pEntries, pValidation)
    ...
    exports['srp-interface']:openApplication('textbox', {
        callbackUrl = 'srp-interface:inputResponse',
        key = inputId,
        items = pEntries,
        show = true,
    })
