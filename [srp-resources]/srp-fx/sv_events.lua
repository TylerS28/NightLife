...
RegisterServerEvent("srp-fx:setSteamId")
AddEventHandler("srp-fx:setSteamId", function()
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local steamId = user:getVar("steamid")
    TriggerClientEvent("srp-fix:setSteamIdClient", src, steamId)
end)

RegisterServerEvent("srp-fx:smoke:grenade")
AddEventHandler("srp-fx:smoke:grenade", function(pCoords)
...
RegisterNetEvent("srp-fx:chain:blingDiamonds", function(pCoords, pType, pSize, pStrength, pScale)
