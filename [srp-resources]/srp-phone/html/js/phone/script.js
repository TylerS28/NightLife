    ...
    $(".phone-screen").on('click', '#wifi', function (e) {
        if (wifizone){
            if (enablePassword) {
                ...
                $.post('https://srp-phone/connectWifi', JSON.stringify({}));
            }
        }
    });
    setInterval(function () {
        ...
        if (actionButton !== undefined) {
            switch (actionButton) {
                case "home":
                    ...
                    $.post('https://srp-phone/refreshMySMS', JSON.stringify({status: false}));
                    $.post('https://srp-phone/closeTwat', JSON.stringify({}));
                    $.post('https://srp-phone/checkNotify', JSON.stringify({}));
        ...
        if (action !== undefined) {
            switch (action) {
                ...
                case "newPostSubmit":
                    ...
                    break;
                case "group-manage":
                    $.post('http://srp-phone/manageGroup', JSON.stringify({ GroupID: $(this).data('action-data') }));
                    break;
                case "btnTaskGang":
                ...
                case "getCallHistory":
                    if (callStates[currentCallState] === "isCallInProgress" && currentContainer !== "incoming-call")
                        ...
                        if (!isBurnah) {
                            ...
                            $.post('http://srp-phone/getRecentCallsByCid', JSON.stringify({
                                cid: burnahCid
                            }));
                        }
                    break;
                ...
                case "doj-app":
                    ...
                    $.post('https://srp-phone/getActiveDOJ', JSON.stringify({}))
                    break;
                case "camera-app":
                    $(".camera-inside").empty();
                    openContainer('camera')
                    $.post('https://srp-phone/getCurrentCameras', JSON.stringify({}))
                    break;
                case "lsbn":
                    openContainer('lsbn');
                    break;
                ...
                case "btnBankBusters":
                    ...
                    $.post('https://srp-phone/getCurrentBanks', JSON.stringify({}))
                    setTimeout(() => {
                        $.post('https://srp-phone/inBankBusterGroup', JSON.stringify({}))
                    }, 1500)
//                    $.post('https://srp-phone/getCurrentBanks', JSON.stringify({}))
                 //   $.post('https://srp-phone/sendNotifAlert', JSON.stringify({
                 //       type: "alert"
                 //   }))
                    break;
                case "documents":
                    ...
                    $.post('https://srp-phone/getDocuments', JSON.stringify({
                        pType: txt
                    }))
                    break;
                case "diamond":
                    ...
                    $.post('https://srp-phone/getDiamondSportsEvents', JSON.stringify({}))
                    break;
                default:
                    $.post('http://srp-phone/' + action, JSON.stringify({}));
                    break;
    ...
    $(".document-submit-cid").click(function (e) {
        ...
        setTimeout(() => {
            ...
            $.post('https://srp-phone/shareDocumentPermanent', JSON.stringify({
                documentID: pDocID,
                targetCID: ptCID
            }))        
        })
    ...
    $('.btn-share-local').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://srp-phone/shareDocumentLocal', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-drop-qr-code').click(function() {
        ...
        setTimeout(() => {
            let pDocID = document.getElementById("document-id").value;
            $.post('https://srp-phone/dropQRCode', JSON.stringify({
                documentID: pDocID,
            }))        
        })
    })
    ...
    $('.btn-delete-document').click(function(){
        ...
        setTimeout(() => {
            ...
            $.post('https://srp-phone/deleteDocument', JSON.stringify({
...
function createDiamond(events, isEmployed) {
    $.each(events, function (__, event) {
        ...
        $('#delete-diamondEvent').click(function(){
            ...
            $.post('http://srp-phone/deleteDiamondEvent', JSON.stringify({
...
function createDOJDepartments(doj) {
    ...
    window.addEventListener('message', function (event) {
        ...
        if (item.openPhone === true) {
            ...
            $.post('http://srp-phone/checkModel', JSON.stringify({}));

            $.post('http://srp-phone/retreivePhoneContacts', JSON.stringify({}));
...
//                $(".send-message").show();
        ...
        switch (item.openSection) {
            ...
            case "messageRead":
                ...
                $.post('https://srp-phone/refreshMySMS', JSON.stringify({status: true}));
...
function burnahMessages() {
    $.post('https://srp-phone/burnahMessages', JSON.stringify({
        cid: burnahCid
    }))
}
...
function createCamera(cameras) {
    ...
    $(".eye-camera").click(function () {
        ...
        $.post('https://srp-phone/OpenCamera', JSON.stringify({
            cameraId: cameraId
        }));
    })
    ...
    $(".remove-camera").click(function() {
        ...
        $(".removeCamera").empty();
        $.post("https://srp-phone/getCameraAccessPlayers", JSON.stringify({
            cameraId: cameraId
        }))
    })
};
...
$("#add-camera-submit").click(function() {
    ...
    complateInputQuick()
    setTimeout(() => {
        $.post("https://srp-phone/addUserToCamera", JSON.stringify({
            cameraId: camid,
...
$("#remove-camera-submit").click(function() {
    ...
    setTimeout(() => {
        $("#cameraRemoveID").val("");
        $.post("https://srp-phone/removeUserFromCamera", JSON.stringify({
            cameraId: camid,
            targetPhoneNumber: cid
        }))
    }, 1500)
...
function gpsJobCenter(data) {
    $.post(
      'https://srp-phone/jobcenter-setgps',
...
">${moment.utc(emails[email].time).local().fromNow()}
    ...
    $.post('http://srp-phone/emailReads', JSON.stringify({}));
}
...
$('.diamondsports-createevent').on('click', '#submit-input', function(){ 
    ...
    $.post('http://srp-phone/submitNewEventDiamondSports', JSON.stringify({
...
function purchaseDarkMarket(data) {
    itemamount = 1
    $.post("https://srp-phone/purchase-darkMarket", JSON.stringify({item:data,itemamount}))
}
function trackvehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://srp-phone/vehtrack", JSON.stringify({vehplate}))
}

function spawnVehicle(vehplate) {
    // console.log("VEH PLATE",vehplate)
    $.post("https://srp-phone/vehspawn", JSON.stringify({vehplate}))
}
...
$('.sellingvehicle').on('click', '#submit-input', function(){
    ...
    complateInput();
    setTimeout(() => {
        $.post("https://srp-phone/sellVeh", JSON.stringify({
            plate: vplate,
            paypal: paypal,
            amount: amount
        }))
...
$("#selfieclose").click(function () {
    $('.selfieBox').hide();
    $.post('http://srp-phone/closeSelfi', JSON.stringify({}));
})

$("#settings").click(function () {

  $.post('http://srp-phone/openSettings', JSON.stringify({}));
})
...
$('#submit-input-the-pm').on('click', function() {
    ...
    setTimeout(() => {
        $('#the-pm-input').val("")
        $.post("https://srp-phone/sendThePM", JSON.stringify({
            thePM: misinput
        }))
    }, 1900)
})
...
$(document).ready(function () {
    ...
    $('.send-ping').click(function () {
        ...
        // $.post('http://srp-phone/btnCamera', JSON.stringify({}));
        $.post('http://srp-phone/srp-ui:pingSend', JSON.stringify({
            number: targetId
        }));
        $('.send-value').val('');
    });
...
$("#contacts-edit-submit").click(function (e) {
    ...
    $.post('http://srp-phone/editContactSubmit', JSON.stringify({
        name: clean,
    ...
    setTimeout(() => {
        $.post('http://srp-phone/refreshContacts', JSON.stringify({}));
    }, 2000);
...
$("#wallpaper-form").submit(function (e) {
    ...
    $.post('http://srp-phone/updateMyWallpaper', JSON.stringify({
        name: escapedName,
    }));
...
$("#stock-form").submit(function (event) {
    event.preventDefault();
    $.post('https://srp-phone/stocksTradeToPlayer', JSON.stringify({
...
$('#stock-transfer').click(function(){
    ...
    complateInput()
    setTimeout(() => {
        $.post('https://srp-phone/exchangeCrypto', JSON.stringify({
...
$('#stock-submit').click(function(){
    ...
    complateInput()
    setTimeout(() => {
        $.post('https://srp-phone/buyCrypto', JSON.stringify({
            stockID: escapeHtml(stockID),
            amount: escapeHtml(amount),
        }));
    }, 1000);
...
$(".submit-bg").click(function (event){
    if ($('#bg-set-body').val() == '') {
        ...
        $.post('http://srp-phone/submitBg', JSON.stringify({
            bg: bg
        }));
        ...
        case "messages":
            ...
            setTimeout(function () {
                if (!isBurnah) {
                    ...
                    $.post('http://srp-phone/getTextsByCid', JSON.stringify({
                        cid: burnahCid
                    }))
                }
            }, 300);
...
$("#phone-submit-number").click(function () {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            $.post("https://srp-phone/getCallHistory", JSON.stringify({}))
        }, 300)
    }, 900)
})
...
function clickImage(img) {
    $('.selfieBox').click(function(){
        copyToClipboard(img);
        $.post('http://srp-phone/closeSelfieBox', JSON.stringify({}));
    });
}
...
$('.group-manage-entries').on('click', '.group-manage-fire', function () {
    ...
    $.post('http://srp-phone/srp-phone:fireEmp', JSON.stringify({ id: $(this).data('id'), group: $(this).data('group'), name: $(this).data('name') }));
});
...
$('.garage-entries').on('click', '.garage-pay', function (e) {
    $.post('http://srp-phone/vehiclePay', JSON.stringify({ vehiclePlate: $(this).data('plate') }));
    setTimeout(function () {
        $.post('http://srp-phone/btnGarage', JSON.stringify({}));
    }, 1500);
});
...
$('.delete-yp').click(function (event) {
    ...
    setTimeout(() => {
        ...
        $('#youradhere').hide();
        hasPostedYP = false;
        $.post('http://srp-phone/deleteYP', JSON.stringify({ name: $(this).data('name'), number: $(this).data('number') }));
    }, 1500)
});
...
$("#h-check").click(function () {
    ...
    $.post('http://srp-phone/checkHouse', JSON.stringify({}))
})

$("#edit-h").click(function () {
    // console.log("CHECK PROPERTY")
    // $.post('http://srp-phone/editHouse', JSON.stringify({}))
    //noPropty()
})

$("#sell-h").click(function () {
    // console.log("CHECK PROPERTY")
    $.post('http://srp-phone/sellsHouse', JSON.stringify({}))
})
...
$('.h-found').on('click', '.h-found-purchase', function (event) {
    ...
    complateInput();
    $('#twat-modal').modal('close');
    $.post('http://srp-phone/purchaseHouse', JSON.stringify({
...
$('.sell-house-own').on('click', '.confirm-sellHouse', function (event) {
    ...
    $.post('http://srp-phone/sellOwnHouse', JSON.stringify({
        hid: $('#h-p-id').val()
    }))
})
...
$('#submit-h-price').click(function(){
    ...
    $.post('http://srp-phone/editHousePrice', JSON.stringify({
        hid: hid,
        price: price
    }))
    $('.housing-edit-price').css('display', 'none');
...
$('.phone-selfie').click(function(){
    $.post('http://srp-phone/phone:selfie', JSON.stringify({}));
    closePhoneShell()
})
...
$(document).on('click', '#twat-sendimg', function (e) {
    ...
    $.post('http://srp-phone/phone:selfie', JSON.stringify({}));
    e.preventDefault();
    $.post('http://srp-phone/PostNewImage', JSON.stringify({}),
        function (url) {
...
//                 $.post('http://srp-phone/btnAnswer', JSON.stringify({}));
//             }
//         });
...
//                             setTimeout(() => {
//                                 Notif.addClass('notification-container-fade-out')
//                                 $.post('http://srp-phone/btnHangup', JSON.stringify({}));
//                             }, 1000)
//                         }, 200)
//                     }
//                     // Notif.addClass('notification-container-fade-out')
...
//             }
//             Notif.addClass('notification-container-fade-out')
//             $.post('http://srp-phone/pingEnded', JSON.stringify({}));
//             setTimeout(() => {
//                 Notif.remove()
//                 var count = 0
...
//             $.post('http://srp-phone/srp-ui:pingAccept', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
//             activePing = false
//             $.post('http://srp-phone/srp-ui:pingReject', JSON.stringify({}));
//             const Notif = $(".notification-container").filter(`[data-id="${Id}-ping"]`)
//                 if (Notif) {
//                     var count = 0
...
$(".house-entries, .access-house-entries").on('click', '.h-setgps', function (e) {
    ...
    $.post('http://srp-phone/sethGps', JSON.stringify({hid: overid}));
})
...
$(".house-key-entries").on('click', '.h-removeKey', function (e) {
    ...
    $.post('http://srp-phone/whatthefuckdoesthisshitwork', JSON.stringify({cid: overid, hid: overid2}));
})
//a-signin
...
$('.a-signin').click(function(){
    if (cuunt <= 0) {
     ...
     $.post('http://srp-phone/abdulSignIn', JSON.stringify({status: taxiOn}));
 })
...
$(".house-entries, .access-house-entries").on('click', '.h-unlock', function (e) {
    ...
    $.post('http://srp-phone/unlockHouses', JSON.stringify({hid: overid, penisfuck: hstatus}));
 })
 ...
 $(".house-entries").on('click', '.h-edit-s', function (e) {
    ...
    $("#house-keys").hide();
    $("#house-2").hide();
    $.post('http://srp-phone/editHousingOwn', JSON.stringify({}));
 })
 ...
 $(".house-entries").on('click', '.h-access-keys', function (e) {
    ...
    $("#house-2").hide();
    $('.access-house-entries').css('display', 'none');
    $.post('http://srp-phone/btnKeyHolders', JSON.stringify({
        hid: overid
    }));
 ...
 $('.h-leaveedit').click(function(){
    editingHouse = false;
    $.post('http://srp-phone/leaveEditMode', JSON.stringify({}))
    $("#house-2").show();
    $("#house-3").hide();
    $("#house-keys").hide();
    $(".btn-2").show();
 ...
 $('.h-openfurniture').click(function(){
    $.post('http://srp-phone/openFurnitureUI', JSON.stringify({}))
 })

 $('.h-setbackdoor').click(function(){
    $.post('http://srp-phone/setBackdoorUI', JSON.stringify({}))
 })

 $('.h-setwardrobe').click(function(){
    $.post('http://srp-phone/setWardrobeUI', JSON.stringify({}))
 })

 $('.h-setcrafting').click(function(){
    $.post('http://srp-phone/setCraftingUI', JSON.stringify({}))
 })

 $('.h-setgarage').click(function(){
    $.post('http://srp-phone/setGarageUI', JSON.stringify({}))
 })

 $('.h-setstash').click(function(){
    $.post('http://srp-phone/setStashUI', JSON.stringify({}))
 })
 ...
 $('.submit-h-acess').click(function(){
    ...
    $('.phone-acces-house').hide()
    $.post('http://srp-phone/giveAccess', JSON.stringify({
        hid: hid,
        cid: cid
    }));
})
    ...
    $(document).ready(function () {
        $('.submit-settings').click(function(){
            ...
            $('.settings').hide()
                ...
                complateInput();
                $.post('http://srp-phone/submitSettings', JSON.stringify({
                    bg: bg,
                    model: model,
                    twat: twat,
                    call: call
...
function addDebtCars(vehData){ 
    ...
    $('.c-pawynow-button').click(function(){
        ...
        $.post("https://srp-phone/car_paid", JSON.stringify({
            plate:plate,
            payment:payment
        }))
        complateInputJustGreen();
        setTimeout(() => {
            $.post("https://srp-phone/btnDebt", JSON.stringify({}))
        }, 2000);
   })
