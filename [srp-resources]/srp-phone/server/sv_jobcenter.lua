...
RPC.register("srp-phone:getIdleGroup", function(source)
    local src = source
    local idle = {}
...
RPC.register("srp-phone:getMyGroup", function(source)
    local src = source
    local idle = {}
    local g = MySQL.query.await([[
        SELECT * FROM job_group
...
RPC.register("srp-phone:c_group", function(source)
    local src = source
...
RPC.register("srp-phone:j_group", function(source, id)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
RPC.register("srp-phone:get_groupsMem", function(source, id)
...
RPC.register('srp-phone:g_ready', function(source,stat,id)
    local src = source
    local status = stat
    local gid = id
...
RPC.register('srp-phone:leave_group', function(source,id)
    local src = source
    local gId = id
...
--         end
--       end
--     --   TriggerLatentClientEvent('mhsrp-housing:collectHouses', -1, 30000, Houses)
--     end)
-- end)