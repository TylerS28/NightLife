...
RPC.register("srp-phone:addToCurrentBanks", function(pSource, pBank)
...
RPC.register("srp-phone:getCurrentBanks", function(pSource)
    return currentBanks
end)

RPC.register("srp-phone:requestPlayerToGroup", function(pSource, pCid, pGroupID)
...
RPC.register("srp-phone:addPlayerToGroup", function(pSource, pCid, pGroupID, pSenderID)
...
RPC.register('srp-phone:removePlayerFromGroup', function(pSource, pCid, pGroupID)
...
RPC.register("srp-phone:createGroup", function(pSource)
...
RPC.register('srp-phone:deleteGroup', function(pSource, pGroupID)
    local jsonDecodedGroups = json.decode(groupTable)
    table.remove(jsonDecodedGroups, pGroupID)
    groupTable = json.encode(jsonDecodedGroups)
end)

RPC.register('srp-phone:getMyGroup', function(pSource, pCid)
    local jsonDecodedGroups = json.decode(groupTable)
    for k,v in pairs(jsonDecodedGroups) do
        local jsonDecodedGroupMembers = json.decode(v.members)
        for j,s in pairs(jsonDecodedGroupMembers) do
