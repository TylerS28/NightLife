RegisterNetEvent('srp-phone:transferWenmo', function(num,amount,comm)
    local src = source
    local zcid = getIdentifierByPhoneNumber(num)
    local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(src))
...
RPC.register('srp-phone:getWenmo', function(source)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(src))
