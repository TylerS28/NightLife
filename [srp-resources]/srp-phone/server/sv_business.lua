...
RegisterNetEvent('srp-phone:fireEmp')
AddEventHandler('srp-phone:fireEmp', function(id, job, name)
  local src = source
...
RPC.register("srp-phone:sendChargeCustomerNotif", function(pSource, BusinessName, GroupID, stateID, amount, reason)
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local tax = exports['srp-business']:taxTotal(amount)
    ...
    TriggerClientEvent("srp-phone:chargeCustomerNotif", tSrc, { 
...
RPC.register("srp-phone:chargeCustomerBus", function(pSource, amount, GroupID, stateID, sID)
    local amtTax = amount
    local tSrc = getPlayerFromCidForCharge(tonumber(stateID))
    local user = exports['srp-fw']:getModule("Player"):GetUser(tSrc)
    local char = user:getCurrentCharacter()
    ...
    if bank >= tonumber(amtTax) then
        ...
        TriggerClientEvent("srp-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They couldn't accept the payment")
        TriggerClientEvent("srp-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Not enough money to complete the payment.")
        return
    end
    TriggerClientEvent("srp-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "Payment Successful.")
    TriggerClientEvent("srp-phone:businessRegisterNoti", tSrc, "SERVICE CHARGE", "You have been deducted " .. amtTax .. " from you're bank account")
    TriggerClientEvent("srp-phone:businessRegisterNoti", sID, "SERVICE CHARGE", "They accepted the payment")
    return
end)
...
RegisterServerEvent('srp-phone:business_hired', function(StateID, Rank, pBusiness)
...
RegisterServerEvent('srp-phone:business_edit', function(pRank, pStateID, pPassType)
...
RPC.register('srp-phone:business_paycheck', function(source,bus,p,a)
    local src = source
    local bId = bus
    local cid = p
    local amount = a
...
function getPlayerFromCid(zcid, amountAdded)
    for _, player in ipairs(GetPlayers()) do
        local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(player))
...
RPC.register("srp-business:payExternal", function(pSource, pCid, pAmount, pBusinessID)
...
RPC.register('srp-phone:bus_addBank', function(source,id,a)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local ply = user:getCurrentCharacter()
    local bId = id
...
RegisterServerEvent('srp-phone:business_fire', function(pBusinessName, pStateID)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
