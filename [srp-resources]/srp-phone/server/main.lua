...
RPC.register("srp-phone:getPhoneNumber", function(pSource)
...
function getSIDFromStateID(cid)
    for k,v in pairs(GetPlayers()) do
        local user = exports['srp-fw']:getModule("Player"):GetUser(tonumber(v))
        local char = user:getCurrentCharacter()
...
RPC.register("srp-phone:sendThePM", function(src, msg)
...
RPC.register("acceptBusinessCharge", function(pSource, pBID, pAmt, pCID)
    ...
    local user = exports['srp-fw']:getModule("Player"):GetUser(SID)
    local char = user:getCurrentCharacter()
...
RegisterNetEvent("srp-phone:h-Lock:sv", function(hid, status)
    print("HELLO? " .. " " .. hid .. " " ..status)
    exports.oxmysql:execute("UPDATE housing SET status = ? WHERE hid = ?",{status,hid})
end)

RPC.register("srp-financials:getCurrentBank", function(pSource)
    local user = exports['srp-fw']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("srp-apartments:getCurrentApartment", function(src)
...
function getOrGeneratePhoneNumber (sourcePlayer, identifier, cb)
  ...
  local user = exports["srp-fw"]:getModule("Player"):GetUser(sourcePlayer)
...
AddEventHandler('racing-join-race', function(identifier)
  local src = source
  local player = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
--     local src = source
--     local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(id))
--     local char = user:getCurrentCharacter()
...
RegisterServerEvent("srp-phone:grabWallpaper")
AddEventHandler("srp-phone:grabWallpaper", function()
  ...
  local cid = char.id
    exports.oxmysql:execute("SELECT `wallpaper` FROM characters WHERE id = @id",{['id'] = cid}, function(data)
      if data[1].wallpaper ~= nil then
          TriggerClientEvent('srp-phone:grabBackground', src, data[1].wallpaper)
        --   print('Grabbbing',data[1].wallpaper)
      else
          TriggerClientEvent('srp-phone:grabBackground', src, "https://i.imgur.com/JVPavDg.jpg")
        --   print('Not Grabbbing')
      end
    end)
end)

RegisterServerEvent("srp-phone:GiveCrypto")
AddEventHandler("srp-phone:GiveCrypto", function(type,amount)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:getChips", function(pSource)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:getGroupSanitation", function(pSource)
    local src = source
    -- local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    -- local char = user:getVar("character")
    -- local id = char.id
...
RPC.register("srp-phone:incomingCrypto", function(pSource)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
RPC.register("srp-phone:blueprintid", function(pSource)
    return BPid
end)

RPC.register("srp-phone:exchangeCrypto", function(pSource,type,amount)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:transferCrypto", function(pSource,type,amount,target)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:GetShungite", function (pSource)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:GetTgb", function (pSource)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:GetDvd", function (pSource)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
    local id = char.id
...
RPC.register("srp-phone:buyCrypto", function (pSource,ctype,amount)
    local src = source
    local amount = amount
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getVar("character")
...
RPC.register("srp-phone:purchaseDarkMarket", function (pSource,type,price)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local id = char.id
...
RPC.register('srp-phone:getMyGroup', function(cid)
...
RPC.register('srp-phone:getHouses', function(src)
...
RPC.register('srp-phone:apt', function(source)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
RPC.register('srp-phone:getCurrentOwned', function(source)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local house
...
RegisterNetEvent('srp-phone:house:status')
AddEventHandler('srp-phone:house:status', function(hid,status)
     local src = source
...
RPC.register("srp-housing:getKeyHolderz", function(src,hid)
    print(src,hid)
    local myKeyz = {}
...
RPC.register("srp-housing:removeKey:phone", function(src,cid,hid)
    local keys = MySQL.query.await([[
...
RPC.register('srp-phone:getHousePrice', function(source,hid)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local hIds = hid
...
RPC.register('srp-phone:GiveAccess', function(source,hid,cid)
    local src = source
    local hIds = hid
    local cid = cid
    local access
...
-- RPC.register('srp-phone:getAccessHouse', function(source)
--     local src = source
--     local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local cid = user:getCurrentCharacter().id
--     local house
...
RPC.register('srp-phone:getAccessHouse', function(source,house)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register('srp-phone:getAccessHouse_2', function(house)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local cid = user:getCurrentCharacter().id
    local hid = house
...
RPC.register("srp-phone:addDOJ", function(pSource, pJob)
    local user = exports['srp-fw']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("srp-phone:removeDOJ", function(pSource, pJob)
    local user = exports['srp-fw']:getModule("Player"):GetUser(pSource)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("srp-phone:getDOJ", function(src)
    return availableDOJ
end)

local availableDrivers = {}

RPC.register("srp-phone:addAbdulTaxi", function(src)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
    local phonenumber = char.phone_number
...
RPC.register("srp-phone:getAbdulTaxies", function(src)
    return availableDrivers
end)

RegisterNetEvent("srp-phone:getAbdulTaxies", function()
    TriggerClientEvent("srp-phone:manualUpdateAbdulTaxies", source)
end)

RPC.register("srp-phone:removeAbdulTaxi", function(src)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local cid = char.id
...
RPC.register("srp-phone:removeKeys", function(src, pHouseId, pPlayerId)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
-- RPC.register("srp-garages:states",function(pSource,state,plate,curGarage,fuel)
--     local src = source
--     local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RPC.register("srp-phone:Groups", function(source,gIp)
--     local src = source
...
RegisterServerEvent('srp-phone:sendVehBill')
AddEventHandler('srp-phone:sendVehBill', function(paypal,plate,amount)
    src = source
    TriggerClientEvent('srp-phone:sellCar', paypal, plate, amount,src)
end)


RPC.register('srp-phone:sellVehicle', function(pSource,pl,py,am)
    local src = pSource
    local plate = pl
    local paypal = py
    local amount = am
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["srp-fw"]:getModule("Player"):GetUser(paypal)
    local ply = user:getCurrentCharacter()
...
RegisterServerEvent('srp-phone:purchasedCar')
AddEventHandler('srp-phone:purchasedCar', function(plate,amount,sellerID)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local char = user:getCurrentCharacter()
    local player = exports["srp-fw"]:getModule("Player"):GetUser(sellerID)
...
RegisterNetEvent('srp-phone:send_gas_bill')
AddEventHandler('srp-phone:send_gas_bill', function(targetId, pFuelCost)
    local pSrc = source
    
	if targetId ~= nil then
        TriggerClientEvent('srp-hud:send_fuel_bill', targetId, pFuelCost, pSrc)
    end
end)
