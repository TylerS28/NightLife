...
AddEventHandler('Tweet', function(handle, data, time)
  ...
  TriggerClientEvent('srp-phone:insertTwat', -1, handle,data.text,time,data.attachment)
  -- TriggerClientEvent('Client:UpdateTweet1', src, newtwat,allTweet)
 
end)

RegisterNetEvent('srp-phone:twatSendNotification')
AddEventHandler('srp-phone:twatSendNotification', function(handle, data, time)

  local newtwat = { ['handle'] = handle, ['message'] = data, ['time'] = time}
  TriggerClientEvent('srp-phone:TwatNotify', -1, newtwat)
end)

RegisterNetEvent('srp-phone:openGetTweets')
AddEventHandler('srp-phone:openGetTweets', function()
  local src = source
  exports.oxmysql:execute('SELECT * FROM (SELECT * FROM tweets ORDER BY `time` DESC LIMIT 50) sub ORDER BY time ASC', {}, function(tweets) -- Get most recent 100 tweets
...
RPC.register('srp-phone:getTwatters', function(source)
...
RPC.register("srp-phone:twitterhandle", function(pSource)
  local src = source
  local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
