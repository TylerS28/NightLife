...
local documents = {}
RPC.register('srp-phone:getDocuments', function(pSource, pType)
...
RPC.register("srp-phone:sendDocumentEvent", function(pSource, pTarget, pDocumentID)
    TriggerClientEvent("srp-phone:sendDocumentEvent", pTarget, pDocumentID)
end)

RegisterNetEvent("srp-phone:checkForNonDocumentedLicenses", function(pCid)
    local sex = "sex"
...
RPC.register("srp-phone:shareDocumentPermanent", function(pSource, dID, tCID)
...
RPC.register("srp-phone:acceptDocumentReceiveRequest", function(pSource, pDocumentID)
    local doc = MySQL.query.await([[
        SELECT * FROM __documents WHERE id = ?
    ]], {pDocumentID})
...
RPC.register('srp-phone:deleteDocument', function(pSource, pId)
    exports.oxmysql:execute("DELETE FROM __documents WHERE id = ?",{pId})
end)

RPC.register('srp-phone:updateDocument', function(pSource, pId, pTitle, pText)
...
RPC.register("srp-phone:createDocument", function(pSource, pOwner, pTitle, pText, pType)
    MySQL.query.await([[
