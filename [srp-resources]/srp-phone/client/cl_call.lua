...
RegisterNUICallback('getCallHistory', function()
  local recentCallHistory = RPC.execute("srp-phone:getRecentCallsByCid", exports['isPed']:isPed("cid"))
  SendNUIMessage({
...
Citizen.CreateThread(function()
  while true do
    Wait(10000)
    TriggerServerEvent("srp-phone:sendRecentCalls", recentcalls, exports['isPed']:isPed("cid"))
  end
end)
...
AddEventHandler('phone:otherClientEndCall', function()
  ...
  Citizen.CreateThread(function()
    exports["srp-keybinds"]:registerKeyMapping("", "Player", "Hangup Call", "+hangup", "-hangup", "5")
    exports["srp-keybinds"]:registerKeyMapping("", "Player", "Answer Call", "+answer", "-answer", "6")
  end)
...
RegisterNetEvent('srp-phone:RemoveCall')
AddEventHandler('srp-phone:RemoveCall', function()
    SendNUIMessage({
      openSection = 'callnotifyEnd'
    })
    SendNUIMessage({
...
function endCall()
  ...
  mySourceID = 0
  TriggerEvent('phone:setCallState', isNotInCall)
  TriggerEvent('srp-phone:RemoveCall')
  onhold = false
  mySourceHoldStatus = false
  callid = 0
end
