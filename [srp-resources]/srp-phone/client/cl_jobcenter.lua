...
RegisterNUICallback('job-center', function(data, cb)
  -- local idle = RPC.execute("srp-phone:getIdleGroup")
  -- local myG = RPC.execute("srp-phone:getMyGroup")
  -- local data, members = RPC.execute("srp-phone:getGroupSanitation")
  -- -- print(json.encode(data),data)
  -- local myGroup = RPC.execute('srp-phone:getMyGroup',exports['isPed']:isPed('cid'))
  -- local members = RPC.execute("srp-phone:get_groupsMem")
...
RegisterNUICallback('c_group', function()
  local create = RPC.execute("srp-phone:c_group")
end)

RegisterNUICallback('j_group', function(data)
  local gId = data.gId
  print("Group ID:",gId)
  RPC.execute("srp-phone:j_group", gId)
end)
...
RegisterNUICallback('group_status', function(data)
  ...
  status = data.status
  local gId = data.gid
  RPC.execute("srp-phone:g_ready",status,id)
end)

RegisterNUICallback('leave_group', function(data)
  local gId = data.gid
  RPC.execute('srp-phone:leave_group', gId)
end)
