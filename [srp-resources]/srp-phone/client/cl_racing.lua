...
RegisterNUICallback('racing:event:start', function(data)
  local canJoinOrErr = exports["srp-racing"]:canJoinOrStartRace()
  if canJoinOrErr ~= true then
    exports["srp-phone"]:phoneNotification("racen",canJoinOrErr,"RACING", 3000)
    return
  end
  print(data.id, json.encode(data))
    exports["srp-racing"]:createPendingRace(data.id, data)
    Wait(100)
    SendNUIMessage({
...
RegisterNUICallback("racing:event:join", function(data)
  ...
  exports["srp-racing"]:joinRace(id, alias, cid)
  Wait(100)
  SendNUIMessage({
    openSection = "racing:event:update",
    canJoin = false,
...
RegisterNetEvent("srp-racing:removedActiveRaced")
AddEventHandler("srp-racing:removedActiveRaced", function(id)
  -- print("PUTANG INA")
    openRacingApp()
...
function openRacingApp()
  ...
  local alias = RPC.execute("GetAlias")
  print(alias)
  local _races = exports["srp-racing"]:getAllRaces()
...
AddEventHandler("srp-racing:api:updatedState", function()
...
RegisterNUICallback("racing:map:save", function(data)
  if creating then
      ...
      if veh ~= 0 and GetPedInVehicleSeat(veh, -1) == ped then
          ...
          TriggerEvent("srp-racing:cmd:racecreate", options)
          closeGui()
...
RegisterNUICallback('racing:map:cancel', function()
  creating = false
  TriggerEvent('srp-racing:cmd:racecreatecancel')
  openRacingApp()
end)
...
RegisterNUICallback('srp-phone:racing:setGPS', function(data)
  local mapID = data.mID
  exports["srp-racing"]:locateRace(mapID)
  phoneNotification("racen","GPS Set to your map.","RACING")
end)

RegisterNUICallback('srp-phone:racing:seePreview', function(data)
  local mapID = data.mID
  if not mapID then return end
  id = tostring(mapID)
  exports["srp-racing"]:previewRace(mapID)
...
RegisterNUICallback('srp-phone:r-users', function(data, cb)
  ...
  if race ~= nil then
    ...
    local racer = RPC.execute('srp-phone:racing:getRacer', raceID)
    cb(racer)
  end
  print(json.encode(newPendingRace[raceID]))
end)

AddEventHandler("srp-racing:api:startingRace", function(startTime)
  TriggerEvent("DoLongHudText", "Starting race in " .. tostring(startTime / 1000) .. " seconds")
end)

AddEventHandler("srp-racing:api:playerJoinedYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " joined your race")
end)

AddEventHandler("srp-racing:api:playerLeftYourRace", function(characterId, name)
  TriggerEvent("chatMessage", "", {255, 0, 0}, "^1" .. name .. " left your race")
end)

RegisterNetEvent('srp-racing:pendingRacenotif')
AddEventHandler("srp-racing:pendingRacenotif", function(race)
  ...
  exports["srp-phone"]:phoneNotification("racen","Pending race available...","From the PM", 12000)
end)
-- Citizen.CreateThread(function()
--   while true do
--     Wait(5000)
