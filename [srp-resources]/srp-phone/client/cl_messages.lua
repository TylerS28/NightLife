...
function loadSMS(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("srp-phone:getSMS",cid)
  local _messages = {}
...
function loadSMSBurner(cid)
  print("LOAD SMS")
  local messages, phone = RPC.execute("srp-phone:getTextsFromNumber",cid)
  local _messages = {}
  for i, v in ipairs(messages) do
      local name = ""
...
local data = message
local app = "sms"
if exports["srp-phone"]:phasPhone() then
  if exports['srp-phone']:pNotify() == true then
