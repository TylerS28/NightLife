...
RegisterNetEvent('srp-fw:playerSpawned')
AddEventHandler('srp-fw:playerSpawned', function()
  Wait(1000)
...
RegisterCommand('renten', function()
  TriggerEvent('srp-housing:rent_activate_cl')
end)

RegisterNetEvent('srp-housing:rent_activate_cl')
AddEventHandler('srp-housing:rent_activate_cl', function()
...
AddEventHandler('srp-phone:sanitationIn', function()
  -- print("SANITATION SIGN IN")
  sanitation = true
end)

RegisterNetEvent("srp-phone:grabBackground")
AddEventHandler("srp-phone:grabBackground", function(link)
    wallPaper = link
end)
...
RegisterNUICallback('wallpaper', function(data, cb)
  ...
  TriggerEvent('srp-phone:grabBackground', wallPaperSelecionado)
...
AddEventHandler('phone:reset', function(cidsent)
    ...
    wallPaper = ""
    Wait(1)
    TriggerServerEvent('srp-phone:grabWallpaper')
end)

RegisterNUICallback('sendThePM', function(data, cb)
  local thePM = data.thePM
  RPC.execute("srp-phone:sendThePM", thePM)
end)

RegisterCommand("fuckmeintheasdasvf", function()
  RPC.execute("srp-phone:sendThePM", 'f')
end)
...
RegisterNUICallback('submitSettings', function(data, cb)
  ...
  Wait(5)
  if bg ~= "" then
    TriggerEvent('srp-phone:grabBackground', bg)
...
RegisterNetEvent("srp-phone:setSomeShit", function(bg, model, twatNotify, callNotify)
  SetResourceKvp(exports['isPed']:isPed('cid').."-bg", bg)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-pmodel", model)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ntwat", twatNotify)
  SetResourceKvpInt(exports['isPed']:isPed('cid').."-ncall", callNotify)
...
RegisterNUICallback('btnHousing', function()
  ...
  allNewHouses = {}
  myAccessHouse = {}
  --local apt = RPC.execute("srp-phone:apt")
  local housing = exports['srp-housing']:housingForPhone()
  local apartment = exports['srp-apartments']:getMyCurrentApartment()
  myHousess = RPC.execute("srp-phone:getCurrentOwned")
  accessHouse = RPC.execute('srp-phone:getAccessHouse_2')
  ...
  for i,v in pairs(myHousess) do
    ...
    table.insert(newHouses, {
      ...
      cid = v.cid,
      status = v.status,
      cat = exports['srp-housing']:getHousingCatFromPropertID(v.hid),
    })
  end
  for l,s in pairs(accessHouse) do
      table.insert(myAccessHouse, {
        ...
        cid = s.cid,
        status = s.status,
        cat = exports['srp-housing']:getHousingCatFromPropertID(s.house),
      })
  end
  SendNUIMessage({
    openSection = "housing",
...
RegisterNUICallback('btnKeyHolders', function(data, cb)
  local myKittens = {}
  print(data.hid)
  local keys = RPC.execute('srp-housing:getKeyHolderz', data.hid)
  SendNUIMessage({
...
RegisterNUICallback('whatthefuckdoesthisshitwork', function(data,cb)
  local cid = data.cid
  local hid = data.hid
  local success,msg = RPC.execute('srp-housing:removeKey:phone',cid, hid)
  if success then
...
RegisterNUICallback('OpenCamera', function(data,cb)
  RPC.execute('srp-phone:openCamera', data.cameraId)
end)
...
--   while true do
--     Wait(3000)
--     local myHouse = RPC.execute("srp-phone:getCurrentOwned")
...
RegisterInterfaceCallback("srp-phone:signInHandler", function(data, cb)
  if data.key.sign_in then
    RPC.execute("srp-phone:addDOJ", data.key.job)
  else
    RPC.execute("srp-phone:removeDOJ", data.key.job)
  end
...
RegisterNUICallback('getActiveDOJ', function(data,cb)
      SendNUIMessage({
        ...
        penisinmybuttholefuckshit = RPC.execute("srp-phone:getDOJ")
      })
end)
...
RegisterNUICallback('abdulSignIn', function(data,cb)
    ...
    if taxiStatus then
      RPC.execute("srp-phone:addAbdulTaxi")
      addAbdulTaxi()
    ...
    elseif not taxiStatus then
      RPC.execute("srp-phone:removeAbdulTaxi")
      removeAbdulTaxi()
      local availableDrivers = getAbdulTaxi()
      SendNUIMessage({
        openSection = "addTaxi",
...
RegisterNetEvent("srp-phone:manualUpdateAbdulTaxies", function(availableDrivers)
  addAbdulTaxi()
  local availableDrivers = getAbdulTaxi()
  SendNUIMessage({
    openSection = "addTaxi",
...
function addAbdulTaxi () 
  currenttable = {}
  local table = RPC.execute("srp-phone:getAbdulTaxies")
--  local currenttable = {}
...
RegisterNUICallback('btnGarage', function()
  local LocalPlayer = exports["srp-fw"]:getModule("LocalPlayer")
...
RegisterNUICallback('sellVeh', function(data)
  ...
  TriggerServerEvent('srp-phone:sendVehBill', data.paypal, data.plate, data.amount)
end)

RegisterNetEvent('srp-phone:sellCar')
AddEventHandler('srp-phone:sellCar', function(plate,amount,sId)
  sellCarPlate = plate
...
RegisterNUICallback('purchaseCar', function(data)
  TriggerServerEvent('srp-phone:purchasedCar', sellCarPlate, data.amount,data.senderID)
end)

RegisterNUICallback('purchase_gas', function(data)
  TriggerEvent('srp-fuel:OfferAccepted')
end)

RegisterNetEvent('srp-hud:send_fuel_bill', function(pCost)
...
function SpawnVehicle(vehicle, x,y,z, Fuel, customized, plate, IsViewing, enginehealth, bodyhealth)
  if DoesEntityExist(spawningVeh) then
	...
	Citizen.CreateThread(function()			
        ...
        exports['srp-vehicles']:SetVehicleIdentifier(spawningVeh, vid)
        ...
        if not IsViewing then    
            ...
            RPC.execute("srp-garages:states", "Out", plate, exports['srp-menu']:currentGarage(), pUpdatedFuel)
        end
    end)
end
...
RegisterNUICallback('sethGps', function(data)
 local house = exports["srp-housing"]:retrieveHousingTableMapped()
...
RegisterNUICallback('unlockHouses', function(data)
  ...
  if statuus == "Unlock" then
    --TriggerEvent('srp-phone:h-Lock', hId,"Lock")
  else
    --TriggerEvent('srp-phone:h-Lock', hId,"Unlock")
  end
  TriggerServerEvent('srp-phone:house:status',hId,statuus)
  Wait(200)
  TriggerEvent('updateUIHousing')
end)

RegisterNetEvent("srp-phone:h-Lock", function(hId, status)
  TriggerServerEvent("srp-phone:h-Lock:sv", hId, status)
...
RegisterNUICallback('editHousingOwn', function()
  -- if exports['srp-housing']:isNearProperty() then
    if exports["srp-housing"]:canEdit() then
      print("HELLO!")
      local isComplete, _propertyID, dist, zone = exports['srp-housing']:findClosetedPerson()
      TriggerEvent("srp-housing:editOptions",{action = "enter", propertyId = _propertyID})
    else
      SendNUIMessage({
...
RegisterNUICallback('sellOwnHouse', function(data)
  local price = RPC.execute('srp-phone:getHousePrice', data.hid)
end)

RegisterNUICallback('giveAccess', function(data)
  local complete = RPC.execute('srp-phone:GiveAccess', data.hid,data.cid)
...
RegisterNUICallback('houseAccess', function(data)
  ...
  local access = RPC.execute('srp-phone:getAccessHouse', data.hid)
  local data = {
      {
        title = "Close",
        action = "srp-housing:close_access",
        key = {},
    },
  }
  local player = {}
  for i,k in pairs(access) do
    data[#data + 1] = {
          ...
          key = {"lockdown"},
          -- action = "srp-ui:apartmentsContext"
          children = {
            {
                title = "Remove Key",
  ...
  exports["srp-contexts"]:showContext(data)
end)

AddEventHandler("srp-housing:close_access", function()
  exports["srp-contexts"]:hideContext()
end)

AddEventHandler('srp-ui:apartmentsContext', function(data)
  RPC.execute("srp-phone:removeKeys",data[2],data[1])
  exports["srp-ui"]:hideContext(data)
end)
...
function hasDecrypt2()
    if exports["srp-inventory"]:hasEnoughOfItem("vpnxj",1,false,true) then
      return true
    else
      return false
    end
...
function hasTrucker()
    if exports["srp-fw"]:getModule("LocalPlayer"):getVar("job") == "trucker"  then
      return true
    else
      return false
    end
...
function hasPhone()
    if
      ...
      exports["srp-inventory"]:hasEnoughOfItem("assphone",1,false) or
...
function hasRadio()
    if (exports["srp-inventory"]:hasEnoughOfItem("radio",1,false) or exports["srp-inventory"]:hasEnoughOfItem("pdradio",1,false)) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGui(isBurner, cid)
   ...
   TriggerEvent("srp-spawn:phonerefresh") --Refreshes jobs
  ...
  if hasPhone() then
    ...
    local near = exports['srp-housing']:isNearProperty()
    local house = RPC.execute("srp-phone:getHouses")
    ...
    SendNUIMessage({
      ...
      hasDevice = device, 
      hasDecrypt = decrypt, 
      pNum = RPC.execute("srp-phone:getPhoneNumber"),
...
function canCreateRace()
  local racingCreateUsbItem = exports["srp-inventory"]:GetInfoForFirstItemOfName("racingusb0")
...
function canOpenRaceApp()
  local racingUsbItem = exports["srp-inventory"]:GetInfoForFirstItemOfName("racingusb2")
...
function closeGui()
  ...
  -- TriggerEvent('srp-phone:UpdateStatePhone')
  pPhoneOpen = false
  insideDelivers = false
end

-- RegisterNetEvent('srp-phone:UpdateStatePhone')
-- AddEventHandler('srp-phone:UpdateStatePhone', function()
--     Wait(5)
...
RegisterNUICallback('updateMyWallpaper', function(data, cb)
  wallPaper = ""
  Wait(5)
  TriggerEvent('srp-phone:grabBackground', data.name .."?auto=compress&cs=tinysrgb&h=350")
...
AddEventHandler('phone:casino_paid', function(message)
  if exports['srp-phone']:pOpen() == false then 
    SendNUIMessage({openSection = "emailnotify", pEHandle = 'Payment Received.', pEMessages = message})
    SendNUIMessage({openSection = "phonemedio", timeout = "5200", pOpen = exports['srp-phone']:pOpen()}) 
  end
...
RegisterNetEvent("phone:sendThePM", function(name, msg)
  if exports["srp-inventory"]:hasEnoughOfItem("thepmdongle",1,false,true) then
...
RegisterNetEvent('srp-spawn:confirm_spawned')
AddEventHandler('srp-spawn:confirm_spawned', function()
  serverSpawn = true
end)
...
RegisterNUICallback('jobcenter-setgps', function(data)	
  ...
  if data.item == "oxy" then	
    TriggerServerEvent("srp-deliveries:JobCenter")	
...
RegisterCommand('house', function()
  local isComplete, propertyID, dist, zone = exports['srp-housing']:findClosetedPerson()
  print("HOUSING",isComplete, propertyID, dist, zone)
end)
RegisterNUICallback('checkHouse', function()
  
 -- if exports["srp-housing"]:canEdit() then
    TriggerEvent('srp-housing:phone:check')
 -- else
 --   SendNUIMessage({
 --     openSection = "noProperty"
 --   })
...
RegisterNUICallback('leaveEditMode', function()
  print("LEAVING EDIT")
  TriggerEvent('srp-housing:editOptions', { action = "exit" })
end)
...
RegisterNUICallback('openFurnitureUI', function()
  ...
  TriggerEvent('srp-housing:editOptions', { action = "furniture" })
end)

RegisterNUICallback('setBackdoorUI', function()
  print("SET BACKDOOR")
  TriggerEvent('srp-housing:editOptions', { action = "backdoor" })
end)

RegisterNUICallback('setWardrobeUI', function()
  print("SET WARDROBE")
  TriggerEvent('srp-housing:editOptions', { action = "wardrobe" })
end)

RegisterNUICallback('setCraftingUI', function()
  print("SET CRAFTING")
  TriggerEvent('srp-housing:editOptions', { action = "crafting" })
end)

RegisterNUICallback('setGarageUI', function()
  print("SET GARAGE")
  TriggerEvent('srp-housing:editOptions', { action = "garage" })
end)

RegisterNUICallback('setStashUI', function()
  print("SET STASH")
  TriggerEvent('srp-housing:editOptions', { action = "stash" })
end)

RegisterNUICallback('sellsHouse', function()
  print("EDITING")
  TriggerEvent('srp-housing:phone:check',"edit")
end)

AddEventHandler('srp-phone:sentHouse', function(hasPrice,id,street,price,category)
...
AddEventHandler('srp-phone:editingHouse', function(id,street,price)
  print("EDITING HERE")
  SendNUIMessage({
...
RegisterNUICallback('purchaseHouse', function(data)
  ...
  if rentEnabled then
      print("can purchase")
   --   TriggerEvent('srp-phone:sentHouse')
      local success, msg = RPC.execute("srp-housing:buy", data.hid, data.price, 0, exports['isPed']:isPed("fullname"))
      TriggerEvent("DoLongHudText",msg,1)
     return
  end
...
-- AddEventHandler('srp-phone:noProperty', function()
...
RegisterNUICallback('editHousePrice', function(data)
  TriggerServerEvent('srp-housing:editPrice', data.hid,data.price)
end)

AddEventHandler('srp-phone:EnableSellRent', function()
    exports['srp-ui']:openApplication('textbox', {
    callbackUrl = 'srp-phone:SellRentEnable',
...
RegisterInterfaceCallback("srp-phone:SellRentEnable", function(data, cb)
  cb({ data = {}, meta = { ok = true, message = '' } })
  print('THIS IS ENABLED RENT/SALE FOR PLAYER', data[1].value,json.encode(data))
  TriggerServerEvent('srp-housing:rent_activate_sv', data[1].value)
  exports["srp-ui"]:hideContextMenu()
  cb({ data = {}, meta = { ok = true, message = '' } })
end)
...
RegisterNetEvent('srp-phone:signInInfoTowing')
AddEventHandler('srp-phone:signInInfoTowing', function()
  phoneNotification("email", "Grab the towtruck parked in the bay.", "Job Center")
end)

RegisterNetEvent('srp-phone:towing_offer')
AddEventHandler('srp-phone:towing_offer', function()
...
RegisterNUICallback('accept_tow_job', function(data)
  TriggerEvent('srp-jobs:towing_accept')
end)

-- 24/7 Delivery --

RegisterNetEvent('srp-phone:signInInfoDelivery')
AddEventHandler('srp-phone:signInInfoDelivery', function()
  phoneNotification("email", "Signed in as 24/7 Delivery Driver.", "Job Center")
end)

RegisterNetEvent('srp-phone:delivery_offer')
AddEventHandler('srp-phone:delivery_offer', function()
...
RegisterNUICallback('accept_delivery_job', function(data)
    TriggerEvent('srp-jobs:deliveries:getCar')
end)

-- Dodo Logistics --

RegisterNetEvent('srp-phone:signInDodo')
AddEventHandler('srp-phone:signInDodo', function()
  phoneNotification("email", "Signed in as a Dodo Logistics worker.", "Job Center")
end)

RegisterNetEvent('srp-phone:dodoJobOffer')
AddEventHandler('srp-phone:dodoJobOffer', function()
...
RegisterNUICallback('accept_dodo_job', function(data)
    TriggerEvent('srp-jobs:spawn-truck')
end)

RegisterNetEvent('srp-phone:goToLocation')
AddEventHandler('srp-phone:goToLocation', function()
  phoneNotification("email", "Go to the delivery location on the GPS.", "Job Center")
end)

RegisterNetEvent('srp-phone:goBackDepo')
AddEventHandler('srp-phone:goBackDepo', function()
  phoneNotification("email", "Return back to the depo.", "Job Center")
end)

-- House Robberies --

RegisterNetEvent('srp-phone:signInInfoHouseRobbery')
AddEventHandler('srp-phone:signInInfoHouseRobbery', function()
  phoneNotification("email", "Signed in as Criminal.", "Job Center")
end)

RegisterNetEvent('srp-phone:house_rob_offer')
AddEventHandler('srp-phone:house_rob_offer', function()
...
RegisterNUICallback('accept_house_robbery', function(data)
    TriggerEvent('srp-activities:acceptJob')
end)

-- Chopshop --

RegisterNetEvent('srp-phone:signInInfoChopshop')
AddEventHandler('srp-phone:signInInfoChopshop', function()
  phoneNotification("email", "Signed in as Chopshop.", "Job Center")
end)

RegisterNetEvent('srp-phone:chop_shop_offer')
AddEventHandler('srp-phone:chop_shop_offer', function()
...
-- Oxy Runs --
RegisterNetEvent('srp-phone:oxyRunNoti')
AddEventHandler('srp-phone:oxyRunNoti', function(pText)
  phoneNotification("email", pText, "Job Center")
end)

RegisterNetEvent('srp-phone:oxyrun:offer')
AddEventHandler('srp-phone:oxyrun:offer', function()
...
RegisterNUICallback('accept_oxy_run', function(data)
    TriggerEvent('srp-oxy:Accept:Offer')
end)

-- Paycheck --

RegisterNetEvent('srp-phone:paymentNoti')
AddEventHandler('srp-phone:paymentNoti', function(pAmt, pType)
  phoneNotification("email", "$"..pAmt..".00 has been added to your "..pType.. " Balance", "Job Center")
end)

RegisterNetEvent('srp-phone:businessRegisterNoti')
AddEventHandler('srp-phone:businessRegisterNoti', function(pBusinessName, pText)
  phoneNotification("charge", pText, pBusinessName)
end)

-- Fishing --

RegisterNetEvent('srp-phone:signInFishing')
AddEventHandler('srp-phone:signInFishing', function()
