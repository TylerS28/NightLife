...
function getQuantity(pServerId, pItemId)
    local user, amount = exports["srp-fw"]:getModule("Player"):GetUser(pServerId), 0
    ...
    if user then
        ...
        if checkQuality then
            decayDate = exports['srp-inventory']:GetDecayDate(pItemId)
...
function removeItem(pServerId, pItemId, pAmount, pShouldNotify)
    local user = exports["srp-fw"]:getModule("Player"):GetUser(pServerId)
