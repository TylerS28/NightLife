...
local function processStressBlock()
    TriggerEvent("srp-buffs:applyBuff", "joints", { { buff = "stressblock", percent = 1.0 } })
end
...
AddEventHandler("srp-katana:removeEquippedKatana", function()
...
RegisterCommand('+useKeyFob', function()
    if Throttled("srp-doorss:doorKeyFob", 1000) then return end
    TriggerEvent("srp-doors:doorKeyFob")
end, false)
RegisterCommand('-useKeyFob', function() end, false)

Citizen.CreateThread(function()
    exports["srp-keybinds"]:registerKeyMapping("", "Vehicle", "Door Keyfob", "+useKeyFob", "-useKeyFob", "Y")
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon, passedItemInfo)
    ...
    if (isWeapon) then
        ...
        Wait(1500)
        TriggerEvent("AttachWeapons")
        TriggerEvent('srp-hud:armed', 100)
        return
    end

    if itemid == "murdermeal" then
        local finished = exports['srp-taskbar']:taskBar(1000, 'Unwrapping')
        local cid = exports["isPed"]:isPed("cid")
    ...
    if itemid == "bstoy" then
        TriggerEvent('srp-burgershot:gettoy')
        ...
        if GetEntityModel(PlayerPedId()) == `Mr_kebun` then
          if tonumber(ItemInfo.quality) > 0 then
              if not isKatanaEquipped then
                ...
                Citizen.CreateThread(function()
                    Citizen.Wait(1000)
                    TriggerServerEvent("srp-katana:cursedKatanaEquip", GetEntityCoords(PlayerPedId()))
                    TriggerEvent("srp-katana:cursedKatanaEquipC")
                end)
              else
                isKatanaEquipped = false
              end
    ...
    if stolenItems[itemid] and exports["srp-npcs"]:isCloseToPawnPed() then
    ...
    if itemid == "key1" then
        TriggerEvent('srp-doors:unlockContainer1')
    end

    if itemid == "key2" then
        TriggerEvent('srp-doors:unlockContainer2')
    end

    if (itemid == "pdbadge") then
        TriggerEvent("srp-policeBadge:showBadge")
    end
    ...
    if (itemid == "heistlaptop3") then
        TriggerEvent('srp-robbery:usb')
    end 

    if (itemid == "heistlaptop2") then
        TriggerEvent('srp-paleto:UseBlueLapTop')
    end
    ...
    if (itemid == "qtakis") then
        ...
        TriggerEvent('srp-hud:ChangeHunger', 30)
    end
    ...
    if (itemid == "qdrpepper") then
        ...
        TriggerEvent('srp-hud:ChangeThirst', 30)
    end
    ...
    if itemid == "clotion" or itemid == "cabsinthe" then
        remove = true
        local finished = exports['srp-taskbar']:taskBar(2500, 'Consuming Lotion')
        if finished == 100 then
            SetPlayerMaxArmour(PlayerPedId(), 100)
    ...
    if Police >= 4 then
        if targetVehicle ~= 0 and GetHashKey("stockade") == GetEntityModel(targetVehicle) then
            ...
            if aDist < 2.0 then
                ...
                TriggerServerEvent('srp-heists:bankTruckLog')
                TriggerEvent('srp-dispatch:bank_truck_robbery')
                local finished = exports["srp-taskbar"]:taskBar(timeToOpen,"Unlocking Vehicle",false,false,playerVeh)
                if finished == 100 then
                    TriggerEvent('srp-hud:show_hackerman')
                    exports['srp-thermite']:OpenThermiteGame(function(success)
                        if success then
                            TriggerEvent('srp-hud:hide_hackerman')
                            TriggerEvent('inventory:removeItem', 'Gruppe6Card', 1)
                            TriggerEvent("srp-heists:start_hitting_truck", targetVehicle)
                        else
                            TriggerEvent('srp-hud:hide_hackerman')
                            TriggerEvent('inventory:removeItem', 'Gruppe6Card', 1)
                        end
    ...
    if (itemid == "flamethrowerammo") then
        local finished = exports["srp-taskbar"]:taskBar(20000,"Reloading flamethrower",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "huntingammo") then
        local finished = exports["srp-taskbar"]:taskBar(10000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "rpgammo") then
        local finished = exports["srp-taskbar"]:taskBar(25000,"Reloading",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "armor" or itemid == "pdarmor") then
        local finished = exports["srp-taskbar"]:taskBar(4500,"Putting on Armor",true,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "newaccountbox") then
        local finished = exports["srp-taskbar"]:taskBar(5000,"Opening present",false,false,playerVeh)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "housesafe") then
        if not hasEnoughOfItem("heavydutydrill", 1, false) then
            ...
            local finished = exports["srp-taskbar"]:taskBar(30000, "Cracking Safe...", true, false)
            if (finished == 100) then
                remove = true
                TriggerServerEvent('loot:useItem', itemid)
            end
    ...
    if (itemid == "fishingchest") then
        local finished = exports["srp-taskbar"]:taskBar(5000, "Opening Chest", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "repcrate") then
        local finished = exports["srp-taskbar"]:taskBar(30000, "Opening...", true, false)
        if (finished == 100) then
            remove = true
            TriggerServerEvent('loot:useItem', itemid)
        end
    ...
    if (itemid == "babyoil") then
        local finished = exports["srp-taskbar"]:taskBar(10000,"Applying baby oil to dome",true,false,playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "pixellaptop")  then
        TriggerEvent("srp-boosting:open:boostingApp")
    end    
    ...
    if (itemid == "emptyballoon") then
        if (hasEnoughOfItem("nitrous",1,false)) then
            local finished = exports["srp-taskbar"]:taskBar(5000,"Filling Balloon.",false,false,playerVeh)
            if (finished == 100) then
                remove = true
    ...
    if (itemid == "Peecup_empty") then
        ...
        local finished = exports["srp-taskbar"]:taskBar(10000, "Peeing in cup.", false, false, playerVeh)
        if (finished == 100) then
            remove = true
    ...
    if (itemid == "megaphone") then
      TriggerEvent("srp-usableprops:megaphone")
    end
    ...
    if (itemid == "nitrous") then
        ...
        if currentVehicle == nil or currentVehicle == 0 then
            ...
            if (finished == 100 and not cancelNos) then
                TriggerServerEvent('srp-vehicles:ApplyNitrous')
                remove = true
            else
    ...
    if itemid == "rice_balls" then
        ...
        local finished = exports['srp-taskbar']:taskBar(2500, 'Eating...')
        if finished == 100 then
            TriggerEvent('srp-hud:ChangeHunger', 50)
    ...
    if itemid == "chicken_noodle_soup" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['srp-taskbar']:taskBar(5000, 'Slurping Soup...')
        if finished == 100 then
            TriggerEvent('srp-hud:ChangeHunger', 80)
    ...
    if itemid == "doki_doki_pancakes" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['srp-taskbar']:taskBar(5000, 'Eating Pancakes...')
        if finished == 100 then
            TriggerEvent('srp-hud:ChangeHunger', 50)
    ...
    if itemid == "chocolate_cake" then
        TriggerEvent('animation:PlayAnimation', 'eat')
        local finished = exports['srp-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('srp-hud:ChangeHunger', 25)
    ...
    if itemid == "strawberry_shortcake" then
        ...
        local finished = exports['srp-taskbar']:taskBar(5000, 'Eating Cake...')
        if finished == 100 then
            TriggerEvent('srp-hud:ChangeHunger', 45)
    ...
    if (itemid == "softdrink") then
        ...
        TriggerEvent('srp-hud:ChangeThirst', 69)
        TriggerEvent('thirstbuff')
    end
    ...
    if itemid == "torpedo" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('srp-hud:ChangeHunger', 15)
        TriggerEvent('aspect-buffs:bulb_buff')
    end
    ...
    if itemid == "moneyshot" then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('srp-hud:ChangeHunger', 69)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    if (itemid == "pepperonipizza") then
        ...
        --attachPropsToAnimation(itemid, 6000)
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49, 6000, "Eating", "inv:wellfed", true,itemid)
        TriggerEvent('srp-hud:ChangeHunger', 60)
        TriggerEvent('aspect-buffs:money_boost_buff')
    end
    ...
    or itemid == "experiment_09") then
        TriggerEvent("animation:PlayAnimation","pill")
        local finished = exports["srp-taskbar"]:taskBar(3000,"Downing content of bottle. 💊",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "pillbox") then
        local finished = exports["srp-taskbar"]:taskBar(5000, "Opening pill box", false, false, playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "skateboard") then
        TriggerEvent('srp-longboard:PlaceDown')
    end
    ...
    if itemid == "customfooditem" then
        TriggerEvent("srp-hud:ChangeHunger", 100)
    end
    ...
    if itemid == "kdragonwater" then
        ...
        AttachPropAndPlayAnimation(
          ...
          6000,
          "Drink",
          "srp-katana:transformKFromDrink",
          true,
          itemid,
          playerVeh
        )
    ...
    if (itemid == "methlabproduct") then
        local finished = exports["srp-ui"]:taskBarSkill(2400, math.random(12, 18))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if itemid == "ketamine" then
        local finished = exports["srp-ui"]:taskBarSkill(2000, math.random(10, 15))
        if finished == 100 then
            TriggerEvent("attachItemObjectnoanim","drugpackage01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
    ...
    if (itemid == "shitlockpick") then
        ...
        local finished = exports["srp-ui"]:taskBarSkill(1000, math.random(7, 12))
        if (finished == 100) then
    ...
    if itemid == "roostertea" then
        AttachPropAndPlayAnimation("amb@world_human_drinking@coffee@male@idle_a", "idle_c", 49,15000,"Drink","srp-roostersrest:drinkTea",true,itemid,playerVeh)
    end
    ...
    if itemid == "franksflute" then
      TriggerServerEvent("srp-inventory:franksFlute", GetEntityCoords(PlayerPedId()))
    ...
    if itemid == "tattooremover" then
        local target = exports['srp-target']:GetCurrentEntity()
        --local target = PlayerPedId()
        if IsEntityAPed(target) and IsPedAPlayer(target) then
            TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_WELDING", 0, true)
            local finished = exports["srp-taskbar"]:taskBar(60000, "Removing tattoos")
    ...
    if itemid == "buddhaletter" then
        if GetEntityModel(PlayerPedId()) == `ig_buddha` or GetEntityModel(PlayerPedId()) == `g_m_m_buddha` then
            local finished = exports["srp-taskbar"]:taskBar(5000,"Opening letter.",false,false,playerVeh)
            if finished == 100 then
        ...
        if hasEnoughOfItem("buddhablade",1,false) and hasEnoughOfItem("buddhaguard",1,false) and hasEnoughOfItem("buddhahilt",1,false) then
            if hasEnoughOfItem("buddhamedalion",1,false) then
                ...
                if #(playerPos - vector3(-172.54, 319.13, 87.35)) < 10 then
                    local finished = exports["srp-taskbar"]:taskBar(30000,"Putting sword together.",false,false,playerVeh)
                    if (finished == 100) then
    ...
    if itemid == "buddhashovel" then
        ...
        if #(playerPos - vector3(-1911.78, 1388.1, 219.35)) < 10 then
            if BuddhaIsItemFound1 == false then
                ...
                local finished = exports["srp-taskbar"]:taskBar(15000, "Digging for something.", false, true, false, false, nil, 5.0, PlayerPedId())
                ClearPedTasks(PlayerPedId())
                if finished == 100 then
    ...
    TriggerEvent("srp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
    TriggerServerEvent("srp-inventory:itemUsed", itemid, passedItemInfo, inventoryName, slot)
...
AddEventHandler("srp-inventory:attachPropPlayAnim", function(pType)
    ...
    TriggerEvent("srp-buffs:itemUsedSuccess", success)
end)
...
RegisterNetEvent('srp-business:repairItem')
AddEventHandler('srp-business:repairItem', function()
    
    local info = GetItemInfo(1)
    local Quality = info.quality
    local QualityCheck = (Quality < 80 and Quality or false)
    local PercentRepair = 80 - Quality
    local itemListInfo = json.decode(exports["srp-inventory"]:itemListInfo(info.item_id))
    local allowed = true  
...
RegisterNetEvent('srp-business:examineItem')
AddEventHandler('srp-business:examineItem', function()
...
function checkForItems()
    ...
    for _, item in ipairs(items) do
        ...
        if hasItem and not PreviousItemCheck[item] then
            ...
            TriggerEvent('srp-inventory:itemCheck', item, true, quantity)
        elseif not hasItem and PreviousItemCheck[item] then
            PreviousItemCheck[item] = false
            TriggerEvent('srp-inventory:itemCheck', item, false, quantity)
        end
    end
end
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["srp-ui"]:taskBarSkill(15000,3)

            if finished ~= 100 then
                 lockpicking = false
                return
            end
            exports['srp-dispatch']:dispatchadd('10-99B', 'Stolen Vehicle', '227')

            local finished = exports["srp-ui"]:taskBarSkill(2200,4)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["srp-ui"]:taskBarSkill(math.random(5000,10000),math.random(10,15))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            
            TriggerEvent('srp-dispatch:stolenCar', GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)))
            ...
            local finished = exports["srp-ui"]:taskBarSkill(1500,math.random(5,10))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["srp-ui"]:taskBarSkill(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["srp-ui"]:taskBarSkill(timer,math.random(4,7))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
local nvgItems = {
  ...
  AddEventHandler("srp-inventory:itemUsed", function(item)
...
function getInformationFromFirstItem(itemToFind)
  local foundItem = exports["srp-inventory"]:GetInfoForFirstItemOfName(itemToFind)
...
AddEventHandler("srp-inventory:itemUsed", function(name, info)
...
RegisterInterfaceCallback("srp-ui:inv:moneycaseAction", function(data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
  exports['srp-ui']:closeApplication('textbox')
  RPC.execute("srp-inventory:doMoneycaseAction", data)
end)

AddEventHandler("srp-inventory:openGolfStore", function(name, info)
...
function openBurner(info)
    ...
    exports["srp-ui"]:openApplication("burner", info)
end
...
AddEventHandler("ammonium_bicarbonate:insert", function(pOriginInventory, pTargetInventory, pOriginSlot, pTargetSlot, pOriginItemId, pTargetItemId, pOriginItemInfo, pTargetItemInfo)
    ...
    local finished = exports["srp-taskbar"]:taskBar(5000 * quantity, "Breaking down into baggies...", false, false)
...
function notepadUsed()
    exports["srp-ui"]:openApplication("notepad", {
        canSave = true
    })
    TriggerEvent("animation:PlayAnimation", "notepad")
end

RegisterInterfaceCallback('srp-ui:cancelNotepadEmote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent("animation:PlayAnimation", "cancel")
end)

RegisterInterfaceCallback('srp-ui:createNotepadNote', function (data, cb)
    cb({ data = {}, meta = { ok = true, message = '' } })

    exports["srp-ui"]:closeApplication("notepad")
