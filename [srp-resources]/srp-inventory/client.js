...
on('srp-fw:playerSpawned', async (broughtData) => {
...
on('inventory:open_hidden', (penis, vehicleFound) => {
    let vehId = exports['srp-vehicles'].GetVehicleIdentifier(vehicleFound)
...
on('inventory-open-request', async () => {
  ...
  //  const apartmentFloor = exports["srp-apartments"].getModule("func").getApartment()
    let targetid = 0;
    cid = exports.isPed.isPed("cid");
    ...
    const currentTarget = exports['srp-target'].GetCurrentEntity()
    ...
    if (isInVehicle) {
       ...
       // let serverCode = exports["srp-config"].GetServerCode();
...
on('__cfx_nui:ServerCloseInventory', (data, cb) => {
    ...
    if (data.name != 'none') {
        ...
        emit('srp-inventory:closed', data.name);
    }
    cb({});
});
...
on('__cfx_nui:craftProgression', (data, cb) => {
    cb("ok");
    emit("srp-inventory:craftProgression", data);
    emitNet("srp-inventory:craftProgression", data);
});
...
on('__cfx_nui:move', (data, cb) => {
    if (global.exports['srp-taskbar'].pInInv()) {
        emit('DoLongHudText', 'Nice try exploiting mate.', 2)
        emitNet('srp-inventory:exploiterKick')
    } else {
...
function SetCustomNuiFocus(hasKeyboard, hasMouse) {
    ...
    //   if (HasNuiFocus === true) {
    //   	emit("srp-voice:focus:set", HasNuiFocus, hasKeyboard, hasMouse);
    //   } else {
    // 	  setTimeout(() => {if (HasNuiFocus !== true) emit("srp-voice:focus:set", false, false, false);}, 1000)
    //   }
}
...
exports('setPlayerWeight', (cid, weight) => {
    ...
    emitNet('srp-inventory:server:weightChange', cid, weight);
