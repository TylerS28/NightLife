...
RegisterNetEvent("srp-nitro:client:placeNitro")
AddEventHandler("srp-nitro:client:placeNitro", function() 
...
RegisterNetEvent('srp-nitro:client:particlefx')
AddEventHandler('srp-nitro:client:particlefx', function (veh)	
	local exhaustNames = {
...
RegisterNetEvent('srp-nitro:client:particlefisfis')
AddEventHandler('srp-nitro:client:particlefisfis', function (type, veh)
		...
		if nitroactive and pNitrousLevel > 0 and IsPedInVehicle(PlayerPedId(), GetVehiclePedIsIn(PlayerPedId(), false), false) then
			...
			if fisfismode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- fışfış
					TriggerServerEvent('srp-nitro:server:particlefisfis', 'fisfisacc', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
				else
					pNitrousInUse = false
					TriggerServerEvent('srp-nitro:server:particlefisfis', 'fisfiskapatt', nitroveh)
				end
			elseif nitromode then
				if IsControlPressed(0, 21) and nitroveh == GetVehiclePedIsIn(PlayerPedId(), false) then -- nitro
					TriggerServerEvent('srp-nitro:server:particlefx', nitroveh)
					SetVehicleBoostActive(nitroveh, 4) --Boost Sound
					pNitrousInUse = true
					pNitrousLevel = pNitrousLevel - 0.1
...
AddEventHandler('onResourceStart', function (resource)
	pNitrousLevel = 0
	nitroactive = false
	TriggerServerEvent('srp-nitro:server:particlefisfis', 'fisfiskapatt')
	nitroveh = nil
	pNitrousInUse = false 
end)
...
Citizen.CreateThread(function()
    exports["srp-keybinds"]:registerKeyMapping("", "Nitrous", "Change Mode", "+nosMode", "-nosMode", "")

	exports["srp-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Higher", "+changeFlowPlus", "-changeFlowPlus", "")

	exports["srp-keybinds"]:registerKeyMapping("", "Nitrous", "Flow Rate Lower", "+changeFlowMinus", "-changeFlowMinus", "")
end)