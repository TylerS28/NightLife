...
RegisterServerEvent("srp-vehicles:checkrepo")
AddEventHandler("srp-vehicles:checkrepo", function(plate)
    local src = source
...
-- RegisterServerEvent("srp-vehicles:repo")
-- AddEventHandler("srp-vehicles:repo", function(plate)
--     local src = source
-- 	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                         ['@repoed'] = "1"
--                     })
--                     TriggerClientEvent("srp-vehicles:repo:success", src)
...
-- RegisterServerEvent("srp-vehicles:judgerepo")
-- AddEventHandler("srp-vehicles:judgerepo", function(plate)
--     local src = source
-- 	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
-- RegisterServerEvent("srp-vehicles:judgerepodelete")
-- AddEventHandler("srp-vehicles:judgerepodelete", function(plate)
--     local src = source
-- 	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
--     exports.oxmysql:execute("SELECT `license_plate`, `repoed` FROM `characters_cars` WHERE license_plate = ?", {char.id}, function(data)
...
-- RegisterServerEvent("srp-vehicles:repo2")
-- AddEventHandler("srp-vehicles:repo2", function(plate)
--     local src = source
-- 	local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
--     local char = user:getCurrentCharacter()
...
--                     ['@vehicle_state'] = "In"
--                 })
--                 TriggerClientEvent("srp-vehicles:repo:success2", src)
--             else
--                 TriggerClientEvent("srp-vehicles:repo:success2", src)
--             end
--         end
...
-- RegisterServerEvent("srp-vehicles:release:vehicle")
-- AddEventHandler("srp-vehicles:release:vehicle", function(ServerID, plate)
...
--         ['repoed'] = "0"
--     })
--     exports['srp-exports']:removeCash(SrcID, 500)
--     TriggerClientEvent('DoLongHudText', SrcID, 'Your vehicle has been released!', 1)
-- end)

RegisterCommand('towbill', function(source, args)
    local src = source
	local user = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(args[1]))
	local character = user:getCurrentCharacter()
	local user2 = exports["srp-fw"]:getModule("Player"):GetUser(src)
	...
	exports.oxmysql:execute('SELECT * FROM jobs_whitelist WHERE cid = ?', {character2.id}, function(result)
		if result[1].job == 'towunion' then
			...
			TriggerClientEvent("srp-vehicles:towBill", src, tonumber(args[2]))
		else
		end
	end)
end)