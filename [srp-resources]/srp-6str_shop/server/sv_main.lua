...
RegisterServerEvent('srp-6str_shop:CarTableTunerShop')
AddEventHandler('srp-6str_shop:CarTableTunerShop', function(table)
    if table ~= nil then
        carTable = table
        TriggerClientEvent('srp-6str_shop:ReturnVehicleTunerTable', -1, carTable)
        for i=1, #carTable do
...
RegisterServerEvent('srp-6str_shop:FinaceEnabledSV')
AddEventHandler('srp-6str_shop:FinaceEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('srp-6str_shop:FinaceEnabledCL', -1, plate)
    end
end)

RegisterServerEvent('srp-6str_shop:BuyEnabledSV')
AddEventHandler('srp-6str_shop:BuyEnabledSV', function(plate)
    if plate ~= nil then
        TriggerClientEvent('srp-6str_shop:BuyEnabledCL', -1, plate)
    end
end)

-- return table
-- TODO (return db table)
RegisterServerEvent('srp-6str_shop:RequestTableTunerShopCars')
AddEventHandler('srp-6str_shop:RequestTableTunerShopCars', function()
    local user = source
    exports.oxmysql:execute("SELECT * FROM vehicle_display", {}, function(display)
        ...
        TriggerClientEvent('srp-6str_shop:ReturnVehicleTunerTable', user, carTable)
    end)
end)

-- Check if player has enough money
RegisterServerEvent('srp-6str_shop:ChechMoney')
AddEventHandler('srp-6str_shop:ChechMoney', function(name, model, price)
    local src = source
    ...
    elseif tonumber(cash) <= price then
        ...
        TriggerClientEvent('srp-6str_shop:FailedPurchase', src)
    end
end)

RegisterServerEvent('srp-6str_shop:BuyVehicle')
AddEventHandler('srp-6str_shop:BuyVehicle', function(plate, name, vehicle, price, personalvehicle)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
...
AddEventHandler('srp-cardealer:log', function(cid, model, price, plate, carDealer)
    ...
    local connect = {
      ...
      PerformHttpRequest("https://discord.com/api/webhooks/1012082705128759366/CC3mBxVRZgSGPz87L_MuQzldIqf-Y-y50p0BSfDQyD1wfwUVT-YpopWgbZtK5JGJeHMI", function(err, text, headers) end, 'POST', json.encode({username = "qpixel", embeds = connect, avatar_url = "https://i.imgur.com/hMqEEQp.png"}), { ['Content-Type'] = 'application/json' })
end)