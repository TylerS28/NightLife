...
function attemptToLockPickHouse(skipPicking)
    ...
    if not skipPicking then
        if not Housing.housingBeingRobbedClient[propertyID] then
            ...
            local finished = exports["srp-taskbar"]:taskBar(math.random(5000, 10000), "Breaking in")
...
function alarm(clearAlarm)
    if Housing.alarm then
        local alarmOffset = exports["srp-build"]:getModule("func").getAlarm()
...
function displayPickup(force,itemName)
    ...
    if Housing.currentlyDisplayingPickup == false and not force then
        ...
        if type(itemName) == "string" then
            exports["srp-textui"]:showInteraction("[E] Take Object")
        else
            exports["srp-textui"]:showInteraction("[E] Look for some belongings.")
        end
    end
end
...
function interactRob()
    ...
    if type(item) == "string" then
        local weight = exports["srp-inventory"]:getCurrentWeight()
    ...
    if type(item) == "string" then
        ...
        local finished = exports["srp-taskbar"]:taskBar(timer,"Securing Item")
        ClearPedTasks(PlayerPedId())
        ...
        TriggerEvent("animation:PlayAnimation","search")
        local finished = exports["srp-taskbar"]:taskBar(15000,"Procurando")
        ClearPedTasks(PlayerPedId())
        if finished ~= 100 then
            return
        end
    ...
    if finished then
        if type(item) == "string" then
            ...
            if euemeusmanosodiamosoartmike >= 90 then
                TriggerServerEvent("srp-heists:complete", math.random(30, 50))
...
RegisterNetEvent("srp-weathersync:currentTime", function(pHour, pMinute)
...
RegisterNetEvent("srp-housing:alarmFail")
AddEventHandler("srp-housing:alarmFail", function()
...
RegisterNetEvent("srp-housing:alarmEnter")
AddEventHandler("srp-housing:alarmEnter", function()
    insideGame = true
    local successevrp
...
AddEventHandler("housing:exitFrontDoor", function()
    ...
    TriggerServerEvent("srp-housing:leftHouse", Housing.currentHousingInteractions.id)
...
RegisterNetEvent("srp-housing:alarm")
AddEventHandler("srp-housing:alarm", function(pHouseId)
    if pHouseId == Housing.currentlyRobInside then
        local gameTimer = GetGameTimer() + 30000


        while true do
            local alarmOffset = exports["srp-build"]:getModule("func").currentBuildingVector()
...
RegisterNetEvent("srp-housing:beingRobbed", function(pPropertyId)
