...
function Housing.func.enterBuilding(propertyID,enterOverride,counterPart)
    ...
    local isBuiltCoords, objects = exports["srp-build"]:getModule("func").buildRoom(model,spawnBuildingLocation,false,Housing.destroyedObjects,enterOverride,true)
    if counterPart and Housing.staticObjectRobPoints == nil then
        Housing.staticObjectRobPoints = exports["srp-build"]:getModule("func").getRobLocationsForObjects(model,spawnBuildingLocation,Housing.housingRobTargets.static)
    ...
    if isBuiltCoords then
        ...
        TriggerServerEvent("srp-housing:enterHouse", propertyID)

        if not counterPart then
            TriggerEvent("srp-editor:buildName", propertyID, objects)
        end
    else
        Housing.currentHousingInteractions = nil
        Housing.currentlyInsideBuilding = false
...
RegisterNetEvent("srp-housing:loadHousingClient")
AddEventHandler("srp-housing:loadHousingClient", function()
    gatherPlayerInfo()
end)

RegisterNetEvent("srp-housing:reset")
AddEventHandler("srp-housing:reset", function()
    -- cl_housing
    Housing.currentOwned = {}
    Housing.currentKeys = {}
    Housing.currentlyEditing = false
...
RegisterNetEvent("srp-housing:refresh")
AddEventHandler("srp-housing:refresh", function()
    gatherPlayerInfo()
end)
