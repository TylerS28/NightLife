...
RegisterNetEvent("srp-housing:enterBUILDINGCMON", function()
    InteractionPressed()
end)
...
RegisterCommand("srp-housing:getGarages", function()
...
function getIsPropLock()
    ...
    local pp =  RPC.execute("srp-housing:isHousingLockedServer", _propertyID)
...
AddEventHandler('srp-housing:phone:check', function(type)
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
    ...
    local isThisPropertyOwned = RPC.execute("srp-housing:isThisPropertyOwned", propertyId)
    ...
    if type == "edit" then
        ...
        TriggerEvent('srp-phone:editingHouse',id,street,price)
    else
        TriggerEvent('srp-phone:sentHouse',nil,id,street,price,category)
...
function setInventory()
    ...
    if Housing.currentlyInsideBuilding then
        ...
        local buildingVector = exports["srp-build"]:getModule("func").currentBuildingVector()
...
function exitingBuilding()
    ...
    if id then
        TriggerEvent("srp-editor:destroyName", id)
    end
...
function Housing.func.loadInteractions(model,overWrite)
    ...
    local isBuiltCoords = exports["srp-build"]:getModule("func").addInteractionPoints(model,interactionPoints,overWrite)
end
...
AddEventHandler("srp-housing:sell", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
AddEventHandler("srp-housing:sellTo", function(pParams)
    local input = exports["srp-ui"]:showInput({
		{
            icon = "id-card",
            label = "PayPal ID",
            name = "cid",
	...
	if input["cid"] then
		...
		TriggerServerEvent("srp-housing:sell", cid, pParams.propertyId)
	end
end)

RegisterNetEvent("srp-housing:buy")
AddEventHandler("srp-housing:buy", function(pPropertyId, pSeller)
...
RegisterCommand('rent', function()
    TriggerEvent('srp-housing:rent')
end)

RegisterNetEvent("srp-housing:rent", function()
    local isComplete, propertyId, dist, zone = Housing.func.findClosestProperty()
    if not isComplete then
        TriggerEvent("DoLongHudText", propertyId, 2)
        return
...
RegisterNetEvent("srp-housing:rent2", function(pParams)
    local complete, info = RPC.execute("srp-housing:rent", pParams.propertyId, pParams.total, pParams.tax)
...
RegisterNetEvent("srp-housing:buyed", function(pParams)
    print('hello?')
    local complete, info = RPC.execute("srp-housing:buy", pParams.propertyId, pParams.total, pParams.tax, pParams.seller)
...
RegisterNetEvent("srp-housing:edit", function()
end)

RegisterNetEvent("srp-housing:editOptions", function(pParams)
...
AddEventHandler("housing:charLogout", function()
    ...
    if max.canHaveCharSelect then
        ...
        exports["srp-build"]:getModule("func").CleanUpArea()
        local cid = exports["isPed"]:isPed("cid")
        TriggerServerEvent("srp-jobmanager:onCharSwap", cid)
        Citizen.Wait(1000)   
        TriggerEvent("srp-fw:clearStates")
        exports["srp-ui"]:sendAppEvent("hud", { display = false })
        TriggerServerEvent("apartments:cleanUpRoom")
        exports["srp-fw"]:getModule("SpawnManager"):Initialize()
...
AddEventHandler("housing:frontdoor", function()
    DoScreenFadeOut(1)
    exports["srp-build"]:getModule("func").exitCurrentRoom(Housing.info[Housing.currentHousingInteractions.id]["pos"])
    exitingBuilding()
    DoScreenFadeIn(1900)
end)
...
AddEventHandler("housing:internalBackdoor", function()
    ...
    if max.canHaveBackDoor then
        ...
        DoScreenFadeOut(1)
        exports["srp-build"]:getModule("func").exitCurrentRoom(Housing.currentHousingInteractions.backdoor_coordinates.external)
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)
...
AddEventHandler("housing:kickFromHouse", function(propertyID)
    ...
    if id == propertyID then
        DoScreenFadeOut(1)
        exports["srp-build"]:getModule("func").exitCurrentRoom(Housing.info[id]["pos"])
        exitingBuilding()
        DoScreenFadeIn(1900)
    end
end)

RegisterNetEvent("srp-housing:blips")
AddEventHandler("srp-housing:blips", function()
    Housing.Blips = not Housing.Blips

    local houses = RPC.execute("srp-housing:owned")
...
RegisterNetEvent('srp-housing:logShit')
AddEventHandler('srp-housing:logShit', function(hID, Price)
    TriggerServerEvent('srp-housing:buyLog', hID, Price)
end)