...
RegisterNetEvent("srp-housing:sell")
AddEventHandler("srp-housing:sell", function(pCid, pPropertyId)
    local src = source
    ...
    print('here')
    TriggerClientEvent("srp-housing:buy", pCid, pPropertyId, seller)
end)
...
AddEventHandler("CheckFurniture", function(pData, pPropertyId)
    ...
    if housingEditing[pPropertyId] ~= nil then
        ...
        TriggerClientEvent("srp-editor:loadEditor", src, pData)
    end
end)
...
RPC.register("srp-housing:buy", function(src, pPropertyId, pTotal, pTax, pSeller)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    ...
    TriggerClientEvent("srp-housing:loadHousingClient",src)
    TriggerClientEvent('srp-housing:logShit', src, pPropertyId, pTotal)
    return true, "Purchase Successful" 
end)

RPC.register("srp-housing:rent", function(src, pPropertyId, pTotal, pTax)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    ...
    --local success, message = exports["srp-financials"]:transaction(accountId, groupBank, pTotal, comment, cid, 5)
    --if not success then
    --    return false, message
    --end

    --exports["srp-financials"]:addTax("Propertys", pTax)
...
RPC.register("updateCurrentSelected", function(src, pPropertyId, pInformation, pOrigin)
    ...
    --exports["srp-vehicles"]:setGarage(name, data, nil, false)
...
RPC.register("srp-housing:isHousingLockedServer", function(pSrc, pHid)
    print(pHid)
...
RPC.register("srp-housing:owned", function(src)
...
RegisterNetEvent("srp-housing:getGarages", function()
    local src = source
    ...
    for i, v in ipairs(houses) do
        ...
        TriggerClientEvent("menu:send:srp-housing:garages", src,json.decode(garagedata),v.hid)
    end
end)