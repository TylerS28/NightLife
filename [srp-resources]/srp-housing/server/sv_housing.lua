...
GetUserByCid = function(cid)
    for index, value in pairs(GetPlayers()) do
        local xPlayer = exports['srp-fw']:getModule("Player"):GetUser(tonumber(value))
...
RPC.register("srp-housing:isThisPropertyOwned", function(src, propertyId)
    local result = MySQL.query.await([[
...
RegisterNetEvent("srp-housing:enterHouse")
AddEventHandler("srp-housing:enterHouse", function(pPropertyId)
    local src = source
    Housing.Players[src] = pPropertyId
end)

RegisterNetEvent("srp-housing:leftHouse")
AddEventHandler("srp-housing:leftHouse", function(pPropertyId)
    local src = source
...
RPC.register("srp-phone:getHouseKeys", function(src, pHouseId)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return {} end
...
RPC.register("srp-phone:giveKey", function(src, pHouseId, pPlayerId)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false, "ID não encontrado" end

    local user2 = exports['srp-fw']:getModule("Player"):GetUser(pPlayerId)
    ...
    TriggerClientEvent("srp-phone:notification", pPlayerId, "fas fa-key", "Keys", "Você recebeu a chave da propriedade " .. Housing.info[pHouseId]["street"], 5000)
    TriggerClientEvent("srp-housing:refresh", pPlayerId)

    return true, "Chave recebida"
end)

RPC.register("srp-phone:removeKey", function(src, pHouseId, pPlayerId)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
...
RPC.register("srp-phone:payHouse", function(src, pHouseId)
  ...
  --  local tax = price
   ...
   -- local accountId = exports["srp-fw"]:getChar(src, "bankid")
   -- local bank = exports["srp-financials"]:getBalance(accountId)
   -- if bank < priceTotal then
   --     return false, "Você não tem $" .. priceTotal .. " em sua conta"
    --end

    --local groupBank = exports["srp-groups"]:groupBank("real_estate")

    --local comment = "Pagamento de aluguel da propriedade " .. Housing.info[pHouseId]["street"]
    --local success, message = exports["srp-financials"]:transaction(accountId, groupBank, price, comment, cid, 5)
    --if not success then
      --  return false, message
    --end

    --exports["srp-financials"]:addTax("Propertys", tax.tax)
...
RPC.register("srp-phone:removeSharedKey", function(src, pHouseId)
    local user = exports['srp-fw']:getModule("Player"):GetUser(src)
    local character = user:getCurrentCharacter()
    local cid = character.id
    if not cid then return false end
    ...
    if affectedRows and affectedRows > 0 then
        TriggerClientEvent("srp-housing:refresh", src)
        return true
    end
...
RegisterServerEvent('srp-housing:buyLog')
AddEventHandler('srp-housing:buyLog', function(pHouse, price)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
