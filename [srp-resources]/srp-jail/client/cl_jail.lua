...
function drink()
    CleqpixeledSecondaryTask(PlayerPedId())
...
RegisterNetEvent('srp-jail:checkTime')
AddEventHandler('srp-jail:checkTime', function()
...
RegisterNetEvent('srp-jailmenu')
AddEventHandler('srp-jailmenu', function()

	local Jailmenu = {
		{
            title = "List Lawyers",
            description = "Check how many lawyers are in the city!",
            action = "srp-jail:lawyerspog",
        },
        {
            title = "Jail Time",
            description = "Check your jail time",
            action = "srp-jail:checktime2",
        },
        {
            ...
            children = {
				{
                    title = "Yes",
                    action = "srp-jail:swapchar",
                },
				{
                    title = "No",
                    action = "srp-jail:swap_char_no",
                },
            },
        },
    }
    exports["srp-interface"]:showContextMenu(Jailmenu)
end)

RegisterInterfaceCallback('srp-jail:swap_char_no', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
end)

RegisterInterfaceCallback('srp-jail:swapchar', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('swappingCharsLoop')
end)

RegisterInterfaceCallback('srp-jail:checktime2', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('srp-jail:checkTime')
end)

RegisterInterfaceCallback('srp-jail:lawyerspog', function(data, cb)
	cb({ data = {}, meta = { ok = true, message = '' } })
    TriggerEvent('yellowPages:retrieveLawyersOnline')
end)

exports["srp-polytarget"]:AddCircleZone("jail_interactions", vector3(1828.76, 2579.78, 46.51), 0.3, {
    useZ = true
})

exports["srp-interact"]:AddPeekEntryByPolyTarget("jail_interactions", {
    {
        event = "srp-jailmenu",
        id = "jail_interact_3",
        icon = "circle",
