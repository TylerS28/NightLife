...
RPC.register("BankWithdraw", function(pSource, pId, pAmount, pComment, pTransId, pDate)
    ...
    --TriggerClientEvent("srp-financials:transferUpdate", pSource, ply, newbankamount)
...
RPC.register("DoTransaction", function(pSource, pId, pAmount, pComment, pTransId, pDate, pTarget)
    ...
    if id == "personal" then
        ...
        local userSender = exports["srp-fw"]:getModule("Player"):GetUser(pSource)
        ...
        local userReceiver = exports["srp-fw"]:getModule("Player"):GetUser(target)
...
AddEventHandler('bank:givemecash', function(reciever, amount)
  local src = source
  local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
  local player = exports["srp-fw"]:getModule("Player"):GetUser(tonumber(reciever))
  if tonumber(amount) <= user:getCash() then
    ...
    TriggerEvent('srp-fw:bankgivecash', src, tonumber(receiver), pName, pSteam, pDiscord, amount, tonumber(user:getCash()), tonumber(user:getBalance()), amount, tonumber(player:getCash()), tonumber(player:getBalance()))
  else
...
RegisterServerEvent('srp-financials:addMoney')
AddEventHandler('srp-financials:addMoney', function(amount)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    user:addMoney(tonumber(amount))
end)

RegisterServerEvent('srp-financials:removeMoney')
AddEventHandler('srp-financials:removeMoney', function(amount)
    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    user:removeMoney(tonumber(amount))
end)

RegisterServerEvent("srp-financials:cash:get")
AddEventHandler('srp-financials:cash:get', function(amount)

    local src = source
    local user = exports["srp-fw"]:getModule("Player"):GetUser(src)
    local cash = user:getCash()
    TriggerClientEvent("srp-financials:cash",cash)
end)

RegisterServerEvent("srp-financials:business_money", function(amt, account, case)
...
RegisterServerEvent('srp-financials:deposit_log')
AddEventHandler('srp-financials:deposit_log', function(pDepositAmt, pDepositComment)
    ...
    PerformHttpRequest(Deposit_Log, function(err, text, headers) end, 'POST', json.encode({username = "qpixel",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)
...
RegisterServerEvent('srp-financials:withdraw_log')
AddEventHandler('srp-financials:withdraw_log', function(pWithDrawAmt, pWithDrawComment)
    local pName = GetPlayerName(source)
    local connect = {
        {
            ["color"] = "16777215",
    ...
    PerformHttpRequest(Withdraw_Log, function(err, text, headers) end, 'POST', json.encode({username = "qpixel",  avatar_url = "https://cdn.discordapp.com/attachments/991122949828530258/992926357413711872/unknown.png",embeds = connect}), { ['Content-Type'] = 'application/json' })
end)