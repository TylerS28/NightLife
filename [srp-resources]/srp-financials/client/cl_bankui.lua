...
RegisterNetEvent("srp-fw:addedMoney")
AddEventHandler("srp-fw:addedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:addCash", amt)
end)

RegisterNetEvent("srp-fw:removedMoney")
AddEventHandler("srp-fw:removedMoney", function(amt, total)
  TriggerEvent("banking:updateCash", total)
  TriggerEvent("banking:removeCash", amt)
end)

RegisterNetEvent("srp-financials:cash")
AddEventHandler("srp-financials:cash", function(pCash, pChange)
    if not pCash then return false, "No cash" end
    exports["srp-ui"]:cashFlash(pCash, pChange)
end)


RegisterCommand("cashh", function()
    TriggerServerEvent("srp-financials:cash:get")
end)
